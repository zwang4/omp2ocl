//-------------------------------------------------------------------------------
//Host code 
//Generated at : Tue Aug  7 13:28:33 2012
//Compiler options: 
//      Software Cache  true
//      Local Memory    true
...
static void compute_rhs()
{
    ...
    startTimer (&ti);
#endif
	oclSwapDimensions (__ocl_buffer_rhs, 5 * (36 / 2 * 2 + 1), 36 / 2 * 2 + 1, 1,
			  36 / 2 * 2 + 1, sizeof(double));
	oclSwapDimensions (__ocl_buffer_u, 5 * (36 / 2 * 2 + 1), 36 / 2 * 2 + 1, 1,
			  36 / 2 * 2 + 1, sizeof(double));
#ifdef PROFILE_SWAP
    oclSync();
    stopTimer (&ti);
...
static void init_ocl_runtime()
{
	...
	__ocl_program = oclBuildProgram("sp.W.cl");
...
static void create_ocl_buffers()
{
	...
	__ocl_buffer_cv = oclCreateBuffer(cv, (36 * 36 * 36) * sizeof(double));
	...
	__ocl_buffer_rhon =
	    oclCreateBuffer(rhon, (36 * 36 * 36) * sizeof(double));
	...
	__ocl_buffer_rhoq =
	    oclCreateBuffer(rhoq, (36 * 36 * 36) * sizeof(double));
	...
	__ocl_buffer_rhos =
	    oclCreateBuffer(rhos, (36 * 36 * 36) * sizeof(double));
