				     ...
				     __global double *__ocl_part_sy)
{
	...
	double x[2048];		/* threadprivate: defined at ep.c : 52 */
	...
	//-------------------------------------------
	{
		__private double4 __vl4;

		unsigned x_offset = 0;
		for (unsigned __i_cix = 0; __i_cix < 512; __i_cix++) {
			__vl4 = vload4(0, __ocl_copyin_x);
			//vstore4(__v14, 0, (__private double*) x);
			//x += 4;
			x[(x_offset)] = __vl4.x;
			x_offset++;
			x[(x_offset)] = __vl4.y;
	...
	//-------------------------------------------
	{
		...
		myvranlc(2 * (1 << 10), &t1, 1220703125.0, x - 1);
		for (i = 0; i < (1 << 10); i++) {
			x1 = 2.0 * x[2 * i] - 1.0;
			x2 = 2.0 * x[2 * i + 1] - 1.0;
			t1 = ((x1) * (x1)) + ((x2) * (x2));
