//-------------------------------------------------------------------------------
//OpenCL Kernels 
//Generated at : Mon Aug  6 14:13:32 2012
//Compiler options: 
//      Software Cache  true
//      Local Memory    true
		     ...
		     int __ocl_k_bound, int __ocl_j_bound, int __ocl_i_bound)
{
	...
	__global double (*cv)[102][102] = (__global double (*)[102][102])g_cv;
	__global double (*us)[103][103] = (__global double (*)[103][103])g_us;
	__global double (*rhon)[102][102] =
	    (__global double (*)[102][102])g_rhon;
	//-------------------------------------------
	//Convert global memory objects (END)
	//-------------------------------------------
		     ...
		     int __ocl_k_bound, int __ocl_j_bound, int __ocl_i_bound)
{
	...
	__global double (*rhoq)[102][102] =
	    (__global double (*)[102][102])g_rhoq;
	//-------------------------------------------
	//Convert global memory objects (END)
	//-------------------------------------------
		     ...
		     int __ocl_k_bound, int __ocl_j_bound, int __ocl_i_bound)
{
	...
	__global double (*rhos)[102][102] =
	    (__global double (*)[102][102])g_rhos;
	//-------------------------------------------
	//Convert global memory objects (END)
	//-------------------------------------------
			     ...
			     int __ocl_i_bound)
{
	...
	{
		//-------------------------------------------
		//Declare prefetching Buffers (BEGIN) : 1785
		//-------------------------------------------
		double2 u_5;
		//-------------------------------------------
		//Declare prefetching buffers (END)
			     ...
			     int __ocl_i_bound)
{
	...
	{
		//-------------------------------------------
		//Declare prefetching Buffers (BEGIN) : 1805
		//-------------------------------------------
		double4 u_6;
		//-------------------------------------------
		//Declare prefetching buffers (END)
			     ...
			     int __ocl_j_bound, int __ocl_i_bound)
{
	...
	for (m = 0; m < 5; m++) {
		//-------------------------------------------
		//Declare prefetching Buffers (BEGIN) : 1825
		//-------------------------------------------
		double4 u_7;
		//-------------------------------------------
		//Declare prefetching buffers (END)
		...
		rhs[m][i][j][k] =
		    rhs[m][i][j][k] - dssp * (u_7.x /*u[m][i][j][k - 2] */  -
					      4.0 *
					      u_7.y /*u[m][i][j][k - 1] */  +
					      6.0 * u_7.z /*u[m][i][j][k] */  -
					      4.0 *
			     ...
			     int __ocl_i_bound)
{
	...
	{
		//-------------------------------------------
		//Declare prefetching Buffers (BEGIN) : 1848
		//-------------------------------------------
		double4 u_8;
		//-------------------------------------------
		//Declare prefetching buffers (END)
			     ...
			     int __ocl_i_bound)
{
	...
	{
		//-------------------------------------------
		//Declare prefetching Buffers (BEGIN) : 1868
		//-------------------------------------------
		double2 u_9;
		//-------------------------------------------
		//Declare prefetching buffers (END)
			...
			int __ocl_i_bound)
{
	...
	//-------------------------------------------
	{
		for (k = 0; k <= grid_points[2] - 3; k++) {
			k1 = k + 1;
			k2 = k + 2;
			fac1 = 1. / lhs[n + 2][i][k][j];
			lhs[n + 3][i][k][j] = fac1 * lhs[n + 3][i][k][j];
			lhs[n + 4][i][k][j] = fac1 * lhs[n + 4][i][k][j];
			for (m = 0; m < 3; m++) {
				rhs[m][i][k][j] = fac1 * rhs[m][i][k][j];
			}
			lhs[n + 2][i][k1][j] =
			    lhs[n + 2][i][k1][j] - lhs[n +
						       1][i][k1][j] * lhs[n +
									  3][i]
			    [k][j];
			lhs[n + 3][i][k1][j] =
			    lhs[n + 3][i][k1][j] - lhs[n +
						       1][i][k1][j] * lhs[n +
									  4][i]
			    [k][j];
			for (m = 0; m < 3; m++) {
			...
			lhs[n + 1][i][k2][j] =
			    lhs[n + 1][i][k2][j] - lhs[n +
						       0][i][k2][j] * lhs[n +
									  3][i]
			    [k][j];
			lhs[n + 2][i][k2][j] =
			    lhs[n + 2][i][k2][j] - lhs[n +
						       0][i][k2][j] * lhs[n +
									  4][i]
			    [k][j];
			for (m = 0; m < 3; m++) {
			...
			int __ocl_i_bound)
{
	...
	//-------------------------------------------
	{
		...
		lhs[n + 2][i][k1][j] =
		    lhs[n + 2][i][k1][j] - lhs[n + 1][i][k1][j] * lhs[n +
								      3][i][k]
		    [j];
		lhs[n + 3][i][k1][j] =
		    lhs[n + 3][i][k1][j] - lhs[n + 1][i][k1][j] * lhs[n +
								      4][i][k]
		    [j];
		for (m = 0; m < 3; m++) {
			rhs[m][i][k1][j] =
		...
		fac2 = 1. / lhs[n + 2][i][k1][j];
		for (m = 0; m < 3; m++) {
			rhs[m][i][k1][j] = fac2 * rhs[m][i][k1][j];
		}
	}
	//-------------------------------------------
	//OpenCL kernel (END)
			...
			int __ocl_i_bound)
{
	...
	//-------------------------------------------
	{
		...
		lhs[n + 3][i][k1][j] =
		    ...
		    [j];
		rhs[m][i][k1][j] =
		    rhs[m][i][k1][j] - lhs[n + 1][i][k1][j] * rhs[m][i][k][j];
		fac2 = 1. / lhs[n + 2][i][k1][j];
		rhs[m][i][k1][j] = fac2 * rhs[m][i][k1][j];
	}
	//-------------------------------------------
			...
			int n, int k1, int __ocl_j_bound, int __ocl_i_bound)
{
	...
	{
		rhs[m][i][k][j] =
		    rhs[m][i][k][j] - lhs[n + 3][i][k][j] * rhs[m][i][k1][j];
	}
	//-------------------------------------------
	//OpenCL kernel (END)
	//-------------------------------------------
