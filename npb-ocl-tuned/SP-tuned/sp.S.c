//-------------------------------------------------------------------------------
//Host code 
//Generated at : Mon Aug  6 14:34:32 2012
//Compiler options: 
//      Software Cache  true
//      Local Memory    true
...
static void compute_rhs()
{
	...
	//--------------------------------------------------------------
	{
		...
		size_t _ocl_gws[3];
		_ocl_gws[0] = (grid_points[1] - 2) - (1) + 1;
		_ocl_gws[1] = (grid_points[2] - 3 * 1 - 1) - (3 * 1) + 1;
		_ocl_gws[2] = (grid_points[0] - 2) - (1) + 1;
...
static void z_solve()
{
	...
	lhsz();
	n = 0;
	oclSwapDimensions (__ocl_buffer_lhs, 15 * (12 / 2 * 2 + 1), 12 / 2 * 2 + 1, 1,
			  12 / 2 * 2 + 1, sizeof(double));
	oclSwapDimensions (__ocl_buffer_rhs, 5 * (12 / 2 * 2 + 1), 12 / 2 * 2 + 1, 1,
			  12 / 2 * 2 + 1, sizeof(double));
	//--------------------------------------------------------------
...
static void init_ocl_runtime()
{
	...
	__ocl_program = oclBuildProgram("sp.S.cl");
...
static void create_ocl_buffers()
{
	...
	__ocl_buffer_cv = oclCreateBuffer(cv, (12 * 12 * 12) * sizeof(double));
	...
	__ocl_buffer_rhon =
	    oclCreateBuffer(rhon, (12 * 12 * 12) * sizeof(double));
	...
	__ocl_buffer_rhoq =
	    oclCreateBuffer(rhoq, (12 * 12 * 12) * sizeof(double));
	...
	__ocl_buffer_rhos =
	    oclCreateBuffer(rhos, (12 * 12 * 12) * sizeof(double));
