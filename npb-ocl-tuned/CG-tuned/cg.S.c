//-------------------------------------------------------------------------------
//Host code 
//Generated at : Thu Aug  9 15:29:43 2012
//Compiler options: 
//      Software Cache  true
//      Local Memory    true
...
//Do the final reduction part on the CPU
	...
	for (cgit = 1; cgit <= cgitmax; cgit++) {
		...
		//--------------------------------------------------------------
		{
			...
			size_t _ocl_gws[2];
            _ocl_gws[0] = 32;
			_ocl_gws[1] = (lastrow - firstrow + 1) - (1) + 1;
            size_t _ocl_lws[2];
            _ocl_lws[0] = 32;
            _ocl_lws[1] = 4;
			...
			oclRunKernelL(__ocl_conj_grad_2, 2, _ocl_gws, _ocl_lws);
		}
...
//Do the final reduction part on the CPU
	...
	//--------------------------------------------------------------
	{
		...
		oclRunKernelL(__ocl_conj_grad_9, 2, _ocl_gws, _ocl_lws);
	}
...
{
	{
		...
		while (nzv < nz) {
			vecelt = myrandlc(&tran, NULL, amult);
			vecloc = myrandlc(&tran, NULL, amult);
...
static void init_ocl_runtime()
{
	...
	__ocl_program = oclBuildProgram("cg.S.cl");
