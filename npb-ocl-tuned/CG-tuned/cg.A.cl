			  ...
			  __global double *w, int __ocl_j_bound)
{
	...
	//-------------------------------------------
	{
		...
		sum = 0.0;
		for (k = rowstr_0.x /*rowstr[j] */ + n ;
		     k < rowstr_0.y /*rowstr[j + 1] */ ; k+=32) {
			sum = sum + a[k] * p[colidx[k]];
		}
        ...
        temp[l_j][n] = sum;
        barrier (CLK_LOCAL_MEM_FENCE);
        if (n<16) temp[l_j][n] += temp[l_j][n+16];
        barrier (CLK_LOCAL_MEM_FENCE);
        if (n<8) temp[l_j][n] += temp[l_j][n+8];
        barrier (CLK_LOCAL_MEM_FENCE);
        if (n<4) temp[l_j][n] += temp[l_j][n+4];
        barrier (CLK_LOCAL_MEM_FENCE);
        if (n<2) temp[l_j][n] += temp[l_j][n+2];
        barrier (CLK_LOCAL_MEM_FENCE);
        if (n<1) temp[l_j][n] += temp[l_j][n+1];

		if (n==0) w[j] = temp[l_j][0];
	}
	//-------------------------------------------
	//OpenCL kernel (END)
	//-------------------------------------------
			  ...
			  __global double *w, int __ocl_j_bound)
{
	...
	//-------------------------------------------
	{
		...
		d = 0.0;
		for (k = rowstr_1.x /*rowstr[j] */ + n ;
		     k <= rowstr_1.y /*rowstr[j + 1] */  - 1; k+=32) {
			d = d + a[k] * z[colidx[k]];
		}
