//-------------------------------------------------------------------------------
//Host code 
//Generated at : Wed Aug  8 16:02:17 2012
//Compiler options: 
//      Software Cache  true
//      Local Memory    true
...
static void evolve(double u0_real[256][256][512],
		   ocl_buffer * __ocl_buffer_u0_real,
		   double u0_imag[256][256][512],
		   ocl_buffer * __ocl_buffer_u0_imag,
		   double u1_real[256][256][512],
		   ocl_buffer * __ocl_buffer_u1_real,
		   double u1_imag[256][256][512],
		   ocl_buffer * __ocl_buffer_u1_imag, int t,
		   int indexmap[256][256][512],
		   ocl_buffer * __ocl_buffer_indexmap, int d[3],
		   ocl_buffer * __ocl_buffer_d);
static void compute_initial_conditions(double u0_real[256][256][512],
				       ocl_buffer * __ocl_buffer_u0_real,
				       double u0_imag[256][256][512],
				       ocl_buffer * __ocl_buffer_u0_imag,
...
static void setup();
static void compute_indexmap(int indexmap[256][256][512],
			     ocl_buffer * __ocl_buffer_indexmap, int d[3],
			     ocl_buffer * __ocl_buffer_d);
static void print_timers();
static void fft(int dir, double x1_real[256][256][512],
		ocl_buffer * __ocl_buffer_x1_real,
		double x1_imag[256][256][512],
		ocl_buffer * __ocl_buffer_x1_imag,
		double x2_real[256][256][512],
		ocl_buffer * __ocl_buffer_x2_real,
		double x2_imag[256][256][512],
		ocl_buffer * __ocl_buffer_x2_imag);
static void cffts1(int is, int d[3], ocl_buffer * __ocl_buffer_d,
		   double x_real[256][256][512],
		   ocl_buffer * __ocl_buffer_x_real,
		   double x_imag[256][256][512],
		   ocl_buffer * __ocl_buffer_x_imag,
		   double xout_real[256][256][512],
		   ocl_buffer * __ocl_buffer_xout_real,
		   double xout_imag[256][256][512],
...
static void checksum(int i, double u1_real[256][256][512],
		     ocl_buffer * __ocl_buffer_u1_real,
		     double u1_imag[256][256][512],
...
{
	{
		int i, ierr;
		init_ocl_runtime();
		static double u0_real[256][256][512], u0_imag[256][256][512];
		DECLARE_LOCALVAR_OCL_BUFFER(u0_real, double, (256 * 256 * 512));
		DECLARE_LOCALVAR_OCL_BUFFER(u0_imag, double, (256 * 256 * 512));
		...
		static double u1_real[256][256][512], u1_imag[256][256][512];
		DECLARE_LOCALVAR_OCL_BUFFER(u1_real, double, (256 * 256 * 512));
		DECLARE_LOCALVAR_OCL_BUFFER(u1_imag, double, (256 * 256 * 512));
		...
		static double u2_real[256][256][512], u2_imag[256][256][512];
		DECLARE_LOCALVAR_OCL_BUFFER(u2_real, double, (256 * 256 * 512));
		DECLARE_LOCALVAR_OCL_BUFFER(u2_imag, double, (256 * 256 * 512));
		...
		DECLARE_LOCALVAR_OCL_BUFFER(indexmap, int, (256 * 256 * 512));
		int iter;
		int nthreads = 1;
			...
			timer_start(1);
		{
			...
			verify(512, 256, 256, niter, &verified, NULL, &class,
			       NULL);
		}
		sync_ocl_buffers();
		timer_stop(0);
		...
		if (total_time != 0.0) {
			mflops =
			    1.0e-6 * (double)(33554432) * (14.8157 +
							   7.19641 *
							   log((double)
							       (33554432)) +
							   (5.23518 +
...
{
	{
		...
		start = 314159265.0;
		ipow46(1220703125.0,
		       (zstart[0] - 1) * 2 * 512 * 256 + (ystart[0] -
							  1) * 2 * 512, &an,
		       NULL);
		dummy = myrandlc(&start, NULL, an);
		ipow46(1220703125.0, 2 * 512 * 256, &an, NULL);
...
static void init_ocl_runtime()
{
	...
	__ocl_program = oclBuildProgram("ft.B.cl");
...
{
	__ocl_buffer_ex = oclCreateBuffer(ex, (1966081) * sizeof(double));
