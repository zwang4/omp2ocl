...
void fftz2_g6_g7_g8_g9_g10_g11_gtp(int is, int l, int m, int n, int ny, int ny1,
				   ...
				   unsigned int __ocl_mult_factor,
				   unsigned int __ocl_add_offset);
void fftz2(int is, int l, int m, int n, int ny, int ny1, double u_real[512],
	   double u_imag[512], double x_real[512][18], double x_imag[512][18],
	   double y_real[512][18], double y_imag[512][18]);
	...
	if (m % 2 == 1) {
		for (j = 0; j < n; j++) {
			for (i = 0; i < fftblock; i++) {
				x_real[CALC_2D_IDX(512, 18, (j), (i)) *
				       __ocl_mult_factor + __ocl_add_offset] =
				    y_real[CALC_2D_IDX(512, 18, (j), (i)) *
					   __ocl_mult_factor +
					   __ocl_add_offset];
				x_imag[CALC_2D_IDX(512, 18, (j), (i)) *
				       __ocl_mult_factor + __ocl_add_offset] =
				    y_imag[CALC_2D_IDX(512, 18, (j), (i)) *
					   __ocl_mult_factor +
					   __ocl_add_offset];
			}
		}
	...
	for (i = 0; i < li; i++) {
		...
		for (k = 0; k < lk; k++) {
			for (j = 0; j < ny; j++) {
				...
				double x21real, x21imag;
				x11real =
				    x_real[CALC_2D_IDX(512, 18, (i11 + k), (j))
					   * __ocl_mult_factor +
					   __ocl_add_offset];
				x11imag =
				    x_imag[CALC_2D_IDX(512, 18, (i11 + k), (j))
					   * __ocl_mult_factor +
					   __ocl_add_offset];
				x21real =
				    x_real[CALC_2D_IDX(512, 18, (i12 + k), (j))
					   * __ocl_mult_factor +
					   __ocl_add_offset];
				x21imag =
				    x_imag[CALC_2D_IDX(512, 18, (i12 + k), (j))
					   * __ocl_mult_factor +
					   __ocl_add_offset];
				y_real[CALC_2D_IDX(512, 18, (i21 + k), (j)) *
				       __ocl_mult_factor + __ocl_add_offset] =
				    x11real + x21real;
				y_imag[CALC_2D_IDX(512, 18, (i21 + k), (j)) *
				       __ocl_mult_factor + __ocl_add_offset] =
				    x11imag + x21imag;
				y_real[CALC_2D_IDX(512, 18, (i22 + k), (j)) *
				       __ocl_mult_factor + __ocl_add_offset] =
				    u1_real * (x11real - x21real) -
				    u1_imag * (x11imag - x21imag);
				y_imag[CALC_2D_IDX(512, 18, (i22 + k), (j)) *
				       __ocl_mult_factor + __ocl_add_offset] =
				    u1_real * (x11imag - x21imag) +
				    u1_imag * (x11real - x21real);
			}
		       ...
		       int __ocl_i_bound, int __ocl_j_bound, int __ocl_k_bound)
{
	...
	//-------------------------------------------
	{
		u1_real[CALC_3D_IDX(512, 512, 512, (k), (j), (i))] =
		    u0_real[CALC_3D_IDX(512, 512, 512, (k), (j), (i))] *
		    ex[(t *
			indexmap[CALC_3D_IDX(512, 512, 512, (k), (j), (i))])];
		u1_imag[CALC_3D_IDX(512, 512, 512, (k), (j), (i))] =
		    u0_imag[CALC_3D_IDX(512, 512, 512, (k), (j), (i))] *
		    ex[(t *
			indexmap[CALC_3D_IDX(512, 512, 512, (k), (j), (i))])];
	}
	//-------------------------------------------
				 ...
				 int __ocl_j_bound, int __ocl_k_bound)
{
	...
	//-------------------------------------------
	{
		...
		ii2 = ii * ii;
		jj = (j + 1 + ystart_i - 2 + 512 / 2) % 512 - 512 / 2;
		ij2 = jj * jj + ii2;
		kk = (k + 1 + zstart_i - 2 + 512 / 2) % 512 - 512 / 2;
		indexmap[CALC_3D_IDX(512, 512, 512, (k), (j), (i))] =
		    kk * kk + ij2;
	}
	//-------------------------------------------
		       ...
		       int __ocl_jj_bound, int __ocl_k_bound)
{
	...
	//-------------------------------------------
	{
		for (j = 0; j < fftblock; j++) {
			for (i = 0; i < d[(0)]; i++) {
				yy0_real[CALC_2D_IDX(512, 18, (i), (j)) *
					 __ocl_mult_factor + __ocl_add_offset] =
				    x_real[CALC_3D_IDX
					   (512, 512, 512, (k), (j + jj), (i))];
				yy0_imag[CALC_2D_IDX(512, 18, (i), (j)) *
					 __ocl_mult_factor + __ocl_add_offset] =
				    x_imag[CALC_3D_IDX
					   (512, 512, 512, (k), (j + jj), (i))];
			}
		       ...
		       int __ocl_ii_bound, int __ocl_k_bound)
{
	...
	//-------------------------------------------
	{
		for (j = 0; j < d[(1)]; j++) {
			for (i = 0; i < fftblock; i++) {
				yy0_real[CALC_2D_IDX(512, 18, (j), (i)) *
					 __ocl_mult_factor + __ocl_add_offset] =
				    x_real[CALC_3D_IDX
					   (512, 512, 512, (k), (j), (i + ii))];
				yy0_imag[CALC_2D_IDX(512, 18, (j), (i)) *
					 __ocl_mult_factor + __ocl_add_offset] =
				    x_imag[CALC_3D_IDX
					   (512, 512, 512, (k), (j), (i + ii))];
			}
		       ...
		       int __ocl_ii_bound, int __ocl_j_bound)
{
	...
	//-------------------------------------------
	{
		for (k = 0; k < d[(2)]; k++) {
			for (i = 0; i < fftblock; i++) {
				yy0_real[CALC_2D_IDX(512, 18, (k), (i)) *
					 __ocl_mult_factor + __ocl_add_offset] =
				    x_real[CALC_3D_IDX
					   (512, 512, 512, (k), (j), (i + ii))];
				yy0_imag[CALC_2D_IDX(512, 18, (k), (i)) *
					 __ocl_mult_factor + __ocl_add_offset] =
				    x_imag[CALC_3D_IDX
					   (512, 512, 512, (k), (j), (i + ii))];
			}
					 ...
					 __global double *__ocl_part_chk_imag)
{
	...
	//-------------------------------------------
	//__global double (*u1_real)[512][512] =
	//    (__global double (*)[512][512])g_u1_real;
	//__global double (*u1_imag)[512][512] =
	//    (__global double (*)[512][512])g_u1_imag;
	//-------------------------------------------
