//-------------------------------------------------------------------------------
//Host code 
//Generated at : Wed Aug  8 16:02:19 2012
//Compiler options: 
//      Software Cache  true
//      Local Memory    true
...
static void evolve(double u0_real[512][512][512],
		   ocl_buffer * __ocl_buffer_u0_real,
		   double u0_imag[512][512][512],
		   ocl_buffer * __ocl_buffer_u0_imag,
		   double u1_real[512][512][512],
		   ocl_buffer * __ocl_buffer_u1_real,
		   double u1_imag[512][512][512],
		   ocl_buffer * __ocl_buffer_u1_imag, int t,
		   int indexmap[512][512][512],
		   ocl_buffer * __ocl_buffer_indexmap, int d[3],
		   ocl_buffer * __ocl_buffer_d);
static void compute_initial_conditions(double u0_real[512][512][512],
				       ocl_buffer * __ocl_buffer_u0_real,
				       double u0_imag[512][512][512],
				       ocl_buffer * __ocl_buffer_u0_imag,
...
static void setup();
static void compute_indexmap(int indexmap[512][512][512],
			     ocl_buffer * __ocl_buffer_indexmap, int d[3],
			     ocl_buffer * __ocl_buffer_d);
static void print_timers();
static void fft(int dir, double x1_real[512][512][512],
		ocl_buffer * __ocl_buffer_x1_real,
		double x1_imag[512][512][512],
		ocl_buffer * __ocl_buffer_x1_imag,
		double x2_real[512][512][512],
		ocl_buffer * __ocl_buffer_x2_real,
		double x2_imag[512][512][512],
		ocl_buffer * __ocl_buffer_x2_imag);
static void cffts1(int is, int d[3], ocl_buffer * __ocl_buffer_d,
		   double x_real[512][512][512],
		   ocl_buffer * __ocl_buffer_x_real,
		   double x_imag[512][512][512],
		   ocl_buffer * __ocl_buffer_x_imag,
		   double xout_real[512][512][512],
		   ocl_buffer * __ocl_buffer_xout_real,
		   double xout_imag[512][512][512],
...
static void cfftz(int is, int m, int n, double x_real[512][18],
		  ocl_buffer * __ocl_buffer_x_real, double x_imag[512][18],
		  ocl_buffer * __ocl_buffer_x_imag, double y_real[512][18],
		  ocl_buffer * __ocl_buffer_y_real, double y_imag[512][18],
		  ocl_buffer * __ocl_buffer_y_imag);
static void fftz2(int is, int l, int m, int n, int ny, int ny1,
		  double u_real[512], ocl_buffer * __ocl_buffer_u_real,
		  double u_imag[512], ocl_buffer * __ocl_buffer_u_imag,
		  double x_real[512][18], ocl_buffer * __ocl_buffer_x_real,
		  double x_imag[512][18], ocl_buffer * __ocl_buffer_x_imag,
		  double y_real[512][18], ocl_buffer * __ocl_buffer_y_real,
		  double y_imag[512][18], ocl_buffer * __ocl_buffer_y_imag);
static int ilog2(int n);
static void checksum(int i, double u1_real[512][512][512],
		     ocl_buffer * __ocl_buffer_u1_real,
		     double u1_imag[512][512][512],
...
{
	{
		int i, ierr;
		init_ocl_runtime();
		static double u0_real[512][512][512], u0_imag[512][512][512];
		DECLARE_LOCALVAR_OCL_BUFFER(u0_real, double, (512 * 512 * 512));
		DECLARE_LOCALVAR_OCL_BUFFER(u0_imag, double, (512 * 512 * 512));
		...
		static double u1_real[512][512][512], u1_imag[512][512][512];
		DECLARE_LOCALVAR_OCL_BUFFER(u1_real, double, (512 * 512 * 512));
		DECLARE_LOCALVAR_OCL_BUFFER(u1_imag, double, (512 * 512 * 512));
		...
		static double u2_real[512][512][512], u2_imag[512][512][512];
		DECLARE_LOCALVAR_OCL_BUFFER(u2_real, double, (512 * 512 * 512));
		DECLARE_LOCALVAR_OCL_BUFFER(u2_imag, double, (512 * 512 * 512));
		...
		static int indexmap[512][512][512];
		DECLARE_LOCALVAR_OCL_BUFFER(indexmap, int, (512 * 512 * 512));
		int iter;
		int nthreads = 1;
			...
			timer_start(1);
		{
			...
			verify(512, 512, 512, niter, &verified, NULL, &class,
			       NULL);
		}
		sync_ocl_buffers();
		timer_stop(0);
		...
		if (total_time != 0.0) {
			mflops =
			    1.0e-6 * (double)(134217728) * (14.8157 +
							    7.19641 *
							    log((double)
								(134217728)) +
							    (5.23518 +
		...
		c_print_results("FT", class, 512, 512, 512, niter, nthreads,
...
{
	{
		...
		static double tmp[524289];
		DECLARE_LOCALVAR_OCL_BUFFER(tmp, double, (524289));
		int i, j, t;
		start = 314159265.0;
		ipow46(1220703125.0,
		       (zstart[0] - 1) * 2 * 512 * 512 + (ystart[0] -
							  1) * 2 * 512, &an,
		       NULL);
		dummy = myrandlc(&start, NULL, an);
		ipow46(1220703125.0, 2 * 512 * 512, &an, NULL);
		for (k = 0; k < dims[0][2]; k++) {
			x0 = start;

			myvranlc(2 * 512 * dims[0][1], &x0, NULL, 1220703125.0,
...
//Do the final reduction part on the CPU
		...
		{
			sums_real[i] = sums_real[i] / (double)(134217728);
			sums_imag[i] = sums_imag[i] / (double)(134217728);
...
static void init_ocl_runtime()
{
	...
	__ocl_program = oclBuildProgram("ft.C.cl");
...
{
	__ocl_buffer_ex = oclCreateBuffer(ex, (3932161) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_ex, -1);
	__ocl_buffer_yy0_real =
	    oclCreateBuffer(yy0_real, (512 * 18) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_yy0_real, -1);
	__ocl_buffer_yy0_imag =
	    oclCreateBuffer(yy0_imag, (512 * 18) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_yy0_imag, -1);
	__ocl_buffer_yy1_real =
	    oclCreateBuffer(yy1_real, (512 * 18) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_yy1_real, -1);
	__ocl_buffer_yy1_imag =
	    oclCreateBuffer(yy1_imag, (512 * 18) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_yy1_imag, -1);
	__ocl_buffer_u_real = oclCreateBuffer(u_real, (512) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_u_real, -1);
	__ocl_buffer_u_imag = oclCreateBuffer(u_imag, (512) * sizeof(double));
