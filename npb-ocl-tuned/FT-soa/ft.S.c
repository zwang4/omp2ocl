...
static double yy0_real[64][18];
static double yy0_imag[64][18];
static double yy1_real[64][18];
static double yy1_imag[64][18];
static void evolve(double u0_real[64][64][64],
		   ocl_buffer * __ocl_buffer_u0_real,
		   double u0_imag[64][64][64],
		   ocl_buffer * __ocl_buffer_u0_imag,
		   double u1_real[64][64][64],
		   ocl_buffer * __ocl_buffer_u1_real,
		   double u1_imag[64][64][64],
		   ocl_buffer * __ocl_buffer_u1_imag, int t,
		   int indexmap[64][64][64], ocl_buffer * __ocl_buffer_indexmap,
		   int d[3], ocl_buffer * __ocl_buffer_d);
static void compute_initial_conditions(double u0_real[64][64][64],
				       ocl_buffer * __ocl_buffer_u0_real,
				       double u0_imag[64][64][64],
				       ocl_buffer * __ocl_buffer_u0_imag,
				       int d[3], ocl_buffer * __ocl_buffer_d);
...
static void setup();
static void compute_indexmap(int indexmap[64][64][64],
			     ocl_buffer * __ocl_buffer_indexmap, int d[3],
			     ocl_buffer * __ocl_buffer_d);
static void print_timers();
static void fft(int dir, double x1_real[64][64][64],
		ocl_buffer * __ocl_buffer_x1_real, double x1_imag[64][64][64],
		ocl_buffer * __ocl_buffer_x1_imag, double x2_real[64][64][64],
		ocl_buffer * __ocl_buffer_x2_real, double x2_imag[64][64][64],
		ocl_buffer * __ocl_buffer_x2_imag);
static void cffts1(int is, int d[3], ocl_buffer * __ocl_buffer_d,
		   double x_real[64][64][64], ocl_buffer * __ocl_buffer_x_real,
		   double x_imag[64][64][64], ocl_buffer * __ocl_buffer_x_imag,
		   double xout_real[64][64][64],
		   ocl_buffer * __ocl_buffer_xout_real,
		   double xout_imag[64][64][64],
...
static void cfftz(int is, int m, int n, double x_real[64][18],
		  ocl_buffer * __ocl_buffer_x_real, double x_imag[64][18],
		  ocl_buffer * __ocl_buffer_x_imag, double y_real[64][18],
		  ocl_buffer * __ocl_buffer_y_real, double y_imag[64][18],
		  ocl_buffer * __ocl_buffer_y_imag);
static void fftz2(int is, int l, int m, int n, int ny, int ny1,
		  double u_real[64], ocl_buffer * __ocl_buffer_u_real,
		  double u_imag[64], ocl_buffer * __ocl_buffer_u_imag,
		  double x_real[64][18], ocl_buffer * __ocl_buffer_x_real,
		  double x_imag[64][18], ocl_buffer * __ocl_buffer_x_imag,
		  double y_real[64][18], ocl_buffer * __ocl_buffer_y_real,
		  double y_imag[64][18], ocl_buffer * __ocl_buffer_y_imag);
static int ilog2(int n);
static void checksum(int i, double u1_real[64][64][64],
		     ocl_buffer * __ocl_buffer_u1_real,
		     double u1_imag[64][64][64],
...
{
	{
		int i, ierr;
		init_ocl_runtime();
		static double u0_real[64][64][64], u0_imag[64][64][64];
		DECLARE_LOCALVAR_OCL_BUFFER(u0_real, double, (64 * 64 * 64));
		DECLARE_LOCALVAR_OCL_BUFFER(u0_imag, double, (64 * 64 * 64));
		...
		static double u1_real[64][64][64], u1_imag[64][64][64];
		DECLARE_LOCALVAR_OCL_BUFFER(u1_real, double, (64 * 64 * 64));
		DECLARE_LOCALVAR_OCL_BUFFER(u1_imag, double, (64 * 64 * 64));
		...
		static double u2_real[64][64][64], u2_imag[64][64][64];
		DECLARE_LOCALVAR_OCL_BUFFER(u2_real, double, (64 * 64 * 64));
		DECLARE_LOCALVAR_OCL_BUFFER(u2_imag, double, (64 * 64 * 64));
		...
		DECLARE_LOCALVAR_OCL_BUFFER(indexmap, int, (64 * 64 * 64));
		int iter;
		int nthreads = 1;
		...
		setup();
		{
			...
			fft(1, u1_real, __ocl_buffer_u1_real, u1_imag,
			    __ocl_buffer_u1_imag, u0_real, __ocl_buffer_u0_real,
			...
			timer_start(1);
		{
			...
			for (iter = 1; iter <= niter; iter++) {
				...
				fft(-1, u1_real, __ocl_buffer_u1_real, u1_imag,
				    __ocl_buffer_u1_imag, u2_real,
				    __ocl_buffer_u2_real, u2_imag,
			...
			verify(64, 64, 64, niter, &verified, NULL, &class,
			       NULL);
		}
		sync_ocl_buffers();
		timer_stop(0);
...
{
	{
		...
		static double tmp[8193];
		DECLARE_LOCALVAR_OCL_BUFFER(tmp, double, (8193));
		int i, j, t;
		start = 314159265.0;
		ipow46(1220703125.0,
		       (zstart[0] - 1) * 2 * 64 * 64 + (ystart[0] - 1) * 2 * 64,
		       &an, NULL);
		dummy = myrandlc(&start, NULL, an);
		ipow46(1220703125.0, 2 * 64 * 64, &an, NULL);
		for (k = 0; k < dims[0][2]; k++) {
			x0 = start;

			myvranlc(2 * 64 * dims[0][1], &x0, NULL, 1220703125.0,
		...
		ocl_buffer * __ocl_buffer_x2_imag)
{
	{
		if (dir == 1) {
			cffts1(1, dims[0], NULL, x1_real, __ocl_buffer_x1_real,
		   ...
		   ocl_buffer * __ocl_buffer_xout_imag)
{
	...
	CREATE_FUNC_LEVEL_BUFFER(__ocl_buffer_x_real_cffts1,
				 __ocl_p_x_real_cffts1, x_real, (64 * 64 * 64),
				 double);
	CREATE_FUNC_LEVEL_BUFFER(__ocl_buffer_x_imag_cffts1,
				 __ocl_p_x_imag_cffts1, x_imag, (64 * 64 * 64),
				 double);
				 ...
				 (64 * 64 * 64), double);
	{
		...
		//--------------------------------------------------------------
		{
			...
			CREATE_THREAD_PRIVATE_BUF(__ocl_th_yy0_real_cffts1_0,
						  ...
						  double,
						  (1152 * _ocl_thread_num),
						  DEFAULT_ALIGN_SIZE);
			oclSetKernelArgBuffer(__ocl_cffts1_0, 2,
					      __ocl_buffer_yy0_real_cffts1_0);
			oclSetKernelArgBuffer(__ocl_cffts1_0, 3,
					      __ocl_buffer_x_real);
			CREATE_THREAD_PRIVATE_BUF(__ocl_th_yy0_imag_cffts1_0,
						  ...
						  (1152 * _ocl_thread_num),
						  DEFAULT_ALIGN_SIZE);
			oclSetKernelArgBuffer(__ocl_cffts1_0, 4,
					      __ocl_buffer_yy0_imag_cffts1_0);
			oclSetKernelArgBuffer(__ocl_cffts1_0, 5,
					      __ocl_buffer_x_imag);
			oclSetKernelArg(__ocl_cffts1_0, 6, sizeof(int), &is);
			oclSetKernelArg(__ocl_cffts1_0, 7, sizeof(int),
					&logd_0);
			CREATE_THREAD_PRIVATE_BUF(__ocl_th_yy1_real_cffts1_0,
						  ...
						  (1152 * _ocl_thread_num),
						  DEFAULT_ALIGN_SIZE);
			oclSetKernelArgBuffer(__ocl_cffts1_0, 8,
					      __ocl_buffer_yy1_real_cffts1_0);
			CREATE_THREAD_PRIVATE_BUF(__ocl_th_yy1_imag_cffts1_0,
						  ...
						  (1152 * _ocl_thread_num),
						  DEFAULT_ALIGN_SIZE);
			oclSetKernelArgBuffer(__ocl_cffts1_0, 9,
					      __ocl_buffer_yy1_imag_cffts1_0);
			oclSetKernelArgBuffer(__ocl_cffts1_0, 10,
					      __ocl_buffer_xout_real);
			oclSetKernelArgBuffer(__ocl_cffts1_0, 11,
					      __ocl_buffer_xout_imag);
			oclSetKernelArgBuffer(__ocl_cffts1_0, 12,
					      __ocl_buffer_u_real);
			oclSetKernelArgBuffer(__ocl_cffts1_0, 13,
					      __ocl_buffer_u_imag);
			oclSetKernelArg(__ocl_cffts1_0, 14, sizeof(int),
					&fftblockpad);
			int __ocl_jj_inc = fftblock;
			oclSetKernelArg(__ocl_cffts1_0, 15, sizeof(int),
					&__ocl_jj_inc);
			int __ocl_jj_bound = d[1] - fftblock;
			oclSetKernelArg(__ocl_cffts1_0, 16, sizeof(int),
					&__ocl_jj_bound);
			int __ocl_k_bound = d[2];
			oclSetKernelArg(__ocl_cffts1_0, 17, sizeof(int),
					&__ocl_k_bound);
			//------------------------------------------
			//OpenCL kernel arguments (END) 
			//------------------------------------------
		   ...
		   ocl_buffer * __ocl_buffer_xout_imag)
{
	...
	CREATE_FUNC_LEVEL_BUFFER(__ocl_buffer_x_real_cffts2,
				 __ocl_p_x_real_cffts2, x_real, (64 * 64 * 64),
				 double);
	CREATE_FUNC_LEVEL_BUFFER(__ocl_buffer_x_imag_cffts2,
				 __ocl_p_x_imag_cffts2, x_imag, (64 * 64 * 64),
				 double);
		   ...
		   ocl_buffer * __ocl_buffer_xout_imag)
{
	...
	CREATE_FUNC_LEVEL_BUFFER(__ocl_buffer_x_real_cffts3,
				 __ocl_p_x_real_cffts3, x_real, (64 * 64 * 64),
				 double);
	CREATE_FUNC_LEVEL_BUFFER(__ocl_buffer_x_imag_cffts3,
				 __ocl_p_x_imag_cffts3, x_imag, (64 * 64 * 64),
				 double);
...
//Do the final reduction part on the CPU
		...
		{
			sums_real[i] = sums_real[i] / (double)(262144);
			sums_imag[i] = sums_imag[i] / (double)(262144);
...
static void init_ocl_runtime()
{
	...
	__ocl_program = oclBuildProgram("ft.S.cl");
...
{
	__ocl_buffer_ex = oclCreateBuffer(ex, (18433) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_ex, -1);
	__ocl_buffer_yy0_real =
	    oclCreateBuffer(yy0_real, (64 * 18) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_yy0_real, -1);
	__ocl_buffer_yy0_imag =
	    oclCreateBuffer(yy0_imag, (64 * 18) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_yy0_imag, -1);
	__ocl_buffer_yy1_real =
	    oclCreateBuffer(yy1_real, (64 * 18) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_yy1_real, -1);
	__ocl_buffer_yy1_imag =
	    oclCreateBuffer(yy1_imag, (64 * 18) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_yy1_imag, -1);
	__ocl_buffer_u_real = oclCreateBuffer(u_real, (64) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_u_real, -1);
	__ocl_buffer_u_imag = oclCreateBuffer(u_imag, (64) * sizeof(double));
