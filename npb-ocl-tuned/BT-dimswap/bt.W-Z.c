...
{
	{
		...
		if (grid_points[0] > 24 || grid_points[1] > 24
		    || grid_points[2] > 24) {
			printf(" %dx%dx%d\n", grid_points[0], grid_points[1],
			       grid_points[2]);
			printf
...
static void z_solve()
{
    ...
    startTimer (&ti);
#endif
	oclSwapDimensions (__ocl_buffer_lhs, 3 * 5 * 5 * (24 / 2 * 2 + 1), 24 / 2 * 2 + 1, 1,
			  24 / 2 * 2 + 1, sizeof(double));
	oclSwapDimensions (__ocl_buffer_rhs, 5 * (24 / 2 * 2 + 1), 24 / 2 * 2 + 1, 1,
			  24 / 2 * 2 + 1, sizeof(double));
#ifdef PROFILE_SWAP
    oclSync();
    stopTimer (&ti);
...
{
	__ocl_buffer_u =
	    oclCreateBuffer(u, (5 * 25 * 25 * 25) * sizeof(double));
