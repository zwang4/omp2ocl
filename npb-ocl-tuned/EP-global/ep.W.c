//-------------------------------------------------------------------------------
//Host code 
//Generated at : Mon Aug 13 10:49:01 2012
//Compiler options: 
//      Software Cache  true
//      Local Memory    true
...
#ifndef main_0_reduction_step1_MULT_ITER_0
#define main_0_reduction_step1_MULT_ITER_0 1
#endif

				OCL_NEAREST_MULTD(_ocl_gws[0],
						  main_0_reduction_step1_MULT_ITER_0);
...
//init the round-up buffer spaces so that I can apply vectorisation on the second step
				...
				oclSetKernelArg(__ocl_main_0_reduction_step1, 1,
						sizeof(double), &an);
				CREATE_THREAD_PRIVATE_BUF(__ocl_th_x_main_0,
							  __ocl_buffer_x_main_0,
							  double,
							  (2048 *
							   _ocl_thread_num),
							  DEFAULT_ALIGN_SIZE);
				oclSetKernelArgBuffer
				    (__ocl_main_0_reduction_step1, 2,
				     __ocl_buffer_x_main_0);
				oclSetKernelArgBuffer
				    (__ocl_main_0_reduction_step1, 3,
				     __ocl_buffer_x);
				//------------------------------------------
				//OpenCL kernel arguments (BEGIN) 
				//------------------------------------------
				int __ocl_k_bound = np;
				oclSetKernelArg(__ocl_main_0_reduction_step1, 4,
						sizeof(int), &__ocl_k_bound);
				oclSetKernelArgBuffer
				    (__ocl_main_0_reduction_step1, 5,
				     __ocl_buffer_sx_main_0);
				oclSetKernelArgBuffer
				    (__ocl_main_0_reduction_step1, 6,
				     __ocl_buffer_sy_main_0);
				//------------------------------------------
				...
				oclSync();
				if (__ocl_th_x_main_0_length) {
					oclReleaseBuffer(__ocl_buffer_x_main_0);
					free(__ocl_th_x_main_0);
					__ocl_th_x_main_0 = NULL;
					__ocl_buffer_x_main_0 = NULL;
					__ocl_th_x_main_0_length = 0;
				}
#endif
...
//Do the final reduction part on the CPU
		...
		printf
		    ("EP Benchmark Results: \nCPU Time = %10.4f\nN = 2^%5d\nNo. Gaussian Pairs = %15.0f\nSums = %25.15e %25.15e\nCounts:\n",
		     tm, 25, gc, sx, sy);
		c_print_results("EP", 'W', 25 + 1, 0, 0, nit, nthreads, tm,
				Mops, "Random numbers generated", verified,
				"2.3", "13 Aug 2012", "gcc", "gcc", "(none)",
...
static void init_ocl_runtime()
{
	...
	__ocl_program = oclBuildProgram("ep.W.cl");
