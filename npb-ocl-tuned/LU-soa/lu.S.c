//-------------------------------------------------------------------------------
//Host code 
//Generated at : Mon Aug  6 14:04:36 2012
//Compiler options: 
//      Software Cache  true
//      Local Memory    true
...
static void blts(int nx, int ny, int nz, int k, double omega,
		 double v[5][12][13][13], ocl_buffer * __ocl_buffer_v,
		 double ldz[5][5][12][12], ocl_buffer * __ocl_buffer_ldz,
		 double ldy[5][5][12][12], ocl_buffer * __ocl_buffer_ldy,
		 double ldx[5][5][12][12], ocl_buffer * __ocl_buffer_ldx,
		 double d[5][5][12][12], ocl_buffer * __ocl_buffer_d, int ist,
		 int iend, int jst, int jend, int nx0, int ny0);
static void buts(int nx, int ny, int nz, int k, double omega,
		 double v[5][12][13][13], ocl_buffer * __ocl_buffer_v,
		 double tv[5][12][12], ocl_buffer * __ocl_buffer_tv,
		 double d[5][5][12][12], ocl_buffer * __ocl_buffer_d,
		 double udx[5][5][12][12], ocl_buffer * __ocl_buffer_udx,
		 double udy[5][5][12][12], ocl_buffer * __ocl_buffer_udy,
		 double udz[5][5][12][12], ocl_buffer * __ocl_buffer_udz,
...
static void pintgr()
{
	...
	double phi1[14][14];
	DECLARE_LOCALVAR_OCL_BUFFER(phi1, double, (14 * 14));
	double phi2[14][14];
	DECLARE_LOCALVAR_OCL_BUFFER(phi2, double, (14 * 14));
...
static void ssor()
{
	...
	int istep;
	double tmp;
	double delunm[5], tv[5][12][12];
	DECLARE_LOCALVAR_OCL_BUFFER(delunm, double, (5));
	DECLARE_LOCALVAR_OCL_BUFFER(tv, double, (5 * 12 * 12));
...
static void init_ocl_runtime()
{
	...
	__ocl_program = oclBuildProgram("lu.S.cl");
...
{
	__ocl_buffer_frct =
	    oclCreateBuffer(frct, (5 * 12 * 13 * 13) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_frct, -1);
	__ocl_buffer_rsd =
	    oclCreateBuffer(rsd, (5 * 12 * 13 * 13) * sizeof(double));
	...
	__ocl_buffer_flux =
	    oclCreateBuffer(flux, (5 * 12 * 13 * 13) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_flux, -1);
	__ocl_buffer_u =
	    oclCreateBuffer(u, (5 * 12 * 13 * 13) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_u, -1);
	__ocl_buffer_d = oclCreateBuffer(d, (5 * 5 * 12 * 12) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_d, -1);
	__ocl_buffer_a = oclCreateBuffer(a, (5 * 5 * 12 * 12) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_a, -1);
	__ocl_buffer_b = oclCreateBuffer(b, (5 * 5 * 12 * 12) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_b, -1);
	__ocl_buffer_c = oclCreateBuffer(c, (5 * 5 * 12 * 12) * sizeof(double));
