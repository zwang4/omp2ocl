//-------------------------------------------------------------------------------
//Host code 
//Generated at : Mon Aug  6 14:04:59 2012
//Compiler options: 
//      Software Cache  true
//      Local Memory    true
...
static void blts(int nx, int ny, int nz, int k, double omega,
		 double v[5][102][103][103], ocl_buffer * __ocl_buffer_v,
		 double ldz[5][5][102][102], ocl_buffer * __ocl_buffer_ldz,
		 double ldy[5][5][102][102], ocl_buffer * __ocl_buffer_ldy,
		 double ldx[5][5][102][102], ocl_buffer * __ocl_buffer_ldx,
		 double d[5][5][102][102], ocl_buffer * __ocl_buffer_d, int ist,
		 int iend, int jst, int jend, int nx0, int ny0);
static void buts(int nx, int ny, int nz, int k, double omega,
		 double v[5][102][103][103], ocl_buffer * __ocl_buffer_v,
		 double tv[5][102][102], ocl_buffer * __ocl_buffer_tv,
		 double d[5][5][102][102], ocl_buffer * __ocl_buffer_d,
		 double udx[5][5][102][102], ocl_buffer * __ocl_buffer_udx,
		 double udy[5][5][102][102], ocl_buffer * __ocl_buffer_udy,
		 double udz[5][5][102][102], ocl_buffer * __ocl_buffer_udz,
...
static void l2norm(int nx0, int ny0, int nz0, int ist, int iend, int jst,
		   int jend, double v[5][102][103][103],
		 ...
		 int iend, int jst, int jend, int nx0, int ny0)
{
				 ...
				 (5 * 5 * 102 * 102), double);
	{
		...
		oclHostWrites(__ocl_buffer_v);
		oclHostWrites(__ocl_buffer_tmat);
		oclHostReads(__ocl_buffer_ldx);
		oclHostReads(__ocl_buffer_ldy);
		oclHostReads(__ocl_buffer_d);
		oclSync();
		for (i = ist; i <= iend; i++) {
			for (j = jst; j <= jend; j++) {
				for (m = 0; m < 5; m++) {
				...
				v[3][i][j][k] =
				    v[3][i][j][k] - tmat[3][4] * v[4][i][j][k];
				v[3][i][j][k] = v[3][i][j][k] / tmat[3][3];
				v[2][i][j][k] =
				    v[2][i][j][k] - tmat[2][3] * v[3][i][j][k] -
				    tmat[2][4] * v[4][i][j][k];
				v[2][i][j][k] = v[2][i][j][k] / tmat[2][2];
				v[1][i][j][k] =
				    v[1][i][j][k] - tmat[1][2] * v[2][i][j][k] -
				    tmat[1][3] * v[3][i][j][k] -
				    tmat[1][4] * v[4][i][j][k];
				v[1][i][j][k] = v[1][i][j][k] / tmat[1][1];
				v[0][i][j][k] =
				    v[0][i][j][k] - tmat[0][1] * v[1][i][j][k] -
				    tmat[0][2] * v[2][i][j][k] -
				    tmat[0][3] * v[3][i][j][k] -
				    tmat[0][4] * v[4][i][j][k];
				v[0][i][j][k] = v[0][i][j][k] / tmat[0][0];
			}
		}
	}
}
...
//This swaps a decremental loop
		...
		oclHostWrites(__ocl_buffer_tv);
		oclHostWrites(__ocl_buffer_tmat);
		oclHostWrites(__ocl_buffer_v);
		oclHostReads(__ocl_buffer_udx);
		oclHostReads(__ocl_buffer_udy);
		oclHostReads(__ocl_buffer_d);
		oclSync();
		for (i = iend; i >= ist; i--) {
			for (j = jend; j >= jst; j--) {
				...
				tmat[1][3] = tmat[1][3] - tmp * tmat[0][3];
				tmat[1][4] = tmat[1][4] - tmp * tmat[0][4];
				tv[1][i][j] = tv[1][i][j] - tv[0][i][j] * tmp;
				tmp = tmp1 * tmat[2][0];
				tmat[2][1] = tmat[2][1] - tmp * tmat[0][1];
				tmat[2][2] = tmat[2][2] - tmp * tmat[0][2];
				tmat[2][3] = tmat[2][3] - tmp * tmat[0][3];
				tmat[2][4] = tmat[2][4] - tmp * tmat[0][4];
				tv[2][i][j] = tv[2][i][j] - tv[0][i][j] * tmp;
				tmp = tmp1 * tmat[3][0];
				tmat[3][1] = tmat[3][1] - tmp * tmat[0][1];
				tmat[3][2] = tmat[3][2] - tmp * tmat[0][2];
				tmat[3][3] = tmat[3][3] - tmp * tmat[0][3];
				tmat[3][4] = tmat[3][4] - tmp * tmat[0][4];
				tv[3][i][j] = tv[3][i][j] - tv[0][i][j] * tmp;
				tmp = tmp1 * tmat[4][0];
				tmat[4][1] = tmat[4][1] - tmp * tmat[0][1];
				tmat[4][2] = tmat[4][2] - tmp * tmat[0][2];
				tmat[4][3] = tmat[4][3] - tmp * tmat[0][3];
				tmat[4][4] = tmat[4][4] - tmp * tmat[0][4];
				tv[4][i][j] = tv[4][i][j] - tv[0][i][j] * tmp;
				tmp1 = 1.0 / tmat[1][1];
				tmp = tmp1 * tmat[2][1];
				tmat[2][2] = tmat[2][2] - tmp * tmat[1][2];
				tmat[2][3] = tmat[2][3] - tmp * tmat[1][3];
				...
				tv[3][i][j] =
				    tv[3][i][j] - tmat[3][4] * tv[4][i][j];
				tv[3][i][j] = tv[3][i][j] / tmat[3][3];
				tv[2][i][j] =
				    tv[2][i][j] - tmat[2][3] * tv[3][i][j] -
				    tmat[2][4] * tv[4][i][j];
				tv[2][i][j] = tv[2][i][j] / tmat[2][2];
				tv[1][i][j] =
				    tv[1][i][j] - tmat[1][2] * tv[2][i][j] -
				    tmat[1][3] * tv[3][i][j] -
				    tmat[1][4] * tv[4][i][j];
				tv[1][i][j] = tv[1][i][j] / tmat[1][1];
				tv[0][i][j] =
				    tv[0][i][j] - tmat[0][1] * tv[1][i][j] -
				    tmat[0][2] * tv[2][i][j] -
				    tmat[0][3] * tv[3][i][j] -
				    tmat[0][4] * tv[4][i][j];
				tv[0][i][j] = tv[0][i][j] / tmat[0][0];
...
static void erhs()
{
	...
	//--------------------------------------------------------------
	{
		...
		oclSetKernelArg(__ocl_erhs_0, 1, sizeof(int), &m);
		int __ocl_k_bound = nz;
		oclSetKernelArg(__ocl_erhs_0, 2, sizeof(int), &__ocl_k_bound);
		int __ocl_j_bound = ny;
		oclSetKernelArg(__ocl_erhs_0, 3, sizeof(int), &__ocl_j_bound);
		int __ocl_i_bound = nx;
		oclSetKernelArg(__ocl_erhs_0, 4, sizeof(int), &__ocl_i_bound);
		//------------------------------------------
		//OpenCL kernel arguments (END) 
		//------------------------------------------
	...
	//--------------------------------------------------------------
	{
		...
		oclSetKernelArg(__ocl_erhs_1, 1, sizeof(int), &j);
		oclSetKernelArg(__ocl_erhs_1, 2, sizeof(int), &ny);
		oclSetKernelArg(__ocl_erhs_1, 3, sizeof(int), &ny0);
		oclSetKernelArg(__ocl_erhs_1, 4, sizeof(int), &k);
		oclSetKernelArg(__ocl_erhs_1, 5, sizeof(int), &nz);
		oclSetKernelArg(__ocl_erhs_1, 6, sizeof(int), &m);
		oclSetKernelArgBuffer(__ocl_erhs_1, 7, __ocl_buffer_rsd);
		oclSetKernelArgBuffer(__ocl_erhs_1, 8, __ocl_buffer_ce);
		int __ocl_i_bound = nx;
		oclSetKernelArg(__ocl_erhs_1, 9, sizeof(int), &__ocl_i_bound);
		//------------------------------------------
		//OpenCL kernel arguments (END) 
		//------------------------------------------
	...
	//--------------------------------------------------------------
	{
		...
		oclSetKernelArg(__ocl_erhs_3, 0, sizeof(int), &i);
		oclSetKernelArg(__ocl_erhs_3, 1, sizeof(int), &ist);
		oclSetKernelArg(__ocl_erhs_3, 2, sizeof(int), &iend);
		oclSetKernelArg(__ocl_erhs_3, 3, sizeof(int), &m);
		oclSetKernelArgBuffer(__ocl_erhs_3, 4, __ocl_buffer_frct);
		oclSetKernelArg(__ocl_erhs_3, 5, sizeof(double), &tx2);
		oclSetKernelArgBuffer(__ocl_erhs_3, 6, __ocl_buffer_flux);
		oclSetKernelArg(__ocl_erhs_3, 7, sizeof(int), &L2);
		oclSetKernelArgBuffer(__ocl_erhs_3, 8, __ocl_buffer_rsd);
		oclSetKernelArg(__ocl_erhs_3, 9, sizeof(double), &tx3);
		oclSetKernelArg(__ocl_erhs_3, 10, sizeof(double), &dx1);
		oclSetKernelArg(__ocl_erhs_3, 11, sizeof(double), &tx1);
		oclSetKernelArg(__ocl_erhs_3, 12, sizeof(double), &dx2);
		oclSetKernelArg(__ocl_erhs_3, 13, sizeof(double), &dx3);
		oclSetKernelArg(__ocl_erhs_3, 14, sizeof(double), &dx4);
		oclSetKernelArg(__ocl_erhs_3, 15, sizeof(double), &dx5);
		oclSetKernelArg(__ocl_erhs_3, 16, sizeof(double), &dsspm);
		oclSetKernelArg(__ocl_erhs_3, 17, sizeof(int), &nx);
		oclSetKernelArg(__ocl_erhs_3, 18, sizeof(int), &jst);
		int __ocl_k_bound = nz - 2;
		oclSetKernelArg(__ocl_erhs_3, 19, sizeof(int), &__ocl_k_bound);
		int __ocl_j_bound = jend;
		oclSetKernelArg(__ocl_erhs_3, 20, sizeof(int), &__ocl_j_bound);
		//------------------------------------------
		//OpenCL kernel arguments (END) 
		//------------------------------------------
	...
	//--------------------------------------------------------------
	{
		...
		oclGetWorkSize(2, _ocl_gws, NULL);
		oclSetKernelArg(__ocl_erhs_5, 0, sizeof(int), &j);
		oclSetKernelArg(__ocl_erhs_5, 1, sizeof(int), &jst);
		oclSetKernelArg(__ocl_erhs_5, 2, sizeof(int), &jend);
		oclSetKernelArg(__ocl_erhs_5, 3, sizeof(int), &m);
		oclSetKernelArgBuffer(__ocl_erhs_5, 4, __ocl_buffer_frct);
		oclSetKernelArg(__ocl_erhs_5, 5, sizeof(double), &ty2);
		oclSetKernelArgBuffer(__ocl_erhs_5, 6, __ocl_buffer_flux);
		oclSetKernelArg(__ocl_erhs_5, 7, sizeof(int), &L2);
		oclSetKernelArgBuffer(__ocl_erhs_5, 8, __ocl_buffer_rsd);
		oclSetKernelArg(__ocl_erhs_5, 9, sizeof(double), &ty3);
		oclSetKernelArg(__ocl_erhs_5, 10, sizeof(double), &dy1);
		oclSetKernelArg(__ocl_erhs_5, 11, sizeof(double), &ty1);
		oclSetKernelArg(__ocl_erhs_5, 12, sizeof(double), &dy2);
		oclSetKernelArg(__ocl_erhs_5, 13, sizeof(double), &dy3);
		oclSetKernelArg(__ocl_erhs_5, 14, sizeof(double), &dy4);
		oclSetKernelArg(__ocl_erhs_5, 15, sizeof(double), &dy5);
		oclSetKernelArg(__ocl_erhs_5, 16, sizeof(double), &dsspm);
		oclSetKernelArg(__ocl_erhs_5, 17, sizeof(int), &ny);
		oclSetKernelArg(__ocl_erhs_5, 18, sizeof(int), &ist);
		int __ocl_k_bound = nz - 2;
		oclSetKernelArg(__ocl_erhs_5, 19, sizeof(int), &__ocl_k_bound);
		int __ocl_i_bound = iend;
		oclSetKernelArg(__ocl_erhs_5, 20, sizeof(int), &__ocl_i_bound);
		//------------------------------------------
		//OpenCL kernel arguments (END) 
		//------------------------------------------
	...
	//--------------------------------------------------------------
	{
		...
		oclGetWorkSize(2, _ocl_gws, NULL);
		oclSetKernelArg(__ocl_erhs_6, 0, sizeof(int), &k);
		oclSetKernelArg(__ocl_erhs_6, 1, sizeof(int), &nz);
		oclSetKernelArgBuffer(__ocl_erhs_6, 2, __ocl_buffer_flux);
		oclSetKernelArgBuffer(__ocl_erhs_6, 3, __ocl_buffer_rsd);
		oclSetKernelArg(__ocl_erhs_6, 4, sizeof(int), &m);
		oclSetKernelArgBuffer(__ocl_erhs_6, 5, __ocl_buffer_frct);
		oclSetKernelArg(__ocl_erhs_6, 6, sizeof(double), &tz2);
		oclSetKernelArg(__ocl_erhs_6, 7, sizeof(double), &tz3);
		oclSetKernelArg(__ocl_erhs_6, 8, sizeof(double), &dz1);
		oclSetKernelArg(__ocl_erhs_6, 9, sizeof(double), &tz1);
		oclSetKernelArg(__ocl_erhs_6, 10, sizeof(double), &dz2);
		oclSetKernelArg(__ocl_erhs_6, 11, sizeof(double), &dz3);
		oclSetKernelArg(__ocl_erhs_6, 12, sizeof(double), &dz4);
		oclSetKernelArg(__ocl_erhs_6, 13, sizeof(double), &dz5);
		oclSetKernelArg(__ocl_erhs_6, 14, sizeof(double), &dsspm);
		oclSetKernelArg(__ocl_erhs_6, 15, sizeof(int), &jst);
		oclSetKernelArg(__ocl_erhs_6, 16, sizeof(int), &ist);
		int __ocl_j_bound = jend;
		oclSetKernelArg(__ocl_erhs_6, 17, sizeof(int), &__ocl_j_bound);
		int __ocl_i_bound = iend;
		oclSetKernelArg(__ocl_erhs_6, 18, sizeof(int), &__ocl_i_bound);
		//------------------------------------------
		//OpenCL kernel arguments (END) 
		//------------------------------------------
...
static void pintgr()
{
	...
	double phi1[104][104];
	DECLARE_LOCALVAR_OCL_BUFFER(phi1, double, (104 * 104));
	double phi2[104][104];
	DECLARE_LOCALVAR_OCL_BUFFER(phi2, double, (104 * 104));
...
static void rhs()
{
	...
	//--------------------------------------------------------------
	{
		...
		oclSetKernelArg(__ocl_rhs_0, 1, sizeof(int), &m);
		oclSetKernelArgBuffer(__ocl_rhs_0, 2, __ocl_buffer_frct);
		int __ocl_k_bound = nz - 1;
		oclSetKernelArg(__ocl_rhs_0, 3, sizeof(int), &__ocl_k_bound);
		int __ocl_j_bound = ny - 1;
		oclSetKernelArg(__ocl_rhs_0, 4, sizeof(int), &__ocl_j_bound);
		int __ocl_i_bound = nx - 1;
		oclSetKernelArg(__ocl_rhs_0, 5, sizeof(int), &__ocl_i_bound);
		//------------------------------------------
		//OpenCL kernel arguments (END) 
		//------------------------------------------
	...
	//--------------------------------------------------------------
	{
		...
		oclGetWorkSize(2, _ocl_gws, NULL);
		oclSetKernelArg(__ocl_rhs_2, 0, sizeof(int), &i);
		oclSetKernelArg(__ocl_rhs_2, 1, sizeof(int), &ist);
		oclSetKernelArg(__ocl_rhs_2, 2, sizeof(int), &iend);
		oclSetKernelArg(__ocl_rhs_2, 3, sizeof(int), &m);
		oclSetKernelArgBuffer(__ocl_rhs_2, 4, __ocl_buffer_rsd);
		oclSetKernelArg(__ocl_rhs_2, 5, sizeof(double), &tx2);
		oclSetKernelArgBuffer(__ocl_rhs_2, 6, __ocl_buffer_flux);
		oclSetKernelArg(__ocl_rhs_2, 7, sizeof(int), &nx);
		oclSetKernelArgBuffer(__ocl_rhs_2, 8, __ocl_buffer_u);
		oclSetKernelArg(__ocl_rhs_2, 9, sizeof(double), &tx3);
		oclSetKernelArg(__ocl_rhs_2, 10, sizeof(double), &dx1);
		oclSetKernelArg(__ocl_rhs_2, 11, sizeof(double), &tx1);
		oclSetKernelArg(__ocl_rhs_2, 12, sizeof(double), &dx2);
		oclSetKernelArg(__ocl_rhs_2, 13, sizeof(double), &dx3);
		oclSetKernelArg(__ocl_rhs_2, 14, sizeof(double), &dx4);
		oclSetKernelArg(__ocl_rhs_2, 15, sizeof(double), &dx5);
		oclSetKernelArg(__ocl_rhs_2, 16, sizeof(double), &dssp);
		oclSetKernelArg(__ocl_rhs_2, 17, sizeof(int), &jst);
		int __ocl_k_bound = nz - 2;
		oclSetKernelArg(__ocl_rhs_2, 18, sizeof(int), &__ocl_k_bound);
		int __ocl_j_bound = jend;
		oclSetKernelArg(__ocl_rhs_2, 19, sizeof(int), &__ocl_j_bound);
		//------------------------------------------
		//OpenCL kernel arguments (END) 
		//------------------------------------------
	...
	//--------------------------------------------------------------
	{
		...
		oclGetWorkSize(2, _ocl_gws, NULL);
		oclSetKernelArg(__ocl_rhs_4, 0, sizeof(int), &j);
		oclSetKernelArg(__ocl_rhs_4, 1, sizeof(int), &jst);
		oclSetKernelArg(__ocl_rhs_4, 2, sizeof(int), &jend);
		oclSetKernelArg(__ocl_rhs_4, 3, sizeof(int), &m);
		oclSetKernelArgBuffer(__ocl_rhs_4, 4, __ocl_buffer_rsd);
		oclSetKernelArg(__ocl_rhs_4, 5, sizeof(double), &ty2);
		oclSetKernelArgBuffer(__ocl_rhs_4, 6, __ocl_buffer_flux);
		oclSetKernelArg(__ocl_rhs_4, 7, sizeof(int), &ny);
		oclSetKernelArgBuffer(__ocl_rhs_4, 8, __ocl_buffer_u);
		oclSetKernelArg(__ocl_rhs_4, 9, sizeof(double), &ty3);
		oclSetKernelArg(__ocl_rhs_4, 10, sizeof(double), &dy1);
		oclSetKernelArg(__ocl_rhs_4, 11, sizeof(double), &ty1);
		oclSetKernelArg(__ocl_rhs_4, 12, sizeof(double), &dy2);
		oclSetKernelArg(__ocl_rhs_4, 13, sizeof(double), &dy3);
		oclSetKernelArg(__ocl_rhs_4, 14, sizeof(double), &dy4);
		oclSetKernelArg(__ocl_rhs_4, 15, sizeof(double), &dy5);
		oclSetKernelArg(__ocl_rhs_4, 16, sizeof(double), &dssp);
		oclSetKernelArg(__ocl_rhs_4, 17, sizeof(int), &ist);
		int __ocl_k_bound = nz - 2;
		oclSetKernelArg(__ocl_rhs_4, 18, sizeof(int), &__ocl_k_bound);
		int __ocl_i_bound = iend;
		oclSetKernelArg(__ocl_rhs_4, 19, sizeof(int), &__ocl_i_bound);
		//------------------------------------------
		//OpenCL kernel arguments (END) 
		//------------------------------------------
	...
	//--------------------------------------------------------------
	{
		...
		oclGetWorkSize(2, _ocl_gws, NULL);
		oclSetKernelArg(__ocl_rhs_5, 0, sizeof(int), &k);
		oclSetKernelArg(__ocl_rhs_5, 1, sizeof(int), &nz);
		oclSetKernelArgBuffer(__ocl_rhs_5, 2, __ocl_buffer_flux);
		oclSetKernelArgBuffer(__ocl_rhs_5, 3, __ocl_buffer_u);
		oclSetKernelArg(__ocl_rhs_5, 4, sizeof(int), &m);
		oclSetKernelArgBuffer(__ocl_rhs_5, 5, __ocl_buffer_rsd);
		oclSetKernelArg(__ocl_rhs_5, 6, sizeof(double), &tz2);
		oclSetKernelArg(__ocl_rhs_5, 7, sizeof(double), &tz3);
		oclSetKernelArg(__ocl_rhs_5, 8, sizeof(double), &dz1);
		oclSetKernelArg(__ocl_rhs_5, 9, sizeof(double), &tz1);
		oclSetKernelArg(__ocl_rhs_5, 10, sizeof(double), &dz2);
		oclSetKernelArg(__ocl_rhs_5, 11, sizeof(double), &dz3);
		oclSetKernelArg(__ocl_rhs_5, 12, sizeof(double), &dz4);
		oclSetKernelArg(__ocl_rhs_5, 13, sizeof(double), &dz5);
		oclSetKernelArg(__ocl_rhs_5, 14, sizeof(double), &dssp);
		oclSetKernelArg(__ocl_rhs_5, 15, sizeof(int), &jst);
		oclSetKernelArg(__ocl_rhs_5, 16, sizeof(int), &ist);
		int __ocl_j_bound = jend;
		oclSetKernelArg(__ocl_rhs_5, 17, sizeof(int), &__ocl_j_bound);
		int __ocl_i_bound = iend;
		oclSetKernelArg(__ocl_rhs_5, 18, sizeof(int), &__ocl_i_bound);
		//------------------------------------------
		//OpenCL kernel arguments (END) 
		//------------------------------------------
...
static void setiv()
{
	...
	//--------------------------------------------------------------
	{
		...
		oclSetKernelArg(__ocl_setiv_0, 2, sizeof(int), &i);
		oclSetKernelArg(__ocl_setiv_0, 3, sizeof(int), &nx);
		oclSetKernelArg(__ocl_setiv_0, 4, sizeof(int), &nx0);
		oclSetKernelArg(__ocl_setiv_0, 5, sizeof(int), &m);
		oclSetKernelArgBuffer(__ocl_setiv_0, 6, __ocl_buffer_u);
		oclSetKernelArgBuffer(__ocl_setiv_0, 7, __ocl_buffer_ce);
		int __ocl_k_bound = nz - 1;
		oclSetKernelArg(__ocl_setiv_0, 8, sizeof(int), &__ocl_k_bound);
		int __ocl_j_bound = ny;
		oclSetKernelArg(__ocl_setiv_0, 9, sizeof(int), &__ocl_j_bound);
		//------------------------------------------
		//OpenCL kernel arguments (END) 
		//------------------------------------------
		...
		oclRunKernel(__ocl_setiv_0, 2, _ocl_gws);
	}
...
static void ssor()
{
	...
	int istep;
	double tmp;
	double delunm[5], tv[5][102][102];
	DECLARE_LOCALVAR_OCL_BUFFER(delunm, double, (5));
	DECLARE_LOCALVAR_OCL_BUFFER(tv, double, (5 * 102 * 102));
	tmp = 1.0 / (omega * (2.0 - omega));
	{
		...
		//--------------------------------------------------------------
		{
			...
			oclSetKernelArg(__ocl_ssor_0, 1, sizeof(int), &k);
			oclSetKernelArgBuffer(__ocl_ssor_0, 2, __ocl_buffer_b);
			oclSetKernelArgBuffer(__ocl_ssor_0, 3, __ocl_buffer_c);
			oclSetKernelArgBuffer(__ocl_ssor_0, 4, __ocl_buffer_d);
			//------------------------------------------
			//OpenCL kernel arguments (END) 
			//------------------------------------------
		...
		for (istep = 1; istep <= itmax; istep++) {
			...
			//--------------------------------------------------------------
			{
				...
				oclSetKernelArgBuffer(__ocl_ssor_1, 0,
						      __ocl_buffer_rsd);
				oclSetKernelArg(__ocl_ssor_1, 1, sizeof(int),
						&m);
				oclSetKernelArg(__ocl_ssor_1, 2, sizeof(double),
						&dt);
				...
				oclSetKernelArg(__ocl_ssor_1, 6, sizeof(int),
						&__ocl_j_bound);
				int __ocl_i_bound = iend;
				oclSetKernelArg(__ocl_ssor_1, 7, sizeof(int),
						&__ocl_i_bound);
				//------------------------------------------
				//OpenCL kernel arguments (END) 
				//------------------------------------------
			...
			for (k = 1; k <= nz - 2; k++) {
				...
				blts(nx, ny, nz, k, omega, rsd,
				     __ocl_buffer_rsd, a, __ocl_buffer_a, b,
				     __ocl_buffer_b, c, __ocl_buffer_c, d,
				     __ocl_buffer_d, ist, iend, jst, jend, nx0,
				     ny0);
			}
			for (k = nz - 2; k >= 1; k--) {
				jacu(k);
				...
				buts(nx, ny, nz, k, omega, rsd,
				     __ocl_buffer_rsd, tv, __ocl_buffer_tv, d,
				     __ocl_buffer_d, a, __ocl_buffer_a, b,
				     __ocl_buffer_b, c, __ocl_buffer_c, ist,
				     iend, jst, jend, nx0, ny0);
			}
			//--------------------------------------------------------------
			...
			//--------------------------------------------------------------
			{
				...
				oclSetKernelArgBuffer(__ocl_ssor_2, 0,
						      __ocl_buffer_u);
				oclSetKernelArg(__ocl_ssor_2, 1, sizeof(int),
						&m);
				oclSetKernelArg(__ocl_ssor_2, 2, sizeof(double),
						&tmp);
				oclSetKernelArgBuffer(__ocl_ssor_2, 3,
						      __ocl_buffer_rsd);
				...
				oclSetKernelArg(__ocl_ssor_2, 7, sizeof(int),
						&__ocl_j_bound);
				int __ocl_i_bound = iend;
				oclSetKernelArg(__ocl_ssor_2, 8, sizeof(int),
						&__ocl_i_bound);
				//------------------------------------------
				//OpenCL kernel arguments (END) 
				//------------------------------------------
...
static void init_ocl_runtime()
{
	...
	__ocl_program = oclBuildProgram("lu.B.cl");
...
{
	__ocl_buffer_frct =
	    oclCreateBuffer(frct, (5 * 102 * 103 * 103) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_frct, -1);
	__ocl_buffer_rsd =
	    oclCreateBuffer(rsd, (5 * 102 * 103 * 103) * sizeof(double));
	...
	__ocl_buffer_flux =
	    oclCreateBuffer(flux, (5 * 102 * 103 * 103) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_flux, -1);
	__ocl_buffer_u =
	    oclCreateBuffer(u, (5 * 102 * 103 * 103) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_u, -1);
	__ocl_buffer_d =
	    oclCreateBuffer(d, (5 * 5 * 102 * 102) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_d, -1);
	__ocl_buffer_a =
	    oclCreateBuffer(a, (5 * 5 * 102 * 102) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_a, -1);
	__ocl_buffer_b =
	    oclCreateBuffer(b, (5 * 5 * 102 * 102) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_b, -1);
	__ocl_buffer_c =
	    oclCreateBuffer(c, (5 * 5 * 102 * 102) * sizeof(double));
