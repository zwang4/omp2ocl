...
static void matvec_sub (double ablock[3][5][5][25][25][25],
			int ablock_0,
			int ablock_1, int ablock_2, int ablock_3,
			double avec[5][25][25][25],
			int avec_0,
			int avec_1, int avec_2, double bvec[5][25][25][25],
			int bvec_0,
			int bvec_1, int bvec_2);
static void matmul_sub (double ablock[3][5][5][25][25][25],
			int ablock_0,
			int ablock_1, int ablock_2, int ablock_3,
			double bblock[3][5][5][25][25][25],
			int bblock_0,
			int bblock_1, int bblock_2, int bblock_3,
			double cblock[3][5][5][25][25][25],
			int cblock_0,
			int cblock_1, int cblock_2, int cblock_3);
static void binvcrhs (double lhs[3][5][5][25][25][25],
		      int lhs_0, int lhs_1,
		      int lhs_2, int lhs_3, double c[3][5][5][25][25][25],
		      int c_0, int c_1, int c_2,
		      int c_3, double r[5][25][25][25],
		      int r_0, int r_1, int r_2);
static void binvrhs (double lhs[3][5][5][25][25][25],
		     int lhs_0, int lhs_1,
		     int lhs_2, int lhs_3, double r[5][25][25][25],
		     int r_0, int r_1, int r_2);
static void y_solve ();
...
/*Setting arugments for a loop that is defined at: bt.c : 722*/
  {
    size_t __ocl_global_work_size[3];
    __ocl_global_work_size[0] = (24) - (0);
    __ocl_global_work_size[1] = (24) - (0);
    __ocl_global_work_size[2] = (24) - (0);
...
//These variables are read only:
      ...
      for (j = 1; j < grid_points[1] - 1; j++)
	{
	  for (k = 1; k < grid_points[2] - 1; k++)
	    {
	      matvec_sub (lhs, i, j, k, 0, rhs,
			  i - 1, j, k, rhs,
			  i, j, k);
	      matmul_sub (lhs, i, j, k, 0, lhs,
			  i - 1, j, k, 2, lhs,
			  i, j, k, 1);
	      binvcrhs (lhs, i, j, k, 1, lhs,
			i, j, k, 2, rhs,
			i, j, k);
	    }
	}
  ...
  for (j = 1; j < grid_points[1] - 1; j++)
    {
      for (k = 1; k < grid_points[2] - 1; k++)
	{
	  matvec_sub (lhs, isize, j, k, 0, rhs,
		      isize - 1, j, k, rhs,
		      isize, j, k);
	  matmul_sub (lhs, isize, j, k, 0, lhs,
		      isize - 1, j, k, 2, lhs,
		      isize, j, k, 1);
	  binvrhs (lhs, i, j, k, 1, rhs,
		   i, j, k);
	}
    }
#endif
...
static void
matvec_sub (double ablock[3][5][5][25][25][25],
	    int ablock_0, int ablock_1,
	    int ablock_2, int ablock_3, double avec[5][25][25][25],
	    int avec_0, int avec_1,
	    int avec_2, double bvec[5][25][25][25],
	    int bvec_0, int bvec_1,
	    int bvec_2)
{
  int i;
  for (i = 0; i < 5; i++)
...
static void
matmul_sub (double ablock[3][5][5][25][25][25],
	    int ablock_0, int ablock_1,
	    int ablock_2, int ablock_3, double bblock[3][5][5][25][25][25],
	    int bblock_0, int bblock_1,
	    int bblock_2, int bblock_3, double cblock[3][5][5][25][25][25],
	    int cblock_0, int cblock_1,
...
static void
binvcrhs (double lhs[3][5][5][25][25][25],
	  int lhs_0, int lhs_1, int lhs_2, int lhs_3,
	  double c[3][5][5][25][25][25], int c_0,
	  int c_1, int c_2, int c_3, double r[5][25][25][25],
	  int r_0, int r_1, int r_2)
{
  double pivot, coeff;
...
static void
binvrhs (double lhs[3][5][5][25][25][25],
	 int lhs_0, int lhs_1, int lhs_2, int lhs_3, double r[5][25][25][25],
	 int r_0, int r_1, int r_2)
{
  double pivot, coeff;
  pivot = 1.00 / lhs[lhs_3][0][0][lhs_0][lhs_1][lhs_2];
  ...
  for (i = 1; i < grid_points[0] - 1; i++)
    {
      for (k = 1; k < grid_points[2] - 1; k++)
	{
	  matvec_sub (lhs, i, jsize, k, 0, rhs,
		      i, jsize - 1, k, rhs,
		      i, jsize, k);
	  matmul_sub (lhs, i, jsize, k, 0, lhs,
		      i, jsize - 1, k, 2, lhs,
		      i, jsize, k, 1);
	  binvrhs (lhs, i, jsize, k, 1, rhs,
		   i, jsize, k);
	}
    }
#endif
  ...
  for (i = 1; i < grid_points[0] - 1; i++)
    {
      for (j = 1; j < grid_points[1] - 1; j++)
	{
	  matvec_sub (lhs, i, j, ksize, 0, rhs,
		      i, j, ksize - 1, rhs,
		      i, j, ksize);
	  matmul_sub (lhs, i, j, ksize, 0, lhs,
		      i, j, ksize - 1, 2, lhs,
		      i, j, ksize, 1);
	  binvrhs (lhs, i, j, ksize, 1, rhs,
		   i, j, ksize);
	}
    }
#endif
...
init_ocl_runtime ()
{
  ...
  __ocl_program = oclBuildProgram ("bt.W.cl");
...
create_ocl_buffers ()
{
  __ocl_buffer_u = oclCreateBuffer (u, (5 * 25 * 25 * 25) * sizeof (double));
  DYN_BUFFER_CHECK (__ocl_buffer_u, -1);
  __ocl_buffer_rhs =
    oclCreateBuffer (rhs, (5 * 25 * 25 * 25) * sizeof (double));
  DYN_BUFFER_CHECK (__ocl_buffer_rhs, -1);
  __ocl_buffer_forcing =
    oclCreateBuffer (forcing, (6 * 25 * 25 * 25) * sizeof (double));
  ...
  __ocl_buffer_ue = oclCreateBuffer (ue, (5 * 24) * sizeof (double));
  DYN_BUFFER_CHECK (__ocl_buffer_ue, -1);
  __ocl_buffer_buf = oclCreateBuffer (buf, (5 * 24) * sizeof (double));
  ...
  __ocl_buffer_lhs =
    oclCreateBuffer (lhs, (3 * 5 * 5 * 25 * 25 * 25) * sizeof (double));
  DYN_BUFFER_CHECK (__ocl_buffer_lhs, -1);
  __ocl_buffer_fjac =
    oclCreateBuffer (fjac, (5 * 5 * 25 * 25 * 24) * sizeof (double));
  DYN_BUFFER_CHECK (__ocl_buffer_fjac, -1);
  __ocl_buffer_njac =
    oclCreateBuffer (njac, (5 * 5 * 25 * 25 * 24) * sizeof (double));
