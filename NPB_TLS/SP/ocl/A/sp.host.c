//-------------------------------------------------------------------------------
//Host code 
//Generated at : Thu Oct 25 14:32:19 2012
//Compiler options: 
//      Software Cache  true
//      Local Memory    true
...
static void create_ocl_buffers()
{
	...
	__ocl_buffer_lhs =
	    oclCreateBuffer(lhs, (15 * 65 * 65 * 65) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_lhs, -1);
	__ocl_buffer_speed =
	    oclCreateBuffer(speed, (65 * 65 * 65) * sizeof(double));
	...
	__ocl_buffer_ainv =
	    oclCreateBuffer(ainv, (65 * 65 * 65) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_ainv, -1);
	__ocl_buffer_grid_points =
	    oclCreateBuffer(grid_points, (3) * sizeof(int));
	DYN_BUFFER_CHECK(__ocl_buffer_grid_points, -1);
	__ocl_buffer_forcing =
	    oclCreateBuffer(forcing, (5 * 65 * 65 * 65) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_forcing, -1);

	//------------------------------------------
	// GPU TLS wr/rd buffers (BEGIN)
	//------------------------------------------
	rd_oclb_u = oclCreateBuffer(rd_log_u, (5 * 65 * 65 * 65) * sizeof(int));
	wr_oclb_u = oclCreateBuffer(wr_log_u, (5 * 65 * 65 * 65) * sizeof(int));
	oclHostWrites(rd_oclb_u);
	oclHostWrites(wr_oclb_u);
	DYN_BUFFER_CHECK(rd_oclb_u, -1);
	DYN_BUFFER_CHECK(wr_oclb_u, -1);
	rd_oclb_lhs =
	    oclCreateBuffer(rd_log_lhs, (15 * 65 * 65 * 65) * sizeof(int));
	wr_oclb_lhs =
	    oclCreateBuffer(wr_log_lhs, (15 * 65 * 65 * 65) * sizeof(int));
	oclHostWrites(rd_oclb_lhs);
	oclHostWrites(wr_oclb_lhs);
	DYN_BUFFER_CHECK(rd_oclb_lhs, -1);
	DYN_BUFFER_CHECK(wr_oclb_lhs, -1);
	rd_oclb_rhs =
	    oclCreateBuffer(rd_log_rhs, (5 * 65 * 65 * 65) * sizeof(int));
	wr_oclb_rhs =
	    oclCreateBuffer(wr_log_rhs, (5 * 65 * 65 * 65) * sizeof(int));
