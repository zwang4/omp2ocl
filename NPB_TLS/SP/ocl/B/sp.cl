			...
			__global int *tls_validflag)
{
	...
	//Convert global memory objects (BEGIN)
	//-------------------------------------------
	__global double (*lhs)[103][103][103] =
	    (__global double (*)[103][103][103])g_lhs;
	//-------------------------------------------
	//Convert global memory objects (END)
	//-------------------------------------------
		     ...
		     __global int *tls_validflag)
{
	...
	__global double (*lhs)[103][103][103] =
	    (__global double (*)[103][103][103])g_lhs;
	__global double (*speed)[103][103] =
	    (__global double (*)[103][103])g_speed;
	//-------------------------------------------
	//Convert global memory objects (END)
	//-------------------------------------------
	...
	//-------------------------------------------
	{
		...
		lhs[1 + 5][i][j][k] =
		    lhs[1][i][j][k] - dttx2 * speed_3 /*speed[i - 1][j][k] */ ;
		     ...
		     __global int *tls_validflag)
{
	...
	//-------------------------------------------
	{
		...
		lhs[0 + 5][i][j][k] = lhs[0][i][j][k];
		lhs[1 + 5][i][j][k] =
		    lhs[1][i][j][k] - dtty2 * speed_7 /*speed[i][j - 1][k] */ ;
		lhs[2 + 5][i][j][k] = lhs[2][i][j][k];
		lhs[3 + 5][i][j][k] =
		     ...
		     __global int *tls_validflag)
{
	...
	//-------------------------------------------
	{
		...
		lhs[0 + 5][i][j][k] = lhs[0][i][j][k];
		lhs[1 + 5][i][j][k] =
		    lhs[1][i][j][k] - dttz2 * speed_11 /*speed[i][j][k - 1] */ ;
		lhs[2 + 5][i][j][k] = lhs[2][i][j][k];
		lhs[3 + 5][i][j][k] =
			    ...
			    int __ocl_i_bound, __global int *tls_validflag)
{
	...
	__global double (*speed)[103][103] =
	    (__global double (*)[103][103])g_speed;
	__global double (*ainv)[103][103] =
	    (__global double (*)[103][103])g_ainv;
	//-------------------------------------------
	//Convert global memory objects (END)
	//-------------------------------------------
	...
	//-------------------------------------------
	{
		...
		rho_i[i][j][k] = rho_inv;
		us[i][j][k] = u_1[0] /*u[1][i][j][k] */ *rho_inv;
		vs[i][j][k] = u_1[1] /*u[2][i][j][k] */ *rho_inv;
		ws[i][j][k] = u_1[2] /*u[3][i][j][k] */ *rho_inv;
		square[i][j][k] =
		    0.5 *
	...
	//-------------------------------------------
	{
		...
		rhs[0][i][j][k] =
		    rhs[0][i][j][k] + dx1tx1 * (u[0][i + 1][j][k] -
						...
						[k]) -
		    tx2 *
		    (u_3[1] /*u[1][i + 1][j][k] */ -u_3[0]
		     /*u[1][i - 1][j][k] */ );
		rhs[1][i][j][k] =
		    rhs[1][i][j][k] +
		    dx2tx1 * (u_3[1] /*u[1][i + 1][j][k] */ -2.0 *
			      u[1][i][j][k] + u_3[0] /*u[1][i - 1][j][k] */ ) +
		    xxcon2 * con43 * (up1 - 2.0 * uijk + um1) -
		    tx2 * (u_3[1] /*u[1][i + 1][j][k] */ *up1 -
			   u_3[0] /*u[1][i - 1][j][k] */ *um1 +
			   (u[4][i + 1][j][k] - square[i + 1][j][k] -
			    u_3[3] /*u[4][i - 1][j][k] */ +square_1
			    /*square[i - 1][j][k] */ ) * c2);
		rhs[2][i][j][k] =
		    rhs[2][i][j][k] +
		    dx3tx1 * (u_3[4] /*u[2][i + 1][j][k] */ -2.0 *
			      u[2][i][j][k] + u_3[2] /*u[2][i - 1][j][k] */ ) +
		    xxcon2 * (vs[i + 1][j][k] - 2.0 * vs[i][j][k] +
			      vs[i - 1][j][k]) -
		    tx2 * (u_3[4] /*u[2][i + 1][j][k] */ *up1 -
			   u_3[2] /*u[2][i - 1][j][k] */ *um1);
		rhs[3][i][j][k] =
		    rhs[3][i][j][k] +
		    dx4tx1 * (u_3[6] /*u[3][i + 1][j][k] */ -2.0 *
			      u[3][i][j][k] + u_3[5] /*u[3][i - 1][j][k] */ ) +
		    xxcon2 * (ws[i + 1][j][k] - 2.0 * ws[i][j][k] +
			      ws[i - 1][j][k]) -
		    tx2 * (u_3[6] /*u[3][i + 1][j][k] */ *up1 -
			   u_3[5] /*u[3][i - 1][j][k] */ *um1);
		rhs[4][i][j][k] =
		    ...
		    xxcon5 * (u[4][i + 1][j][k] * rho_i[i + 1][j][k] -
			      2.0 * u_3[7] /*u[4][i][j][k] */ *rho_i[i][j][k] +
			      u_3[3] /*u[4][i - 1][j][k] */ *rho_i[i -
								   1][j][k]) -
		    tx2 * ((c1 * u[4][i + 1][j][k] - c2 * square[i + 1][j][k]) *
			   up1 -
			   (c1 * u_3[3] /*u[4][i - 1][j][k] */ -c2 *
			     ...
			     int __ocl_i_bound, __global int *tls_validflag)
{
	...
	//-------------------------------------------
	{
		...
		__global double *p_u_8_2 = (__global double *)&u[3][i][j][k];
		if ((unsigned long)p_u_8_2 % 64 == 0) {
			u_8[2] = vload2(0, p_u_8_2);
		} else {
			u_8[2].x = p_u_8_2[0];
			p_u_8_2++;
			u_8[2].y = p_u_8_2[0];
		...
		rhs[1][i][j][k] =
		    ...
		    + zzcon2 * (us[i][j][k + 1] -
				...
				us_1.x /*us[i][j][k - 1] */ ) -
		    tz2 * (u[1][i][j][k + 1] * wp1 -
			   u_9[0] /*u[1][i][j][k - 1] */ *wm1);
		rhs[2][i][j][k] =
		    rhs[2][i][j][k] +
		    dz3tz1 * (u_9[2] /*u[2][i][j][k + 1] */ -2.0 *
			      u_8[1].y /*u[2][i][j][k] */  +
			      u_8[1].x /*u[2][i][j][k - 1] */ ) +
		    zzcon2 * (vs[i][j][k + 1] - 2.0 * vs_1.y /*vs[i][j][k] */  +
			      vs_1.x /*vs[i][j][k - 1] */ ) -
		    tz2 * (u_9[2] /*u[2][i][j][k + 1] */ *wp1 -
			   u_8[1].x /*u[2][i][j][k - 1] */  * wm1);
		rhs[3][i][j][k] =
		    rhs[3][i][j][k] +
		    dz4tz1 * (u_8[2].y /*u[3][i][j][k + 1] */  -
			      2.0 * u_8[2].x /*u[3][i][j][k] */  +
			      u_9[1] /*u[3][i][j][k - 1] */ ) +
		    zzcon2 * con43 * (wp1 - 2.0 * wijk + wm1) -
		    tz2 * (u_8[2].y /*u[3][i][j][k + 1] */  * wp1 -
			   u_9[1] /*u[3][i][j][k - 1] */ *wm1 +
			     ...
			     int __ocl_i_bound, __global int *tls_validflag)
{
	...
	//-------------------------------------------
	{
		...
		//      u[m][i][j][k + 1]
		//-------------------------------------------
		__global double *p_u_11_0 = (__global double *)&u[m][i][j][k];
		if ((unsigned long)p_u_11_0 % 64 == 0) {
			u_11 = vload2(0, p_u_11_0);
		} else {
			u_11.x = p_u_11_0[0];
			p_u_11_0++;
			u_11.y = p_u_11_0[0];
		...
		rhs[m][i][j][k] =
		    rhs[m][i][j][k] - dssp * (5.0 * u_11.x /*u[m][i][j][k] */  -
					      4.0 *
					      u_11.y /*u[m][i][j][k + 1] */  +
					      u[m][i][j][k + 2]);
	}
			     ...
			     int __ocl_i_bound, __global int *tls_validflag)
{
	...
	//-------------------------------------------
	{
		...
		rhs[m][i][j][k] =
		    rhs[m][i][j][k] -
		    dssp * (-4.0 * u_13.x /*u[m][i][j][k - 1] */  +
			    6.0 * u_13.y /*u[m][i][j][k] */  -
			    4.0 * u_13.z /*u[m][i][j][k + 1] */  +
			    u_13.w /*u[m][i][j][k + 2] */ );
	}
	//-------------------------------------------
	//OpenCL kernel (END)
			     ...
			     __global int *tls_validflag)
{
	...
	//-------------------------------------------
	{
		for (k = 3 * 1; k <= grid_points[2] - 3 * 1 - 1; k++) {
			...
			//      u[m][i][j][k + 1]
			//-------------------------------------------
			__global double *p_u_15_0 =
			    (__global double *)&u[m][i][j][k - 2];
			if ((unsigned long)p_u_15_0 % 64 == 0) {
				u_15 = vload4(0, p_u_15_0);
			} else {
				u_15.x = p_u_15_0[0];
				p_u_15_0++;
				u_15.y = p_u_15_0[0];
			...
			rhs[m][i][j][k] =
			    rhs[m][i][j][k] -
			    dssp * (u_15.x /*u[m][i][j][k - 2] */  -
				    4.0 * u_15.y /*u[m][i][j][k - 1] */  +
				    6.0 * u_15.z /*u[m][i][j][k] */  -
				    4.0 * u_15.w /*u[m][i][j][k + 1] */  +
				    u[m][i][j][k + 2]);
		}
	}
	//-------------------------------------------
			     ...
			     int __ocl_i_bound, __global int *tls_validflag)
{
	...
	//-------------------------------------------
	{
		...
		rhs[m][i][j][k] =
		    rhs[m][i][j][k] - dssp * (u_17.x /*u[m][i][j][k - 2] */  -
					      4.0 *
					      u_17.y /*u[m][i][j][k - 1] */  +
					      6.0 * u_17.z /*u[m][i][j][k] */  -
					      4.0 *
					      u_17.w /*u[m][i][j][k + 1] */ );
	}
	//-------------------------------------------
			     ...
			     int __ocl_i_bound, __global int *tls_validflag)
{
	...
	//-------------------------------------------
	{
		...
		//      u[m][i][j][k - 1]
		//-------------------------------------------
		__global double *p_u_19_0 =
		    (__global double *)&u[m][i][j][k - 2];
		if ((unsigned long)p_u_19_0 % 64 == 0) {
			u_19 = vload2(0, p_u_19_0);
		} else {
			u_19.x = p_u_19_0[0];
			p_u_19_0++;
			u_19.y = p_u_19_0[0];
		...
		rhs[m][i][j][k] =
		    rhs[m][i][j][k] - dssp * (u_19.x /*u[m][i][j][k - 2] */  -
					      4.0 *
					      u_19.y /*u[m][i][j][k - 1] */  +
					      5.0 * u[m][i][j][k]);
	}
			...
			__global int *tls_validflag)
{
	...
	//-------------------------------------------
	{
		...
		n = (m - 3 + 1) * 5;
		for (i = 1; i <= grid_points_1 /*grid_points[0] */  - 2; i++) {
