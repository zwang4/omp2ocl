//-------------------------------------------------------------------------------
//Host code 
//Generated at : Thu Oct 25 14:33:29 2012
//Compiler options: 
//      Software Cache  true
//      Local Memory    true
...
static int firstcol;
static int lastcol;
static int colidx[15825001];
static int rowstr[75002];
static int iv[150002];
static int arow[15825001];
static int acol[15825001];
static double v[75002];
static double aelt[15825001];
static double a[15825001];
static double x[75003];
static double z[75003];
static double p[75003];
static double q[75003];
static double r[75003];
static double w[75003];
static double amult;
static double tran;
static double rnorm;
...
{
	{
		...
		} else if (75000 == 14000 && 13 == 11 && 75 == 15
			   && 60.0 == 20.0) {
			class = 'A';
			zeta_verify_value = 17.130235054029;
		} else if (75000 == 75000 && 13 == 13 && 75 == 75
			   && 60.0 == 60.0) {
			class = 'B';
			zeta_verify_value = 22.712745482631;
		} else if (75000 == 150000 && 13 == 15 && 75 == 75
			   && 60.0 == 110.0) {
			class = 'C';
		...
		makea(naa, nzz, a, __ocl_buffer_a, colidx, __ocl_buffer_colidx,
		      rowstr, __ocl_buffer_rowstr, 13, firstrow, lastrow,
		      firstcol, lastcol, 1.0e-1, arow, NULL, acol, NULL, aelt,
		      NULL, v, NULL, iv, NULL, 60.0);
		{
			//--------------------------------------------------------------
...
{
	__ocl_buffer_colidx = oclCreateBuffer(colidx, (15825001) * sizeof(int));
	DYN_BUFFER_CHECK(__ocl_buffer_colidx, -1);
	__ocl_buffer_rowstr = oclCreateBuffer(rowstr, (75002) * sizeof(int));
	DYN_BUFFER_CHECK(__ocl_buffer_rowstr, -1);
	__ocl_buffer_x = oclCreateBuffer(x, (75003) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_x, -1);
	__ocl_buffer_z = oclCreateBuffer(z, (75003) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_z, -1);
	__ocl_buffer_q = oclCreateBuffer(q, (75003) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_q, -1);
	__ocl_buffer_r = oclCreateBuffer(r, (75003) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_r, -1);
	__ocl_buffer_p = oclCreateBuffer(p, (75003) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_p, -1);
	__ocl_buffer_w = oclCreateBuffer(w, (75003) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_w, -1);
	__ocl_buffer_a = oclCreateBuffer(a, (15825001) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_a, -1);

	//------------------------------------------
	// GPU TLS wr/rd buffers (BEGIN)
	//------------------------------------------
	rd_oclb_w = oclCreateBuffer(rd_log_w, (75003) * sizeof(int));
	wr_oclb_w = oclCreateBuffer(wr_log_w, (75003) * sizeof(int));
