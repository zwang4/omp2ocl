//-------------------------------------------------------------------------------
//Host code 
//Generated at : Thu Oct 25 14:33:28 2012
//Compiler options: 
//      Software Cache  true
//      Local Memory    true
...
{
	{
		...
		} else if (14000 == 14000 && 11 == 11 && 15 == 15
			   && 20.0 == 20.0) {
			class = 'A';
			zeta_verify_value = 17.130235054029;
		} else if (14000 == 75000 && 11 == 13 && 15 == 75
			   && 20.0 == 60.0) {
			class = 'B';
			zeta_verify_value = 22.712745482631;
		} else if (14000 == 150000 && 11 == 15 && 15 == 75
			   && 20.0 == 110.0) {
			class = 'C';
		...
		makea(naa, nzz, a, __ocl_buffer_a, colidx, __ocl_buffer_colidx,
		      rowstr, __ocl_buffer_rowstr, 11, firstrow, lastrow,
		      firstcol, lastcol, 1.0e-1, arow, NULL, acol, NULL, aelt,
		      NULL, v, NULL, iv, NULL, 20.0);
		{
			//--------------------------------------------------------------
...
{
	__ocl_buffer_colidx = oclCreateBuffer(colidx, (2198001) * sizeof(int));
	DYN_BUFFER_CHECK(__ocl_buffer_colidx, -1);
	__ocl_buffer_rowstr = oclCreateBuffer(rowstr, (14002) * sizeof(int));
	DYN_BUFFER_CHECK(__ocl_buffer_rowstr, -1);
	__ocl_buffer_x = oclCreateBuffer(x, (14003) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_x, -1);
	__ocl_buffer_z = oclCreateBuffer(z, (14003) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_z, -1);
	__ocl_buffer_q = oclCreateBuffer(q, (14003) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_q, -1);
	__ocl_buffer_r = oclCreateBuffer(r, (14003) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_r, -1);
	__ocl_buffer_p = oclCreateBuffer(p, (14003) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_p, -1);
	__ocl_buffer_w = oclCreateBuffer(w, (14003) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_w, -1);
	__ocl_buffer_a = oclCreateBuffer(a, (2198001) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_a, -1);

	//------------------------------------------
	// GPU TLS wr/rd buffers (BEGIN)
	//------------------------------------------
	rd_oclb_w = oclCreateBuffer(rd_log_w, (14003) * sizeof(int));
	wr_oclb_w = oclCreateBuffer(wr_log_w, (14003) * sizeof(int));
