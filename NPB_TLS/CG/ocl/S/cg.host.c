...
static int firstcol;
static int lastcol;
static int colidx[102201];
static int rowstr[1402];
static int iv[2802];
static int arow[102201];
static int acol[102201];
static double v[1402];
static double aelt[102201];
static double a[102201];
static double x[1403];
static double z[1403];
static double p[1403];
static double q[1403];
static double r[1403];
static double w[1403];
static double amult;
static double tran;
static double rnorm;
...
{
	{
		...
		} else if (1400 == 7000 && 7 == 8 && 15 == 15 && 10.0 == 12.0) {
			class = 'W';
			zeta_verify_value = 10.362595087124;
		} else if (1400 == 14000 && 7 == 11 && 15 == 15 && 10.0 == 20.0) {
			class = 'A';
			zeta_verify_value = 17.130235054029;
		} else if (1400 == 75000 && 7 == 13 && 15 == 75 && 10.0 == 60.0) {
			class = 'B';
			zeta_verify_value = 22.712745482631;
		} else if (1400 == 150000 && 7 == 15 && 15 == 75
			   && 10.0 == 110.0) {
			class = 'C';
		...
		makea(naa, nzz, a, __ocl_buffer_a, colidx, __ocl_buffer_colidx,
		      rowstr, __ocl_buffer_rowstr, 7, firstrow, lastrow,
		      firstcol, lastcol, 1.0e-1, arow, NULL, acol, NULL, aelt,
		      NULL, v, NULL, iv, NULL, 10.0);
		{
			//--------------------------------------------------------------
			...
			//--------------------------------------------------------------
			{
				...
				oclSetKernelArg(__ocl_main_0, 4, sizeof(int),
						&__ocl_k_bound);
				oclSetKernelArgBuffer(__ocl_main_0, 5,
...
//Do the final reduction part on the CPU
				...
				//--------------------------------------------------------------
				{
					...
					oclSetKernelArg(__ocl_main_3, 3,
							sizeof(int),
							&__ocl_j_bound);
					oclSetKernelArgBuffer(__ocl_main_3, 4,
							      __oclb_gpu_tls_conflict_flag);
					//------------------------------------------
					//OpenCL kernel arguments (END) 
					//------------------------------------------
			...
			//--------------------------------------------------------------
			{
				...
				oclSetKernelArg(__ocl_main_4, 1, sizeof(int),
						&__ocl_i_bound);
				oclSetKernelArgBuffer(__ocl_main_4, 2,
						      __oclb_gpu_tls_conflict_flag);
				//------------------------------------------
				//OpenCL kernel arguments (END) 
				//------------------------------------------
...
//Do the final reduction part on the CPU
				...
				//--------------------------------------------------------------
				{
					...
					oclSetKernelArg(__ocl_main_6, 3,
							sizeof(int),
							&__ocl_j_bound);
					oclSetKernelArgBuffer(__ocl_main_6, 4,
							      __oclb_gpu_tls_conflict_flag);
					//------------------------------------------
					//OpenCL kernel arguments (END) 
					//------------------------------------------
		...
		if (t != 0.0) {
			mflops =
			    (2.0 * 15 * 1400) * (3.0 + (7 * (7 + 1)) +
						 25.0 * (5.0 + (7 * (7 + 1))) +
						 3.0) / t / 1000000.0;
		} else {
			mflops = 0.0;
		}
		c_print_results("CG", class, 1400, 0, 0, 15, nthreads, t,
...
static void conj_grad()
{
	...
	//--------------------------------------------------------------
	{
		...
		oclSetKernelArg(__ocl_conj_grad_0, 6, sizeof(int),
				&__ocl_j_bound);
		oclSetKernelArgBuffer(__ocl_conj_grad_0, 7,
				      __oclb_gpu_tls_conflict_flag);
		//------------------------------------------
		//OpenCL kernel arguments (END) 
		//------------------------------------------
	...
	for (cgit = 1; cgit <= cgitmax; cgit++) {
		...
		//--------------------------------------------------------------
		{
			...
			oclSetKernelArg(__ocl_conj_grad_2, 5, sizeof(int),
					&__ocl_j_bound);
			oclSetKernelArgBuffer(__ocl_conj_grad_2, 6,
					      __oclb_gpu_tls_conflict_flag);
			//------------------------------------------
			//OpenCL kernel arguments (END) 
			//------------------------------------------
		...
		//--------------------------------------------------------------
		{
			...
			oclSetKernelArg(__ocl_conj_grad_3, 2, sizeof(int),
					&__ocl_j_bound);
			oclSetKernelArgBuffer(__ocl_conj_grad_3, 3,
					      __oclb_gpu_tls_conflict_flag);
			//------------------------------------------
			//OpenCL kernel arguments (END) 
			//------------------------------------------
		...
		//--------------------------------------------------------------
		{
			...
			oclSetKernelArg(__ocl_conj_grad_4, 1, sizeof(int),
					&__ocl_j_bound);
			oclSetKernelArgBuffer(__ocl_conj_grad_4, 2,
					      __oclb_gpu_tls_conflict_flag);
			//------------------------------------------
			//OpenCL kernel arguments (END) 
			//------------------------------------------
...
//Do the final reduction part on the CPU
		...
		//--------------------------------------------------------------
		{
			...
			oclSetKernelArg(__ocl_conj_grad_6, 5, sizeof(int),
					&__ocl_j_bound);
			oclSetKernelArgBuffer(__ocl_conj_grad_6, 6,
					      __oclb_gpu_tls_conflict_flag);
			//------------------------------------------
			//OpenCL kernel arguments (END) 
			//------------------------------------------
...
//Do the final reduction part on the CPU
		...
		//--------------------------------------------------------------
		{
			...
			oclSetKernelArg(__ocl_conj_grad_8, 3, sizeof(int),
					&__ocl_j_bound);
			oclSetKernelArgBuffer(__ocl_conj_grad_8, 4,
					      __oclb_gpu_tls_conflict_flag);
			//------------------------------------------
			//OpenCL kernel arguments (END) 
			//------------------------------------------
	...
	//--------------------------------------------------------------
	{
		...
		oclSetKernelArg(__ocl_conj_grad_9, 5, sizeof(int),
				&__ocl_j_bound);
		oclSetKernelArgBuffer(__ocl_conj_grad_9, 6,
				      __oclb_gpu_tls_conflict_flag);
		//------------------------------------------
		//OpenCL kernel arguments (END) 
		//------------------------------------------
	...
	//--------------------------------------------------------------
	{
		...
		oclSetKernelArg(__ocl_conj_grad_10, 2, sizeof(int),
				&__ocl_j_bound);
		oclSetKernelArgBuffer(__ocl_conj_grad_10, 3,
				      __oclb_gpu_tls_conflict_flag);
		//------------------------------------------
		//OpenCL kernel arguments (END) 
		//------------------------------------------
...
{
	__ocl_buffer_colidx = oclCreateBuffer(colidx, (102201) * sizeof(int));
	DYN_BUFFER_CHECK(__ocl_buffer_colidx, -1);
	__ocl_buffer_rowstr = oclCreateBuffer(rowstr, (1402) * sizeof(int));
	DYN_BUFFER_CHECK(__ocl_buffer_rowstr, -1);
	__ocl_buffer_x = oclCreateBuffer(x, (1403) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_x, -1);
	__ocl_buffer_z = oclCreateBuffer(z, (1403) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_z, -1);
	__ocl_buffer_q = oclCreateBuffer(q, (1403) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_q, -1);
	__ocl_buffer_r = oclCreateBuffer(r, (1403) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_r, -1);
	__ocl_buffer_p = oclCreateBuffer(p, (1403) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_p, -1);
	__ocl_buffer_w = oclCreateBuffer(w, (1403) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_w, -1);
	__ocl_buffer_a = oclCreateBuffer(a, (102201) * sizeof(double));
	DYN_BUFFER_CHECK(__ocl_buffer_a, -1);

	//------------------------------------------
	// GPU TLS wr/rd buffers (BEGIN)
	//------------------------------------------
	rd_oclb_w = oclCreateBuffer(rd_log_w, (1403) * sizeof(int));
	wr_oclb_w = oclCreateBuffer(wr_log_w, (1403) * sizeof(int));
