...
{
	{
		...
		timer_stop(1);
		tmax = (t2.tv_sec - t1.tv_sec) + (t2.tv_usec - t1.tv_usec) * 1E-06;
...
{
	lhsz();
	oclSwapDimensions (__ocl_buffer_lhs, 3 * 5 * 5 * (64 / 2 * 2 + 1), 64 / 2 * 2 + 1, 1,
...
//Checking forcing
		{
			size_t __ocl_gws_forcing = (103 * 103 * 103 * 6);
			oclSetKernelArg(__ocl_tls_1D_checking, 0,
					sizeof(unsigned), &__ocl_gws_forcing);
			oclSetKernelArgBuffer(__ocl_tls_1D_checking, 1,
					      rd_oclb_forcing);
			oclSetKernelArgBuffer(__ocl_tls_1D_checking, 2,
					      wr_oclb_forcing);
			oclSetKernelArgBuffer(__ocl_tls_1D_checking, 3,
					      __oclb_gpu_tls_conflict_flag);
			oclDevWrites(wr_oclb_forcing);
			oclDevWrites(rd_oclb_forcing);
			oclDevWrites(__oclb_gpu_tls_conflict_flag);
...
//Checking lhs
		{
			size_t __ocl_gws_lhs = (103 * 103 * 103 * 5 * 5 * 3);
			oclSetKernelArg(__ocl_tls_1D_checking, 0,
