...
#pragma omp parallel for reduction(+:norm_temp11,norm_temp12) notls
...
#pragma omp parallel for reduction(+:norm_temp11,norm_temp12) schedule(static) notls
	for (j = 1; j <= lastcol-firstcol+1; j++) {
            norm_temp11 = norm_temp11 + x[j]*z[j];
            norm_temp12 = norm_temp12 + z[j]*z[j];
	}
...
c-------------------------------------------------------------------*/
#pragma omp parallel for reduction(+:rho) schedule(static ) notls
    for (j = 1; j <= lastcol-firstcol+1; j++) {
	rho = rho + x[j]*x[j];
    }
...
c  Obtain p.q
c-------------------------------------------------------------------*/
#pragma omp parallel for reduction(+:d) schedule(static) notls
	for (j = 1; j <= lastcol-firstcol+1; j++) { 
            d = d + p[j]*q[j];
	}
...
#pragma omp parallel for private(d, k) schedule(static)
    for (j = 1; j <= lastrow-firstrow+1; j++) {
	d = 0.0;
...
c-----------------------------t--------------------------------------*/
#pragma omp parallel for reduction(+:sum) private(d) schedule(static) notls
    for (j = 1; j <= lastcol-firstcol+1; j++) {
	d = x[j] - r[j];
	sum = sum + d*d;
    }
