...
//Functions (BEGIN)
//-------------------------------------------------------------------------------
void fftz2_g6_g7_g8_g9_g10_g11_spec(int is, int l, int m, int n, int ny, int ny1,
				   __global double *u_real,
				   __global double *rd_u_real,
				   __global double *wr_u_real,
				   __global double *u_imag,
				   __global double *rd_u_imag,
				   __global double *wr_u_imag,
				   __global double *x_real,
				   __global double *rd_x_real,
				   __global double *wr_x_real,
				   __global double *x_imag,
				   __global double *rd_x_imag,
				   __global double *wr_x_imag,
				   __global double *y_real,
				   __global double *rd_y_real,
				   __global double *wr_y_real,
				   __global double *y_imag,
				   __global double *rd_y_imag,
				   __global double *wr_y_imag,
...
void fftz2_g6_g7_g8_g9_g10_g11_gtp(int is, int l, int m, int n, int ny, int ny1,
				   ...
				   unsigned int __ocl_mult_factor,
				   unsigned int __ocl_add_offset);
void fftz2(int is, int l, int m, int n, int ny, int ny1, double u_real[256],
	   double u_imag[256], double x_real[256][18], double x_imag[256][18],
	   double y_real[256][18], double y_imag[256][18]);
	...
	if (m % 2 == 1) {
		for (j = 0; j < n; j++) {
			for (i = 0; i < fftblock; i++) {
				x_real[CALC_2D_IDX(256, 18, (j), (i)) *
				       __ocl_mult_factor + __ocl_add_offset] =
				    y_real[CALC_2D_IDX(256, 18, (j), (i)) *
					   __ocl_mult_factor +
					   __ocl_add_offset];
				x_imag[CALC_2D_IDX(256, 18, (j), (i)) *
				       __ocl_mult_factor + __ocl_add_offset] =
				    y_imag[CALC_2D_IDX(256, 18, (j), (i)) *
					   __ocl_mult_factor +
					   __ocl_add_offset];
			}
		}
...
void cfftz_g3_g4_g5_g6_g7_g10_e8_e9_spec(int is, int m, int n,
					__global double *x_real,
					__global double *rd_x_real,
					__global double *wr_x_real,
					__global double *x_imag,
					...
					__global int *tls_validflag, int tls_thread_id)
{
	...
	mx = (int)(u_real[(0)]);
	for (l = 1; l < m; l += 2) {
		fftz2_g6_g7_g8_g9_g10_g11_spec(is, l, m, n, fftblock,
					      fftblockpad, u_real, rd_u_real, wr_u_real,
					      u_imag, rd_u_imag, wr_u_imag,
					      x_real, rd_x_real, wr_x_real,
					      x_imag, rd_x_imag, wr_x_imag,
					      y_real, rd_y_real, wr_y_real,
					      y_imag, rd_y_imag, wr_y_imag,
					      __ocl_mult_factor,
					      __ocl_add_offset,
					      tls_validflag, tls_thread_id
					      );
		fftz2_g6_g7_g8_g9_g10_g11_spec(is, l + 1, m, n, fftblock,
					      fftblockpad, 
					      u_real, rd_u_real, wr_u_real,
					      u_imag, rd_u_imag, wr_u_imag,
					      y_real, rd_y_real, wr_y_real,
					      y_imag, rd_y_imag, wr_y_imag,
					      x_real, rd_x_real, wr_x_real,
	...
	for (i = 0; i < li; i++) {
		...
		for (k = 0; k < lk; k++) {
			for (j = 0; j < ny; j++) {
				...
				double x21real, x21imag;
				x11real =
				    x_real[CALC_2D_IDX(256, 18, (i11 + k), (j))
					   * __ocl_mult_factor +
					   __ocl_add_offset];
				x11imag =
				    x_imag[CALC_2D_IDX(256, 18, (i11 + k), (j))
					   * __ocl_mult_factor +
					   __ocl_add_offset];
				x21real =
				    x_real[CALC_2D_IDX(256, 18, (i12 + k), (j))
					   * __ocl_mult_factor +
					   __ocl_add_offset];
				x21imag =
				    x_imag[CALC_2D_IDX(256, 18, (i12 + k), (j))
					   * __ocl_mult_factor +
					   __ocl_add_offset];
				y_real[CALC_2D_IDX(256, 18, (i21 + k), (j)) *
				       __ocl_mult_factor + __ocl_add_offset] =
				    x11real + x21real;
				y_imag[CALC_2D_IDX(256, 18, (i21 + k), (j)) *
				       __ocl_mult_factor + __ocl_add_offset] =
				    x11imag + x21imag;
				y_real[CALC_2D_IDX(256, 18, (i22 + k), (j)) *
				       __ocl_mult_factor + __ocl_add_offset] =
				    u1_real * (x11real - x21real) -
				    u1_imag * (x11imag - x21imag);
				y_imag[CALC_2D_IDX(256, 18, (i22 + k), (j)) *
				       __ocl_mult_factor + __ocl_add_offset] =
				    u1_real * (x11imag - x21imag) +
				    u1_imag * (x11real - x21real);
			}
				   ...
				   )
{
	...
	for (i = 0; i < li; i++) {
		...
		int __t = (ku + i);
		u1_real = spec_read_double(&u_real[__t], 
					    &wr_u_real[__t],
					    &rd_u_real[__t],
					    tls_thread_id,
					    tls_validflag);
		
		u1_imag = spec_read_double(&u_imag[__t], 
					    &wr_u_imag[__t],
					    &rd_u_imag[__t],
					    tls_thread_id,
					    tls_validflag);
		       ...
		       int __ocl_i_bound, int __ocl_j_bound, int __ocl_k_bound)
{
	...
	//-------------------------------------------
	{
		u1_real[CALC_3D_IDX(128, 256, 256, (k), (j), (i))] =
		    u0_real[CALC_3D_IDX(128, 256, 256, (k), (j), (i))] *
		    ex[(t *
			indexmap[CALC_3D_IDX(128, 256, 256, (k), (j), (i))])];
		u1_imag[CALC_3D_IDX(128, 256, 256, (k), (j), (i))] =
		    u0_imag[CALC_3D_IDX(128, 256, 256, (k), (j), (i))] *
		    ex[(t *
			indexmap[CALC_3D_IDX(128, 256, 256, (k), (j), (i))])];
	}
	//-------------------------------------------
				 ...
				 int __ocl_j_bound, int __ocl_k_bound)
{
	...
	//-------------------------------------------
	{
		ii = (i + 1 + xstart_i - 2 + 256 / 2) % 256 - 256 / 2;
		ii2 = ii * ii;
		jj = (j + 1 + ystart_i - 2 + 256 / 2) % 256 - 256 / 2;
		ij2 = jj * jj + ii2;
		kk = (k + 1 + zstart_i - 2 + 128 / 2) % 128 - 128 / 2;
		indexmap[CALC_3D_IDX(128, 256, 256, (k), (j), (i))] =
		    kk * kk + ij2;
	}
	//-------------------------------------------
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void cffts1_0(
		       	int fftblock, 
			   __global int *d, 
			  
			   __global double *yy0_real,
			  __global double *rd_yy0_real,
			  __global double *wr_yy0_real,
		       
			  __global double *x_real, 
		       __global double *rd_x_real, 
		       __global double *wr_x_real, 
		       
		       __global double *yy0_imag,
		       __global double *rd_yy0_imag,
		       __global double *wr_yy0_imag,
		       ...
		       __global double *yy1_real, 
		       __global double *rd_yy1_real, 
		       __global double *wr_yy1_real, 
		       
		       __global double *yy1_imag,
		       __global double *rd_yy1_imag,
		       __global double *wr_yy1_imag,
		       
		       __global double *xout_real, 
		       __global double *rd_xout_real, 
		       __global double *wr_xout_real, 
		       
		       __global double *xout_imag,
		       __global double *rd_xout_imag,
		       __global double *wr_xout_imag,
		       ...
		       )
{
	...
	//-------------------------------------------
	{
		for (j = 0; j < fftblock; j++) {
			for (i = 0; i < d[(0)]; i++) {
				yy0_real[CALC_2D_IDX(256, 18, (i), (j)) *
					 __ocl_mult_factor + __ocl_add_offset] =
				    x_real[CALC_3D_IDX
					   (128, 256, 256, (k), (j + jj), (i))];
				yy0_imag[CALC_2D_IDX(256, 18, (i), (j)) *
					 __ocl_mult_factor + __ocl_add_offset] =
				    x_imag[CALC_3D_IDX
					   (128, 256, 256, (k), (j + jj), (i))];
			}
		...
		cfftz_g3_g4_g5_g6_g7_g10_e8_e9_spec(is, logd_0, d[(0)], 
		       ...
		       int __ocl_ii_bound, int __ocl_k_bound)
{
	...
	//-------------------------------------------
	{
		for (j = 0; j < d[(1)]; j++) {
			for (i = 0; i < fftblock; i++) {
				yy0_real[CALC_2D_IDX(256, 18, (j), (i)) *
					 __ocl_mult_factor + __ocl_add_offset] =
				    x_real[CALC_3D_IDX
					   (128, 256, 256, (k), (j), (i + ii))];
				yy0_imag[CALC_2D_IDX(256, 18, (j), (i)) *
					 __ocl_mult_factor + __ocl_add_offset] =
				    x_imag[CALC_3D_IDX
					   (128, 256, 256, (k), (j), (i + ii))];
			}
		       ...
		       int __ocl_ii_bound, int __ocl_j_bound)
{
	...
	//-------------------------------------------
	{
		for (k = 0; k < d[(2)]; k++) {
			for (i = 0; i < fftblock; i++) {
				yy0_real[CALC_2D_IDX(256, 18, (k), (i)) *
					 __ocl_mult_factor + __ocl_add_offset] =
				    x_real[CALC_3D_IDX
					   (128, 256, 256, (k), (j), (i + ii))];
				yy0_imag[CALC_2D_IDX(256, 18, (k), (i)) *
					 __ocl_mult_factor + __ocl_add_offset] =
				    x_imag[CALC_3D_IDX
					   (128, 256, 256, (k), (j), (i + ii))];
			}
					 ...
					 __global double *__ocl_part_chk_imag)
{
	...
	//-------------------------------------------
	//__global double (*u1_real)[256][256] =
	//    (__global double (*)[256][256])g_u1_real;
	//__global double (*u1_imag)[256][256] =
	//    (__global double (*)[256][256])g_u1_imag;
	//-------------------------------------------
