...
namespace clang {
	...
	class Preprocessor : public llvm::RefCountedBase<Preprocessor> {
		...
		//Zheng: this is for OpenCL code generation
		bool topIncFile;
		string concernFile;
		vector<string> headFiles;
		void AddHeadFile(string file);
		...
		/// Zheng: this is used to store omp 
		std::vector<OMPFor> ompfors;
		/// Zheng: this is used for threadprivate variables
		...
		OMPThreadPrivate& getOMPThreadPrivate()
		{
			return otp;
		}
		vector<string>& getMainHeadFiles() { return headFiles; }
		...
		void EnterSourceFile(FileID CurFileID, const DirectoryLookup *Dir,
				SourceLocation Loc, bool isMainFile=false);
		...
		//ZHENG: This is (for omp to opencl
		void ParseParagmOMPEatToken(Token &Tok, OMPFor &omp);
		void ParseParagmOMPNumThreads(Token &tok);
		OMPParallelDepth ParseParagmOMPParallelDepth(Token &tok);
		void ParseParagmOMPSwapSequence(Token &Tok, OMPFor &omp);
		int ParseParagmOMPReduction(Token &tok, OMPFor &omp);
		void ParseOMPThreadPrivate(Token &pragmaTok, OMPThreadPrivate &omp);
		void ParseParagmOMPPrivate(Token &Tok, OMPFor &omp);
		void ParseParagmOMPFirstPrivate(Token &Tok, OMPFor &omp);
		void ParseParagmOMPCopyIn(Token &Tok, OMPFor &omp);
		void ParseParagmOMPTLSVar(Token &Tok, OMPFor &omp);
		void ParseParagmOMPFor(Token &Tok, OMPFor &omp);
		int ParseParagmOMPVariables(Token &Tok, std::vector<std::string> &v);
		std::string ParseParagmOMPSchedule(Token &Tok);
		void ParseParagmOMPMultIteration(Token &Tok, OMPFor &omp);
		int ParseParagmOMPIntSeq(Token &tok, std::vector<std::string> &v);
		void ProcessOCLLocalVars(Token &Tok);
		void ProcessHostFlushVars(Token &Toki,  Omp2OclHostFlush* f );
		void ProcessHostReadVars(Token &Toki,  Omp2OclHostRead* f );
		void ProcessDevReadVars(Token &Toki,  Omp2OclDevRead* f );
		void ProcessDevWriteVars(Token &Toki,  Omp2OclDevWrite* f );

		vector<Omp2OclObj*> omp2oclObjs;
		...
		//Omp2ocl
		void HandlePragmaOMP(Token &OMPToken);
		void HandlePragmaOmp2Ocl(Token &Tok);
		OMPFor* hasMetOMPFor();
		OMPFor PopOMPFor();
		
		Omp2OclObj* topOmp2OclObj();
		void popOmp2OclObj();
		void addOmp2OclObj(Omp2OclObj* obj);
