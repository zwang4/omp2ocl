...
namespace clang {
	...
	class ValueDecl : public NamedDecl {
		...
		//ZHENG: This is used to track the offset that should be applied when access a 
		//global memory object in the OpenCL Kernel
		...
		ValueDecl(Kind DK, DeclContext *DC, SourceLocation L,
			...
			: NamedDecl(DK, DC, L, N), DeclType(T) {
				backupOrigType = false;
				useAsLocalVar = false;
			}
		public:
		QualType getType() const { return DeclType; }

		bool isUseAsLocalBuf()
		{
			return useAsLocalVar;
		}

		void setUseAsLocalVar()
		{
			useAsLocalVar = true;
		}
		...
		QualType getOrigType() { return origType; }

		void setOffsetString(string& offset)
		{
			this->offset_string = offset;
		}
	...
	class VarDecl : public DeclaratorDecl, public Redeclarable<VarDecl> {
		...
		public:
			...
			//ZHENG: USE FOR OPENCL TO SPECIFY WHERE A FUNCTION ARGUMENT IS A
			//GLOBAL MEMORY BUFFER
			void set2GlobalBuf() { isGlobalBuf = true;}
			bool isAGlobalBufDecl() { return isGlobalBuf; }
	...
	//Extended arguments for OpenCL Kernels
	...
	class ParmVarDecl : public VarDecl {
		...
		//ZHENG: THIS IS FOR OPENCL KERNEL ONLY
		bool isDeclareAsOCLGlobal;
		bool isExtended;
		...
		//Zheng, this if for OpenCL code generation
		bool isOCLGlobal() { return isDeclareAsOCLGlobal; }
		void setOCLGlobal() { isDeclareAsOCLGlobal = true; }
		void setOCLExtended() { isExtended = true;}
		bool isOCLExtended() { return isExtended;}
		void setOCLPointerAccess() { isPointerAccess = true; }
		bool isOCLPointerAccess() { return isPointerAccess; }
	...
	public Redeclarable<FunctionDecl> {
		...
		private:
			...
			//ZHENG: THIS IS FOR OPENCL KERNEL
			bool isFuncProtoChanged;
			
			bool bodyHasGlobalVar;
			vector<FuncProtoExt> addedOpenCLNDRangeVars;
			vector<OCLGlobalMemVar> FLevelOCLBufs;
			vector<ValueDecl*> FLOCLBufferDeclared; //FUNCITON LEVEL OBJECTS THAT HAS BEEN DECLARED					
		protected:
			FunctionDecl(Kind DK, DeclContext *DC, SourceLocation StartLoc,
				...
				{ 
					origParamInfo = NULL; 
					isFuncProtoChanged = false;
					bodyHasGlobalVar = false;
				}
		...
		public:
			...
			void addDeclaredFLObj(ValueDecl* D)
			{
				if (!isFLObjDeclare(D))
				{
					FLOCLBufferDeclared.push_back(D);
				}
			}

			bool isFLObjDeclare(ValueDecl* D)
			{
				string name = D->getNameAsString();
				for (unsigned i = 0; i<FLOCLBufferDeclared.size(); i++)
				{
					if (FLOCLBufferDeclared[i]->getNameAsString() == name)
					{
						return true;
					}
				}
			...
			//ZHENG: If a function is used by an OpenCL Kernels and 
			//it accesses some global variables, the arguments of the function will
			//be converted. This means the global variables will be passed as function
			//arguments. Because of this, all the callarg to this funciton in the 
			//OpenCL space will be revised. 
			//
			//In order to do this, I need to keep all the global variables that 
			//is converted to function arguments.
			vector<FuncProtoExt>& getAddedOpenCLNDRangeVars();
			bool hasFuncProtoChanged() { return isFuncProtoChanged; }
			bool isBodyHasGlobalVar() { return bodyHasGlobalVar;}
			void setBodyHasGlobalVar() { bodyHasGlobalVar = true; }

			//Restore the function prototype because it may be revised by 
			//an OpenCL kernel. When it is printed for the host-side, it will 
			//be appear as the original one
			void RestoreOCLRevisedParams()
			{
				if (origParamInfo)
				{
					 setType(getOrigType());
					 ParamInfo = origParamInfo;
				}	
			}

			void addFunctionLevelOCLBuffer(OCLGlobalMemVar o);
			vector<OCLGlobalMemVar>& getFunctionLevelOCLBuffers();
			void addAOpenCLNDRangeVar(FuncProtoExt expr);
