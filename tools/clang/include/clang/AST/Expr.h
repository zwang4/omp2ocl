...
namespace clang {
	...
	class FloatingLiteral : public Expr {
		...
		string getLexString() { return LexString; }
	...
	class CallExpr : public Expr {
		...
		SourceLocation RParenLoc;
		unsigned NumPreArgs;
		vector<DeclRefExpr*> expendedVariables;
		CallArgInfoContainer* cac;
		string newName;
		bool bIsRevised;
		int uNumArgs;
		bool isGTP;
		...
		//ZHENG: This is used to add new call arguments 
		void reAllocArgs(ASTContext& C, unsigned int size);
		bool isRevised() { return bIsRevised; }
		void setRevised() { bIsRevised = true; }
		unsigned getOrigNumArg() { return uNumArgs; }
		void setOrigNumArg(int i)
		{
			if (uNumArgs < 0)
			{
				uNumArgs = i;
		...
		//ZHENG: This is used to expand the callarg to pass offset information to global memory variables
		void setRenameInfo(CallArgInfoContainer* c, string newName, bool isGTP)
		{
			this->cac = c;
		...
		//Whether the function arguments has any thread private whose buffer
		//is in __global memory
		bool hasGTPVar() { return isGTP; }
		string getNewName() { return newName; }
		CallArgInfoContainer* getOffsetedArgument() { return cac; }
		bool hasOffsetedArgument() { return (cac != NULL); }

		void addAExpendedVariable(DeclRefExpr* e)
		{
			expendedVariables.push_back(e);
		}

		vector<DeclRefExpr*>& getExpendedVariables()
		{
			return expendedVariables;
		}
	...
	class CopyInBuffer
	{
		public:
			ValueDecl* d;
			bool isTPGlobalBuf;
			CopyInBuffer(ValueDecl* decl, bool isb)
			{
				d = decl;
				this->isTPGlobalBuf = isb;
			}
	};
