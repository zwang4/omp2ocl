...
namespace clang {
	...
	//ZHENG:
	//      This is for function expanded used in OpenCLKernel
	class ExpendedCallArg
	{
		public:
			//The name of the variable that passed in
			string passInName;
			//The variable may 
			string localName; 
			DeclRefExpr* declRef;
			ExpendedCallArg(string passInName, string localName, DeclRefExpr* expr)
			{
				this->passInName = passInName;
...
#include "clang/AST/StmtNodes.inc"
			...
			vector<ExpendedCallArg> expCallArgs;

			void addedExpendedCallArg(ExpendedCallArg& arg)
			{
				for (vector<ExpendedCallArg>::iterator iter = expCallArgs.begin(); iter != expCallArgs.end(); iter++)
				{
					if (iter->passInName == arg.passInName)
						return;
				}



				expCallArgs.push_back(arg);
			}

			vector<ExpendedCallArg>& getExpCallArgs()
			{
				return expCallArgs;
			}
...
unsigned : NumExprBits;
	...
	//OMP2OCL
	class OclInit : public Stmt {
		SourceLocation Loc;
		public:
		OclInit (SourceLocation L) : Stmt(OclInitClass), Loc(L) {} 
		child_range children() { return child_range(); }
		SourceRange getSourceRange() const { return SourceRange(Loc); }
	};

	class OclFlush : public Stmt {
		SourceLocation Loc;
		public:
		OclFlush (SourceLocation L) : Stmt(OclFlushClass), Loc(L) {} 
		child_range children() { return child_range(); }
		SourceRange getSourceRange() const { return SourceRange(Loc); }
	};
	
	class OclStartProfile : public Stmt {
		SourceLocation Loc;
		public:
		OclStartProfile (SourceLocation L) : Stmt(OclStartProfileClass), Loc(L) {} 
		child_range children() { return child_range(); }
		SourceRange getSourceRange() const { return SourceRange(Loc); }
	};
	
	class OclStopProfile : public Stmt {
		SourceLocation Loc;
		public:
		OclStopProfile (SourceLocation L) : Stmt(OclStopProfileClass), Loc(L) {} 
		child_range children() { return child_range(); }
		SourceRange getSourceRange() const { return SourceRange(Loc); }
	};
	
	class OclDumpProfile : public Stmt {
		SourceLocation Loc;
		public:
		OclDumpProfile (SourceLocation L) : Stmt(OclDumpProfileClass), Loc(L) {} 
		child_range children() { return child_range(); }
		SourceRange getSourceRange() const { return SourceRange(Loc); }
	};
	
	class OclDevWrite : public Stmt {
		SourceLocation Loc;
		vector<string> vars;
		public:
		OclDevWrite (SourceLocation L, vector<string> vars) : Stmt(OclDevWriteClass), Loc(L) { this->vars = vars; } 
		child_range children() { return child_range(); }
		SourceRange getSourceRange() const { return SourceRange(Loc); }
		vector<string> getVars() { return vars; }
	};
	
	class OclDevRead : public Stmt {
		SourceLocation Loc;
		vector<string> vars;
		public:
		OclDevRead (SourceLocation L, vector<string> vars) : Stmt(OclDevReadClass), Loc(L) { this->vars = vars; } 
		child_range children() { return child_range(); }
		SourceRange getSourceRange() const { return SourceRange(Loc); }
		vector<string> getVars() { return vars; }
	};
	
	class OclHostFlush : public Stmt {
		SourceLocation Loc;
		vector<string> vars;
		public:
		OclHostFlush (SourceLocation L, vector<string> vars) : Stmt(OclHostFlushClass), Loc(L) { this->vars = vars; } 
		child_range children() { return child_range(); }
		SourceRange getSourceRange() const { return SourceRange(Loc); }
		vector<string> getVars() { return vars; }
	};
	
	class OclHostRead : public Stmt {
		SourceLocation Loc;
		vector<string> vars;
		public:
		OclHostRead (SourceLocation L, vector<string> vars) : Stmt(OclHostReadClass), Loc(L) { this->vars = vars; } 
		child_range children() { return child_range(); }
		SourceRange getSourceRange() const { return SourceRange(Loc); }
		vector<string> getVars() { return vars; }
	};

	class OclSync : public Stmt {
		SourceLocation Loc;
		public:
		OclSync (SourceLocation L) : Stmt(OclSyncClass), Loc(L) {} 
		child_range children() { return child_range(); }
		SourceRange getSourceRange() const { return SourceRange(Loc); }
	};

	class OclResetMLStmt : public Stmt {
		SourceLocation Loc;
		public:
		OclResetMLStmt (SourceLocation L) : Stmt(OclResetMLStmtClass), Loc(L) {} 
		child_range children() { return child_range(); }
		SourceRange getSourceRange() const { return SourceRange(Loc); }
	};

	class OclDisableMLRecordStmt : public Stmt {
		SourceLocation Loc;
		public:
		OclDisableMLRecordStmt (SourceLocation L) : Stmt(OclDisableMLRecordStmtClass), Loc(L) {} 
		child_range children() { return child_range(); }
		SourceRange getSourceRange() const { return SourceRange(Loc); }
	};

	class OclEnableMLRecordStmt : public Stmt {
		SourceLocation Loc;
		public:
		OclEnableMLRecordStmt (SourceLocation L) : Stmt(OclEnableMLRecordStmtClass), Loc(L) {} 
		child_range children() { return child_range(); }
		SourceRange getSourceRange() const { return SourceRange(Loc); }
	};

	class OclTerm : public Stmt {
		SourceLocation Loc;
		public:
		OclTerm (SourceLocation L) : Stmt(OclTermClass), Loc(L) {} 
		child_range children() { return child_range(); }
		SourceRange getSourceRange() const { return SourceRange(Loc); }
	};


	class OclDumpMLFStmt : public Stmt {
		SourceLocation Loc;
		public:
		OclDumpMLFStmt (SourceLocation L) : Stmt(OclDumpMLFStmtClass), Loc(L) {} 
		child_range children() { return child_range(); }
		SourceRange getSourceRange() const { return SourceRange(Loc); }
	};
	...
	class CompoundStmt : public Stmt {
		...
		//THIS IS USE FOR OCL TO VLOAD VARIABLES
		...
		//THIS IS USE FOR OCL TO VLOAD VARIABLES
		void addVLoadInfo(vector<OCLCompoundVLoadDeclareInfo>& C);
		vector<OCLCompoundVLoadDeclareInfo>& getVLoadInfo();
	...
	class OpenCLInputArgu
	{
		public:
			bool isBuffer;
			bool isCopyIn;
			ValueDecl* decl;
			bool isGlobalThMem; //Whether this variable is thread private and its allocated
			//in the __global share memory
			FunctionDecl* D;
			bool isFLevel;

			OpenCLInputArgu(ValueDecl* decl, bool isb, bool isCopyIn, bool isG, FunctionDecl* D);
			string getType();
			string getNameAsString();
			bool isAGlobalThMemVar();
			bool isNeedWriteSync();
			bool isNeedReadSync();
			bool canbeDeclareAsGlobal();
			string getGType();
			bool isFunclLevel() { return isFLevel; }
			FunctionDecl* getFuncDecl() { return D; }
			string getFuncName() { return D->getNameAsString(); }

	};

	//This records whether a vriable has been written
	class OCLRWSet
	{
		public:
			ValueDecl* decl;
			bool isWrite;
			OCLRWSet(ValueDecl *d, bool isW) { 
				decl = d;
				isWrite = isW; 
			}
	...
	//Use to track write/read of a global variables
	//This is for GPU TLS only
	class OpenCLTLSBufferAccess {
		string buf_name;
		string access;
		bool isWrite;
		int times;
		unsigned long long statement_ver;
		vector<BinaryOperator*> bops;
		public:
		OpenCLTLSBufferAccess(vector<BinaryOperator*> bs, string name, string access_seq, unsigned long long statement_v, bool isWrite)
		{
			buf_name = name;
			access = access_seq;
			this->isWrite = isWrite;
			times = 0;
			this->statement_ver = statement_v;
			this->bops = bs;
		}

		bool hasBinaryOps() { return (bops.size() > 0) ? true : false; }
		string getName() { return buf_name; }
		string getAccess() { return access; }
		unsigned long long getStmtVer() { return statement_ver; }
	...
	class ForStmt : public Stmt {
		...
		//Zheng: added to support OpenMP 12/04/2011
		OMPFor ompForObj;
		bool isOMPForLoop;
		vector<OCLGlobalMemVar> globalMemoryVariables;
		vector<OpenCLInputArgu> inputArgs;
		vector<OpenCLNDRangeVar> loopIndex;
		string kernel_name;
		Stmt* openCLKernel;
		vector<OCLRWSet> rwS;
		vector<ValueDecl*> reductions;
		...
		vector<OpenCLTLSBufferAccess> tlsCheckingVec;
		bool has_tls_ops;
		public:
		ForStmt(ASTContext &C, Stmt *Init, Expr *Cond, VarDecl *condVar, Expr *Inc, 
				Stmt *Body, SourceLocation FL, SourceLocation LP, SourceLocation RP, OMPFor* omp_for = NULL);

		//Zheng: added to support OpenMP 12/04/2011
		bool isParallelForLoop() { return isOMPForLoop; }
		OMPFor& getOMPFor() { return ompForObj; }

		vector<OpenCLTLSBufferAccess> getTLSCheckingVec()
		{
			return tlsCheckingVec;
		}

		void setTLSCheckingVec(vector<OpenCLTLSBufferAccess>& v)
		{
			tlsCheckingVec = v;
		}
		...
		//GPU TLS Support	
		bool isInTLSCeheckingVec(string name)
		{
			for (unsigned i=0; i<tlsCheckingVec.size(); i++)
			{
				if (tlsCheckingVec[i].getName() == name)
					return true;
			}
		...
		void setTLSOpsFlag( bool b)
		{
			has_tls_ops = b;
		}

		bool hasTLSOpFlag() { return has_tls_ops; }

		vector<ValueDecl*> getGlobalWriteBufs()
		{
			return _globalWriteBufs;
		}

		void addGlobalWriteBuf(ValueDecl* d)
		{
			string name = d->getName();
			for (unsigned i=0; i<_globalWriteBufs.size(); i++)
			{
				if (_globalWriteBufs[i]->getName() == name)
					return;
			}

			_globalWriteBufs.push_back(d);
		}

		void addGlobalLCWriteBuf(ValueDecl* d)
		{
			string name = d->getName();
			for (unsigned i=0; i<_globalLCWriteBufs.size(); i++)
			{
				if (_globalLCWriteBufs[i]->getName() == name)
					return;
			}

			_globalLCWriteBufs.push_back(d);
		}

		vector<ValueDecl*> getGlobalLCWriteBufs()
		{
			return _globalLCWriteBufs;
		}

		bool isParallelReduction()
		{
			if (!isOMPForLoop) return false;
			return ompForObj.isReductionFor();
		}

		void addReductionObj(OMPReductionObj& obj)
		{
			reductionObjs.push_back(obj);
		}

		void setReductionVariables(vector<ValueDecl*>& reducs)
		{
			reductions = reducs;
		}

		vector<ValueDecl*>& getReductionVariables()
		{
			return reductions;
		}

		ValueDecl* getReductionDecl(string name)
		{
			for (unsigned i=0; i<reductions.size(); i++)
			{
				ValueDecl* d = reductions[i];
				if (d->getNameAsString() == name)
					return d;
			}
		...
		bool hasReductionVariable()
		{
			return (reductions.size() > 0);
		}

		void addAGlobalMemoryVariable(OCLGlobalMemVar v);
		vector<OCLGlobalMemVar>& getGlobalMemoryVariable();
		void addInputArgu(OpenCLInputArgu& a);
		void setLoopIndex(vector<OpenCLNDRangeVar> g);
		void addOpenCLNDRangeVar(OpenCLNDRangeVar& v)
		{
			loopIndex.push_back(v);
		}

		void setRWS(vector<OCLRWSet> r) { rwS = r; }
		//This is used for generateing host code
		bool isWriteSet(string name);

		void setOpenCLKernel(Stmt* s) { openCLKernel = s; }
		Stmt* getOpenCLKernel() { return openCLKernel; }

		vector<OpenCLNDRangeVar>& getLoopIndex() { return loopIndex; }

		vector<OpenCLInputArgu>& getInputArgs() { return inputArgs; }

		void setKernelName(string kernel_name) { this->kernel_name = kernel_name; }

		string getKernelName() { return kernel_name; }
