...
namespace clang{
	class CurrentVFunction {
		public:
			CurrentVFunction() {vloop = 0; D=NULL;}		
			FunctionDecl* D;
			unsigned int vloop;
	};
}
...
namespace clang {
	...
	class ASTContext : public llvm::RefCountedBase<ASTContext> {
		...
		//Zheng: use this to record current visiting function
		CurrentVFunction vf;
		unsigned int GlobalVLoop;
		...
		//Zheng, this is urgly!!!
		void setCurrentVFunction(FunctionDecl* D) {vf.D = D; vf.vloop=0;}
		const FunctionDecl* getCurrentVFunction() {return vf.D;}
		unsigned int getCurrentVFunctionLoopIndex() {return vf.vloop++;}
		unsigned int getGlobalVLoop() {return GlobalVLoop++; }
