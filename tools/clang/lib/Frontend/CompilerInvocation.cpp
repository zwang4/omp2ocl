...
static const char *getActionName(frontend::ActionKind Kind) {
	switch (Kind) {
		...
		case frontend::OMP2OpenCL:             return "-omp2opencl";
...
static InputKind ParseFrontendArgs(FrontendOptions &Opts, ArgList &Args,
	...
	if (const Arg *A = Args.getLastArg(OPT_Action_Group)) {
		switch (A->getOption().getID()) {
			...
			case OPT_ast_omp2ocl:
				Opts.ProgramAction = frontend::OMP2OpenCL; break;
		}
	}
...
#include "clang/Frontend/LangStandards.def"
	...
	if (const Arg *A = Args.getLastArg(OPT_arch))
	{
		if ((strcmp(A->getValue(Args), "nvidia") == 0) ||
				(strcmp(A->getValue(Args), "NVIDIA") == 0)) {
			cout << "Generating code for NVIDIA GPUs" << endl;
			OCLCommon::setArch("NVIDIA");
		}

		if ((strcmp(A->getValue(Args), "intel") == 0) ||
				(strcmp(A->getValue(Args), "Intel") == 0) ||
				(strcmp(A->getValue(Args), "INTEL") == 0)
		   ) 

		{
			cout << "Generating code for the CPU" << endl;
			OCLCommon::setArch("CPU");
		}

		if ((strcmp(A->getValue(Args), "amd") == 0) ||
				(strcmp(A->getValue(Args), "AMD") == 0)) {
			cout << "Generating code for AMD GPUs" << endl;
			OCLCommon::setArch("AMD");
		}
	}

	if (const Arg *A = Args.getLastArg(OPT_parallel_depth))
	{
		int depth = atoi(A->getValue(Args));
		if (!(depth >= 1 && depth <= OCL_MAX_LOOP_LEVEL))
		{
			Diags.Report(diag::err_omp2ocl_parallel_depth)
				<< A->getAsString(Args) << A->getValue(Args);
		}
		else
		{
			OCLCompilerOptions::DefaultParallelDepth = depth;
			OCLCompilerOptions::UserDefParallelDepth = true;
			cout << "Set the parallelism level to " << depth << endl;
		}
	}

	if (const Arg *A = Args.getLastArg(OPT_local_opt))
	{
		if ((strcmp(A->getValue(Args), "off")) == 0)
		{
			OCLCompilerOptions::UseLocalMemory = false;
			cout << "Local memory optimisation is disabled." << endl;
		}
	} 

	if (const Arg *A = Args.getLastArg(OPT_software_cache))
	{
		if ((strcmp(A->getValue(Args), "off")) == 0)
		{
			OCLCompilerOptions::EnableSoftwareCache = false;
			cout << "Software cache optimisation is disabled." << endl;
		}
	} 

	if (const Arg *A = Args.getLastArg(OPT_debug_cg))
	{
		if ((strcmp(A->getValue(Args), "on")) == 0)
		{
			OCLCompilerOptions::EnableDebugCG = true;
			OCLCompilerOptions::GenProfilingFunc=true;
		}
	} 

	if (const Arg *A = Args.getLastArg(OPT_gpu_tls))
	{
		if ((strcmp(A->getValue(Args), "on")) == 0)
		{
			OCLCompilerOptions::EnableGPUTLs = true;
		}
	} 
	
	if (const Arg *A = Args.getLastArg(OPT_ocl_tls))
	{
		if ((strcmp(A->getValue(Args), "on")) == 0)
		{
			OCLCompilerOptions::OclTLSMechanism = true;
			OCLCompilerOptions::EnableGPUTLs = true;
		}
	} 
	
	if (const Arg *A = Args.getLastArg(OPT_strict_tls_check))
	{
		if ((strcmp(A->getValue(Args), "off")) == 0)
		{
			OCLCompilerOptions::StrictTLSChecking = false;
		}
	} 

	if (const Arg *A = Args.getLastArg(OPT_lit))
	{
		if ((strcmp(A->getValue(Args), "off")) == 0)
		{
			OCLCompilerOptions::EnableLoopInterchange = false;
			cout << "Loop interchange is disabled." << endl;
		}
	} 

	if (const Arg *A = Args.getLastArg(OPT_mlf))
	{
		if ((strcmp(A->getValue(Args), "on")) == 0)
		{
			OCLCompilerOptions::EnableMLFeatureCollection = true;
			cout << "Generating code for collecting ML features" << endl;
		}
	} 
