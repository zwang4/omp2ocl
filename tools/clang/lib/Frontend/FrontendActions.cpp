...
ASTConsumer *OMP2OpenCLAction::CreateASTConsumer(CompilerInstance &CI,
                                               llvm::StringRef InFile) {
  //FIXED ME: ZHENG: Memory leak here
  if (CI.createDefaultOutputFile(false, InFile))
    return CreateASTPrinter(&llvm::nulls());
  return 0;
}
