...
//ZHENG: OPENCL ROUTINES

void ForStmt::addAGlobalMemoryVariable(OCLGlobalMemVar v)
{
	globalMemoryVariables.push_back(v);
}

vector<OCLGlobalMemVar>& ForStmt::getGlobalMemoryVariable()
{
	return globalMemoryVariables;
}

void ForStmt::addInputArgu(OpenCLInputArgu& a)
{
	inputArgs.push_back(a);
}

void ForStmt::setLoopIndex(vector<OpenCLNDRangeVar> g)
{
	loopIndex = g;
}

bool ForStmt::isWriteSet(string name)
{
	for (vector<OCLRWSet>::iterator iter = rwS.begin(); iter != rwS.end(); iter++)
	{
		if (name == iter->decl->getNameAsString())
		{
			return iter->isWrite; 
		}
	}
...
ForStmt::ForStmt(ASTContext &C, Stmt *Init, Expr *Cond, VarDecl *condVar, 
		...
		SourceLocation RP, OMPFor* omp_for)
...
void CompoundStmt::addVLoadInfo(vector<OCLCompoundVLoadDeclareInfo>& C)
{
	CVIs = C;
}

vector<OCLCompoundVLoadDeclareInfo>& CompoundStmt::getVLoadInfo()
{
	return CVIs;
}
