...
//ZHENG: This will revise the Arguments so that we
//can add new called arguments to it
void CallExpr::reAllocArgs(ASTContext &C, unsigned int newSize)
{
	assert((newSize >= NumArgs) && "new arg size should >= NumArgs");
	unsigned NumPreArgs = getNumPreArgs();

	if (newSize > NumArgs)
	{
		Stmt** newSubExpr = new (C) Stmt*[newSize+PREARGS_START+NumPreArgs];
		//Copy the old content
		memcpy(newSubExpr, SubExprs, sizeof(Stmt*) * (NumArgs + PREARGS_START+NumPreArgs));
		NumArgs = newSize;
		delete SubExprs;
		SubExprs = newSubExpr;
	}
