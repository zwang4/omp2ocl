...
vector<FunctionDecl*> DeclPrinter::functions;
vector<EnumDecl*> DeclPrinter::enumDecls;
vector<TypedefDecl*> DeclPrinter::typeDefs;
vector<RecordDecl*> DeclPrinter::recordDecls;

bool DeclPrinter::isEnableFunctionTrack;

// ADDED BY ZHENG
//*This will check whether a decl is a function
//If the decl is a function, then it will be added into the function vecotr (if it has been added into before)
void DeclPrinter::addFunction(FunctionDecl* decl)
{
	OpenCLGlobalInfoContainer::addRecordFunc(decl);
}
...
void DeclPrinter::VisitVarDecl(VarDecl *D) {
	if (isPrototypePrint && D->isAGlobalBufDecl())
	{
		Out << "__global ";
		Out << getGlobalType(D->getType().getAsString()) << " * "; 
		Out << D->getNameAsString();
		return;
	}
