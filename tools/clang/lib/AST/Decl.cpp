...
void FunctionDecl::setParams(ASTContext &C,
	...
	//ZHENG: I comment here because this maybe modified by an OpenCL
	//kernel that accesses to global static variables 
	...
	if (NumParams) {
		...
		{
			origParamInfo = ParamInfo;
			isFuncProtoChanged = true; 
		}
...
ValueDecl* FuncProtoExt::getDecl()
{
	return expr->getDecl();
}

FuncProtoExt::FuncProtoExt(DeclRefExpr* e, unsigned ei, bool hasRevised, bool isGT, bool isLocalVar)
{
	this->expr = e;
	extended_index = ei;
	this->revised = hasRevised;
	this->isGlobalThreadPrivate = isGT;
	this->isLocalVar = isLocalVar;
}

vector<FuncProtoExt>& FunctionDecl::getAddedOpenCLNDRangeVars()
{
	return addedOpenCLNDRangeVars;
}

void FunctionDecl::addAOpenCLNDRangeVar(FuncProtoExt expr)
{
	addedOpenCLNDRangeVars.push_back(expr);
}

vector<OCLGlobalMemVar>& FunctionDecl::getFunctionLevelOCLBuffers()
{
	return FLevelOCLBufs;
}

void FunctionDecl::addFunctionLevelOCLBuffer(OCLGlobalMemVar o)
{
	ValueDecl* d = o.getDecl();
	string name = d->getNameAsString();
	for (unsigned i=0; i<FLevelOCLBufs.size(); i++)
	{
		if (name == FLevelOCLBufs[i].getNameAsString())
		{
			return;
		}
	}

	FLevelOCLBufs.push_back(o);
}	
