...
//Parse Omp2ocl Pragmas
void Parser::ParseOmp2OclObj(StmtVector& Stmts)
{
	do 
	{
		Omp2OclObj* p = getPreprocessor().topOmp2OclObj();
		unsigned lc = 0;
		...
		if (p)
		{
			...
			unsigned l = PLoc.getLine();

			//FIXME: This is use to by pass
			//the problem that two omp2ocl pragmas are on the two adjanct 
			//lines
			if (lc > 0 && (lc + 1) != l)
			{
				break;
			...
			lc = l;
			StmtResult R = ParseOmp2OclObj(p);
			if (R.isUsable())
			{
				Stmts.push_back(R.release());
			}

			getPreprocessor().popOmp2OclObj();
...
Parser::ParseStatementOrDeclaration(StmtVector &Stmts, bool OnlyStatement) {
	...
	//Zheng, support for OMP 
	OMPFor* hfor =  getPreprocessor().hasMetOMPFor();
	if (hfor != NULL)
	{
		if (Kind != tok::kw_for)
		{
			Diag(Tok, diag::err_expected_for);
			//remove omp parallel for obj
			getPreprocessor().PopOMPFor();
		}
	}

	ParseOmp2OclObj(Stmts);
		...
		case tok::kw_for:                 // C99 6.8.5.3: for-statement
						{
							if (hfor)
							{
								OMPFor obj = getPreprocessor().PopOMPFor();
								return ParseForStatement(attrs, obj);
							}
...
///
StmtResult Parser::ParseForStatement(ParsedAttributes &attrs, OMPFor& ompForObj) {
	return ParseForStatement(attrs, &ompForObj);
}

StmtResult Parser::ParseForStatement(ParsedAttributes &attrs, OMPFor *pomp) {
	// FIXME: Use attributes?
	...
	if (!ForEach)
		return Actions.ActOnForStmt(ForLoc, LParenLoc, FirstPart.take(), SecondPart,
				SecondVar, ThirdPart, RParenLoc, Body.take(), pomp);
...
//Omp2Ocl
StmtResult Parser::ParseOmp2OclObj(Omp2OclObj* p)
{
	SourceLocation Loc = p->getLocation();
	switch(p->getType())
	{
		case omp2ocl_init: 
			{
				return Actions.ActOclInit(Loc);
				break;
			}
		case omp2ocl_sync:
			{
				return Actions.ActOclSync(Loc);
				break;
			}
		case omp2ocl_flush:
			{
				return Actions.ActOclFlush(Loc);
				break;
			}
		case omp2ocl_stopprofile:
			{
				return Actions.ActOclStopProfile(Loc);
				break;
			}
		case omp2ocl_startprofile:
			{
				return Actions.ActOclStartProfile(Loc);
				break;
			}
		case omp2ocl_dumpprofile:
			{
				return Actions.ActOclDumpProfile(Loc);
				break;
			}
		case omp2ocl_hostread:
			{
				Omp2OclHostRead* hf = (Omp2OclHostRead*) p;
				return Actions.ActOclHostRead(Loc, hf->getVars());
				break;
			}
		case omp2ocl_hostflush:
			{
				Omp2OclHostFlush* hf = (Omp2OclHostFlush*) p;
				return Actions.ActOclHostFlush(Loc, hf->getVars());
				break;
			}
		case omp2ocl_devread:
			{
				Omp2OclDevRead* hf = (Omp2OclDevRead*) p;
				return Actions.ActOclDevRead(Loc, hf->getVars());
				break;
			}
		case omp2ocl_devwrite:
			{
				Omp2OclDevWrite* hf = (Omp2OclDevWrite*) p;
				return Actions.ActOclDevWrite(Loc, hf->getVars());
				break;
			}
		case omp2ocl_term:
			{
				return Actions.ActOclTerm(Loc);
				break;
			}
		case omp2ocl_resetmlf:
			{
				return Actions.ActOclResetMLF(Loc);
				break;
			}
		case omp2ocl_disablemlfrecord:
			{
				return Actions.ActOclDisableMLFRecord(Loc);
				break;
			}
		case omp2ocl_enablemlfrecord:
			{
				return Actions.ActOclEnableMLFRecord(Loc);
				break;
			}
		case omp2ocl_dumpmlf:
			{
				return Actions.ActOclDumpMLF(Loc);
				break;
			}
		default:
			{
				cerr << endl << "ERROR: unknown Omp2OclObjType: Parser::ParseOmp2OclObj" << endl;
				StmtResult R;
				exit(-1);
				return Actions.ActOclTerm(Loc);
			}
