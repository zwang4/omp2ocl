...
void myReplace(std::string& str, const std::string& pattern, const std::string& newStr)
{
	size_t pos = 0;
...
//Zheng: Create the main file id and store the main file name
FileID SourceManager::createMainFileID(const FileEntry *SourceFile,
		SourceLocation IncludePos,
		SrcMgr::CharacteristicKind FileCharacter,
		unsigned PreallocatedID,
		unsigned Offset) {
	...
	mainFileName = SourceFile->getName();

	if (mainFileName.find(".cpp") != std::string::npos)
	{
		myReplace(mainFileName, ".cpp", ".cl");	
	}
	else 
		if (mainFileName.find(".c") != std::string::npos)
		{
			myReplace(mainFileName, ".c", ".cl");	
		}
		else
		{
			mainFileName = mainFileName + ".cl";
		}
