...
c-------------------------------------------------------------------*/
#pragma omp parallel for private(m, i, j, k) schedule(static)
  for (m = 0; m < 5; m++) {
...
c-------------------------------------------------------------------*/
  for (n = 0; n < 3; n++) {
////#pragma omp for    
#pragma omp parallel for private(i, j, k) notls
    for (i = 0; i < grid_points[0]; i++) {
      for (j = 0; j < grid_points[1]; j++) {
	for (k = 0; k < grid_points[2]; k++) {
	  lhs[5*n+2][i][j][k] = 1.0;
...
#pragma omp parallel for private(i) schedule(static) notls
      for (i = 1; i <= grid_points[0]-2; i++) {
	lhs[0][i][j][k] =   0.0;
...
////#pragma omp for nowait
  //trace_start("lhsx", 3);
#pragma omp parallel for private(j, k) schedule(static) notls
  for (j = 1; j <= grid_points[1]-2; j++) {
    for (k = 1; k <= grid_points[2]-2; k++) {
...
////#pragma omp for nowait
  ////trace_start("lhsx", 4);
#pragma omp parallel for private(i, j, k) schedule(static) notls
  for (i = 3; i <= grid_points[0]-4; i++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
...
c-------------------------------------------------------------------*/
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (k = 1; k <= grid_points[2]-2; k++) {
  ////trace_start("lhsy", 1);
#pragma omp parallel for schedule(static) notls
      for (j = 0; j <= grid_points[1]-1; j++) {
	ru1 = c3c4*rho_i[i][j][k];
	cv[j] = vs[i][j][k];
  ...
  ////trace_start("lhsy", 2);
#pragma omp parallel for private(j) schedule(static) notls
      for (j = 1; j <= grid_points[1]-2; j++) {
	lhs[0][i][j][k] =  0.0;
...
////#pragma omp for nowait
  //trace_start("lhsy", 3);
#pragma omp parallel for private(i, k) schedule(static) notls
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (k = 1; k <= grid_points[2]-2; k++) {
...
c-------------------------------------------------------------------*/
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
  ////trace_start("lhsz", 1);
#pragma omp parallel for private(ru1) schedule(static) notls
      for (k = 0; k <= grid_points[2]-1; k++) {
	ru1 = c3c4*rho_i[i][j][k];
	cv[k] = ws[i][j][k];
...
#pragma omp parallel for schedule(static) notls
  ...
  ////trace_start("lhsz", 5);
#pragma omp parallel for private(i, j) schedule(static) notls
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
      lhs[0][i][j][k] = lhs[0][i][j][k] + comz1;
      lhs[1][i][j][k] = lhs[1][i][j][k] - comz4;
...
////#pragma omp for
 ////trace_start("pinvr", 1);
#pragma omp parallel for private(i, j, k, t1, t2, r1, r2, r3, r4, r5) schedule(static) notls
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
...
#pragma omp parallel for private(i, j, k, aux, rho_inv) schedule(static) notls
  for (i = 0; i <= grid_points[0]-1; i++) {
    for (j = 0; j <= grid_points[1]-1; j++) {
      for (k = 0; k <= grid_points[2]-1; k++) {
...
#pragma omp parallel for private(i, j, k,m) schedule(static) num_threads(5) notls
  for (m = 0; m < 5; m++) {
    for (i = 0; i <= grid_points[0]-1; i++) {
      for (j = 0; j <= grid_points[1]-1; j++) {
	for (k = 0; k <= grid_points[2]-1; k++) {
...
#pragma omp parallel for private(uijk, up1, um1) schedule(static) notls
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
	uijk = us[i][j][k];
...
c-------------------------------------------------------------------*/
 ...
 ////trace_start("compute_rhs", 4);
  for (m = 0; m < 5; m++) {
#pragma omp parallel for schedule(static) num_threads(5) notls
    for (j = 1; j <= grid_points[1]-2; j++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
...
#pragma omp parallel for private(j, k, m) schedule(static) num_threads(5) notls
  for (m = 0; m < 5; m++) {
...
#pragma omp parallel for private(i, j, k, vijk, vp1, vm1) schedule(static) notls
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
	vijk = vs[i][j][k];
...
c-------------------------------------------------------------------*/
  ...
  j = 1;
#pragma omp paralleli for schedule(static) num_threads(5) notls
  for (m = 0; m < 5; m++) {
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
	rhs[m][i][j][k] = rhs[m][i][j][k]- dssp * 
...
#pragma omp parallel for num_threads(5) notls
  for (m = 0; m < 5; m++) {
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
	rhs[m][i][j][k] = rhs[m][i][j][k] - dssp * 
...
#pragma omp parallel for private(wijk, wp1, wm1) schedule(static) notls
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
	wijk = ws[i][j][k];
...
#pragma omp parallel for private(i, j, k, ru1, uu, vv, ww, ac, ac2inv, r1, r2, r3, r4, r5, t1, t2, t3) schedule(static) notls
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
...
#pragma omp parallel for private(i, j, k, xvel, yvel, zvel, ac, acinv, ac2u, r1, r2, r3, r4, r5, uzik1, btuz, t1, t2, t3) schedule(static) notls
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
...
--------------------------------------------------------------------*/
  ...
  for (i = 0; i <= grid_points[0]-3; i++) {
    i1 = i  + 1;
    i2 = i  + 2;
    //trace_start("x_solve", 1);
#pragma omp parallel for private(j, k, m, fac1) schedule(static) notls
    for (j = 1; j <= grid_points[1]-2; j++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
...
#pragma omp parallel for private(j, k, fac1) schedule(static) notls
    for (j = 1; j <= grid_points[1]-2; j++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
	fac1               = 1./lhs[n+2][i][j][k];
	lhs[n+3][i][j][k]   = fac1*lhs[n+3][i][j][k];
...
#pragma omp parallel for private(j, k, m) schedule(static) notls
  for (m = 0; m < 3; m++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
...
#pragma omp parallel for private(m, j, k) schedule(static) notls
    for (m = 0; m < 3; m++) {
      for (j = 1; j <= grid_points[1]-2; j++) {
	for (k = 1; k <= grid_points[2]-2; k++) {
	  rhs[m][i][j][k] = rhs[m][i][j][k] - 
...
#pragma omp parallel for private(i, k, m, fac1) schedule(static)
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
	fac1               = 1./lhs[n+2][i][j][k];
	lhs[n+3][i][j][k]   = fac1*lhs[n+3][i][j][k];
...
#pragma omp parallel for private(i, k, m, fac1) schedule(static) notls
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (k = 1; k <= grid_points[2]-2; k++) {
      fac1               = 1./lhs[n+2][i][j][k];
      lhs[n+3][i][j][k]   = fac1*lhs[n+3][i][j][k];
...
#pragma omp parallel for private(i, k, fac1) schedule(static) notls
      for (i = 1; i <= grid_points[0]-2; i++) {
	for (k = 1; k <= grid_points[2]-2; k++) {
	  fac1               = 1./lhs[n+2][i][j][k];
	  lhs[n+3][i][j][k]   = fac1*lhs[n+3][i][j][k];
...
#pragma omp parallel for private(i, k,m) schedule(static) notls
  for (m = 0; m < 3; m++) {
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
	rhs[m][i][j][k] = rhs[m][i][j][k] -
...
////#pragma omp for
    //trace_start("z_solve", 1);
#pragma omp parallel for private(k1, k2, fac1, m) schedule(static) parallel_depth(2) notls
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
...
#pragma omp parallel for private(m, fac1) schedule(static) notls
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
      fac1               = 1./lhs[n+2][i][j][k];
      lhs[n+3][i][j][k]   = fac1*lhs[n+3][i][j][k];
...
#pragma omp parallel for private(k1, k2, fac1, n) schedule(static) notls
  for (m = 3; m < 5; m++) {
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (j = 1; j <= grid_points[1]-2; j++) {
	for (k = 0; k <= grid_points[2]-3; k++) {
...
c-------------------------------------------------------------------*/
    k  = grid_points[2]-2;
    k1 = grid_points[2]-1;
#pragma omp parallel for private(i, j, fac1, fac2) schedule(static) notls
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (j = 1; j <= grid_points[1]-2; j++) {
	fac1               = 1./lhs[n+2][i][j][k];
	lhs[n+3][i][j][k]   = fac1*lhs[n+3][i][j][k];
...
#pragma omp parallel for private(i, j, m) schedule(static) notls
  for (m = 0; m < 3; m++) {
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (j = 1; j <= grid_points[1]-2; j++) {
	rhs[m][i][j][k] = rhs[m][i][j][k] -
...
#pragma omp parallel for private(i, j, m,n ) schedule(static)
  for (m = 3; m < 5; m++) {
    n = (m-3+1)*5;
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (j = 1; j <= grid_points[1]-2; j++) {
...
#pragma omp parallel for schedule(static) parallel_depth(2) notls
  for (m = 0; m < 3; m++) {
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (j = 1; j <= grid_points[1]-2; j++) {
...
#pragma omp parallel for private(n) schedule(static) parallel_depth(2) notls
  for (m = 3; m < 5; m++) {
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (j = 1; j <= grid_points[1]-2; j++) {
	for (k = grid_points[2]-3; k >= 0; k--) {
