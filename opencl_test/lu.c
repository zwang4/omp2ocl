...
--------------------------------------------------------------------*/
  ...
  int nthreads = 1;
	...
	//memset(flag, 0, (ISIZ1/2*2+1) * sizeof(long long));
...
#pragma omp for nowait private(tmp, tmp1, tmat) schedule(static)//209
  for (i = ist; i <= iend; i++) {
    for (j = jst; j <= jend; j++) {
      for (m = 0; m < 5; m++) {
	v[i][j][k][m] = v[i][j][k][m]
...
#pragma omp for nowait private(tmp, tmp1, tmat) schedule(static) //223
  for (i = ist; i <= iend; i++) {
...
#pragma omp for nowait schedule(static) private(tmp, tmp1, tmat)//447
  //for (i = iend; i >= ist; i--) {
    for (i=ist; i<=iend; i++) {
    //for (j = jend; j >= jst; j--) {
    for (j = jst; j <= jend; j++) {
...
#pragma omp for nowait schedule(static) private(tmp, tmp1, tmat)//461
  //for (i = iend; i >= ist; i--) {
  for (i = ist; i <= iend; i++) {
#if defined(_OPENMP)      
    if (i != iend) {
...
#pragma omp parallel for private(i, j, k, m) //731
  for (i = 0; i < nx; i++) {
    for (j = 0; j < ny; j++) {
      for (k = 0; k < nz; k++) {
	for (m = 0; m < 5; m++) {
...
#pragma omp parallel for private(iglob, xi, jglob, eta, zeta, i, j, k, m) //742
  for (i = 0; i < nx; i++) {
    iglob = i;
...
#pragma omp parallel for private(u21, q, i, j, k) //776
  for (i = L1; i <= L2; i++) {
    for (j = jst; j <= jend; j++) {
      for (k = 1; k < nz - 1; k++) {
...
#pragma omp parallel for private(j, k, i, m, tmp, u21i, u31i, u41i, u51i, u21im1, u31im1, u41im1, u51im1, iend1, ist1) //796
  for (j = jst; j <= jend; j++) {
    for (k = 1; k <= nz - 2; k++) {
      for (i = ist; i <= iend; i++) {
	for (m = 0; m < 5; m++) {
...
#pragma omp parallel for private(i, j, k, u31, q) //908
  for (i = ist; i <= iend; i++) {
    for (j = L1; j <= L2; j++) {
      for (k = 1; k <= nz - 2; k++) {
...
#pragma omp parallel for private(i, k, j, m, tmp, u21j, u31j, u41j, u51j, u21jm1, u31jm1, u41jm1, u51jm1, jst1, jend1) //927
  for (i = ist; i <= iend; i++) {
    for (k = 1; k <= nz - 2; k++) {
      for (j = jst; j <= jend; j++) {
	for (m = 0; m < 5; m++) {
...
--------------------------------------------------------------------*/
#pragma omp parallel for private(i, k, j, m, u41, q, tmp, u21k, u31k, u41k, u51k, u21km1, u31km1, u41km1, u51km1) //1037
  for (i = ist; i <= iend; i++) {
    for (j = jst; j <= jend; j++) {
      for (k = 0; k <= nz-1; k++) {
...
////#pragma omp for nowait schedule(static)
#pragma omp for nowait schedule(static) private(tmp1, tmp2, tmp3)//1257
  for (i = ist; i <= iend; i++) {
    for (j = jst; j <= jend; j++) {
...
#pragma omp for nowait schedule(static) private(tmp1, tmp2, tmp3)
  for (i = iend; i >= ist; i--) {
      for (j = jend; j >= jst; j--) {
#else
#pragma omp for nowait schedule(static) private(tmp1, tmp2, tmp3) //1629
  for (i = ist; i <= iend; i++) {
		for (j = jst; j <= jend; j++) {
#endif
#if 0
...
#pragma omp parallel for reduction(+: sum0, sum1, sum2, sum3, sum4) //2003
  for (i = ist; i <= iend; i++) {
    for (j = jst; j <= jend; j++) {
      for (k = 1; k <= nz0-2; k++) {
...
#pragma omp parallel for private(i, j, k, u21, q) //2364
  for (i = L1; i <= L2; i++) {
    for (j = jst; j <= jend; j++) {
      for (k = 1; k <= nz - 2; k++) {
...
#pragma omp parallel for private(j, k, i, m, L2, tmp, u21i, u31i, u41i, u51i, u21im1, u31im1, u41im1, u51im1, iend1, ist1) //2385
  for (j = jst; j <= jend; j++) {
    for (k = 1; k <= nz - 2; k++) {
      for (i = ist; i <= iend; i++) {
	for (m = 0; m < 5; m++) {
...
////#pragma omp for  
#pragma omp parallel for private(i, j, k, u31, q) //2501
  for (i = ist; i <= iend; i++) {
    for (j = L1; j <= L2; j++) {
      for (k = 1; k <= nz - 2; k++) {
...
////#pragma omp for  
#pragma omp parallel for private(j, k, i, m, L2, tmp, u21j, u31j, u41j, u51j, u21jm1,u31jm1, u41jm1, u51jm1, jend1, jst1) //2521
  for (i = ist; i <= iend; i++) {
    for (k = 1; k <= nz - 2; k++) {
      for (j = jst; j <= jend; j++) {
	for (m = 0; m < 5; m++) {
...
--------------------------------------------------------------------*/
////#pragma omp for  
#pragma omp parallel for private(i, j, k, m, u41, q, u21k, u31k, u41k, u51k, tmp, u21km1, u31km1, u41km1, u51km1)//2635
  for (i = ist; i <= iend; i++) {
    for (j = jst; j <= jend; j++) {
      for (k = 0; k <= nz-1; k++) {
...
--------------------------------------------------------------------*/
////#pragma omp for
#pragma omp parallel for private(i, j, tmp) //2772
  for (i = 0; i < nx; i++) {
    for (j = 0; j < ny; j++) {
      //exact( i, j, 0, &u[i][j][0][0] );
      exact( i, j, 0, tmp );
      u[i][j][0][0]=tmp[0];
      u[i][j][0][1]=tmp[1];
      u[i][j][0][2]=tmp[2];
...
////#pragma omp for
#pragma omp parallel for private(j, jglob, zeta, eta, xi, iglob, pxi, peta, pzeta, k, i, ue_1jk, ue_nx0jk, ue_i1k, ue_iny0k, ue_ij1, ue_ijnz)//2987
  for (j = 0; j < ny; j++) {
    jglob = j;
    for (k = 1; k < nz - 1; k++) {
      zeta = ((double)k) / (nz-1);
...
{
////#pragma omp for    
#pragma omp parallel for private(i, j, k, m) //3052
  for (i = 0; i < ISIZ1; i++) {
    for (j = 0; j < ISIZ2; j++) {
      for (k = 0; k < 5; k++) {
	for (m = 0; m < 5; m++) {
...
--------------------------------------------------------------------*/
#pragma omp parallel for private(i, j, k, m) schedule(static)//3098
    for (i = ist; i <= iend; i++) {
      for (j = jst; j <= jend; j++) {
	for (k = 1; k <= nz - 2; k++) {
	  for (m = 0; m < 5; m++) {
...
////#pragma omp for
#pragma omp parallel for private(i, j, k, m) schedule(static)//3150
    for (i = ist; i <= iend; i++) {
      for (j = jst; j <= jend; j++) {
	for (k = 1; k <= nz-2; k++) {
	  for (m = 0; m < 5; m++) {
