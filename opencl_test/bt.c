...
#pragma omp parallel for private(i, j, k, m) schedule(static)
	...
	/*--------------------------------------------------------------------
	  ...
	  c-------------------------------------------------------------------*/
#pragma omp parallel for private(dtemp, xi, eta, zeta, dtpp, m, i, j, k, ip1, im1, jp1, jm1, km1, kp1) schedule(static)
	for (j = 1; j < grid_points[1]-1; j++) {
		eta = (double)j * dnym1;
...
#pragma omp parallel for private(i, j, k, m, ix, iy, iz, xi, eta, zeta, Pface, Pxi, Peta, Pzeta, temp) schedule(static)
	for (i = 0; i < grid_points[0]; i++) {
		xi = (double)i * dnxm1;
...
#pragma omp parallel for private(j, k, m, eta, zeta, temp) schedule(static)
	for (j = 0; j < grid_points[1]; j++) {
		eta = (double)j * dnym1;
		for (k = 0; k < grid_points[2]; k++) {
			zeta = (double)k * dnzm1;
...
#pragma omp parallel for private(i, k, m, xi, zeta, temp) schedule(static)
	for (i = 0; i < grid_points[0]; i++) {
		xi = (double)i * dnxm1;
		for (k = 0; k < grid_points[2]; k++) {
			zeta = (double)k * dnzm1;
...
#pragma omp parallel for  private(i, j, m, xi, eta, temp) schedule(static)
	for (i = 0; i < grid_points[0]; i++) {
		xi = (double)i *dnxm1;
		for (j = 0; j < grid_points[1]; j++) {
			eta = (double)j * dnym1;
...
#pragma omp parallel for private(i, j, k, m, n) schedule(static)
	for (i = 0; i < grid_points[0]; i++) {
		for (j = 0; j < grid_points[1]; j++) {
			for (k = 0; k < grid_points[2]; k++) {
				for (m = 0; m < 5; m++) {
...
static void lhsx(void) {
	...
	/*--------------------------------------------------------------------
	  ...
	  c-------------------------------------------------------------------*/
	//trace_start("lhsx", 1);  
#pragma omp parallel for private(j, k, i) schedule(dynamic)
	for (j = 1; j < grid_points[1]-1; j++) {
		for (k = 1; k < grid_points[2]-1; k++) {
			for (i = 0; i < grid_points[0]; i++) {
...
#pragma omp parallel for  private(i, j, k, rho_inv) schedule(static)
	for (i = 0; i < grid_points[0]; i++) {
		for (j = 0; j < grid_points[1]; j++) {
			for (k = 0; k < grid_points[2]; k++) {
...
#pragma omp parallel for private(i, j, k, uijk, up1, um1) schedule(static,1)
	for (i = 1; i < grid_points[0]-1; i++) {
		for (j = 1; j < grid_points[1]-1; j++) {
			for (k = 1; k < grid_points[2]-1; k++) {
				uijk = us[i][j][k];
...
#pragma omp parallel for private(j, k, m) schedule(static,1)
	for (j = 1; j < grid_points[1]-1; j++) {
		for (k = 1; k < grid_points[2]-1; k++) {
			for (m = 0; m < 5; m++) {
...
#pragma omp parallel for private(k,m) schedule(static) 
	for (j = 1; j < grid_points[1]-1; j++) {
		for (k = 1; k < grid_points[2]-1; k++) {
			for (m = 0; m < 5; m++) {
				rhs[i][j][k][m] = rhs[i][j][k][m] - dssp *
...
#pragma omp parallel for private(vijk, vp1, vm1, i, j, k) schedule(static)
	for (i = 1; i < grid_points[0]-1; i++) {
		for (j = 1; j < grid_points[1]-1; j++) {
			for (k = 1; k < grid_points[2]-1; k++) {
				vijk = vs[i][j][k];
...
#pragma omp parallel for private(i, k, m) schedule(static,1)
	for (i = 1; i < grid_points[0]-1; i++) {
		for (k = 1; k < grid_points[2]-1; k++) {
			for (m = 0; m < 5; m++) {
				rhs[i][j][k][m] = rhs[i][j][k][m]- dssp * 
...
#pragma omp parallel for private(j, k, m, i) schedule(static)
	for (j = 1; j < grid_points[1]-1; j++) {
		for (k = 1; k < grid_points[2]-1; k++) {
			for (m = 0; m < 5; m++) {
				for (i = 1; i < grid_points[0]-1; i++) {
...
#pragma omp parallel for private(j, k, m, n) schedule(static)
		for (j = 1; j < grid_points[1]-1; j++) {
			for (k = 1; k < grid_points[2]-1; k++) {
...
static void y_backsubstitute(void) {
	...
	for (j = grid_points[1]-2; j >= 0; j--) {
		//trace_start("y_backsubstitute", 1);
#pragma omp parallel for private(i, k, m, n) schedule(static)
		for (i = 1; i < grid_points[0]-1; i++) {
			for (k = 1; k < grid_points[2]-1; k++) {
				for (m = 0; m < BLOCK_SIZE; m++) {
					for (n = 0; n < BLOCK_SIZE; n++) {
...
static void z_backsubstitute(void) {
	...
	//trace_start("z_backsubstitute", 1);
#pragma omp parallel for private(i, j, k, m, n) schedule(static) parallel_depth(1)
	for (i = 1; i < grid_points[0]-1; i++) {
		for (j = 1; j < grid_points[1]-1; j++) {
