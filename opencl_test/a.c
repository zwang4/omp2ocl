//-------------------------------------------------------------------------------
//OpenCL Kernels 
//Generated at : Mon Jun  4 14:19:05 2012
//Compiler options: 
//      Software Cache  true
//      Local Memory    true
...
#define vload2(b,a) {a.x=a.y=0;}
#define vload(b,a)  {a.x=a.y=a.z=a.w=0;}
*/
...
__kernel void TLS_Checking_9D(unsigned dim0, unsigned dim1, unsigned dim2,
			      unsigned dim3, unsigned dim4, unsigned dim5,
			      unsigned dim6, unsigned dim7, unsigned dim8,
			      __global int *rd_log, __global int *wr_log,
			      __global int *conflict_flag)
{
	int wr, rd, index;
...
__kernel void TLS_Checking_10D(unsigned dim0, unsigned dim1, unsigned dim2,
			       unsigned dim3, unsigned dim4, unsigned dim5,
			       unsigned dim6, unsigned dim7, unsigned dim8,
			       unsigned dim9, __global int *rd_log,
			       __global int *wr_log,
			       __global int *conflict_flag)
{
	int wr, rd, index;
...
//-------------------------------------------------------------------------------
__kernel void add_0(__global double *g_u, __global int *g_rd_log_u,
		    __global int *g_wr_log_u, __global double *g_rhs,
		    __global int *grid_points, __global int *rd_log_grid_points,
		    __global int *wr_log_grid_points, int __ocl_k_bound,
		    int __ocl_j_bound)
{
	//-------------------------------------------
	...
	__global int (*rd_log_u)[25][25][5] =
	    (__global int (*)[25][25][5])g_rd_log_u;
	__global int (*wr_log_u)[25][25][5] =
	    (__global int (*)[25][25][5])g_wr_log_u;
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void exact_rhs_0(__global double *g_forcing,
			  __global int *g_rd_log_forcing,
			  __global int *g_wr_log_forcing,
			  __global int *grid_points,
			  __global int *rd_log_grid_points,
			  __global int *wr_log_grid_points, int __ocl_k_bound,
			  int __ocl_j_bound)
{
	//-------------------------------------------
	...
	//TLS Checking Buffers (BEGIN)
	__global int (*rd_log_forcing)[25][25][6] =
	    (__global int (*)[25][25][6])g_rd_log_forcing;
	__global int (*wr_log_forcing)[25][25][6] =
	    (__global int (*)[25][25][6])g_wr_log_forcing;
	//TLS Checking Buffers (END)
...
__kernel void exact_rhs_1(double dnym1, __global int *grid_points, double dnzm1,
			  double dnxm1, __global double *g_forcing,
			  __global int *g_rd_log_forcing,
			  __global int *g_wr_log_forcing, double tx2,
			  ...
			  double dssp, __global double *g_ce,
			  __global int *g_rd_log_ce, __global int *g_wr_log_ce,
			  int __ocl_j_bound)
{
	//-------------------------------------------
	//OpenCL global indexes (BEGIN)
	...
	__global int (*wr_log_forcing)[25][25][6] =
	    (__global int (*)[25][25][6])g_wr_log_forcing;
	__global int (*rd_log_ce)[13] = (__global int (*)[13])g_rd_log_ce;
	__global int (*wr_log_ce)[13] = (__global int (*)[13])g_wr_log_ce;
	//TLS Checking Buffers (END)
	...
	//-------------------------------------------
	{
		...
		for (k = 1; k < grid_points[2] - 1; k++) {
			...
			for (i = 1; i < grid_points[0] - 1; i++) {
				...
				// GPU TLS logs (BEGIN) 
				//-------------------------------------------
				atom_inc(wr_log_forcing[i][j][k][0]);
				rd_log_forcing[i][j][k][0] = 1;
				atom_inc(wr_log_forcing[i][j][k][1]);
				rd_log_forcing[i][j][k][1] = 1;
				atom_inc(wr_log_forcing[i][j][k][2]);
				rd_log_forcing[i][j][k][2] = 1;
				atom_inc(wr_log_forcing[i][j][k][3]);
				rd_log_forcing[i][j][k][3] = 1;
				atom_inc(wr_log_forcing[i][j][k][4]);
				rd_log_forcing[i][j][k][4] = 1;
				//-------------------------------------------
				// GPU TLS logs (END)
				//-------------------------------------------
			}
			for (m = 0; m < 5; m++) {
				...
				// GPU TLS logs (BEGIN) 
				//-------------------------------------------
				atom_inc(wr_log_forcing[i][j][k][m]);
				rd_log_forcing[i][j][k][m] = 1;
				//-------------------------------------------
				// GPU TLS logs (END)
				//-------------------------------------------
			}
...
__kernel void exact_rhs_2(double dnxm1, __global int *grid_points, double dnzm1,
			  double dnym1, __global double *g_forcing,
			  __global int *g_rd_log_forcing,
			  __global int *g_wr_log_forcing, double ty2,
...
__kernel void exact_rhs_3(double dnxm1, __global int *grid_points, double dnym1,
			  double dnzm1, __global double *g_forcing,
			  __global int *g_rd_log_forcing,
			  __global int *g_wr_log_forcing, double tz2,
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void exact_rhs_4(__global double *g_forcing,
			  __global int *g_rd_log_forcing,
			  __global int *g_wr_log_forcing,
			  __global int *grid_points,
			  __global int *rd_log_grid_points,
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void initialize_0(__global double *g_u, __global int *g_rd_log_u,
			   __global int *g_wr_log_u)
{
	//-------------------------------------------
	//OpenCL global indexes (BEGIN)
	//-------------------------------------------
...
__kernel void initialize_1(double dnxm1, __global int *grid_points,
			   ...
			   __global double *g_ce, __global int *g_rd_log_ce,
			   __global int *g_wr_log_ce, int __ocl_i_bound)
{
	//-------------------------------------------
	//OpenCL global indexes (BEGIN)
	//-------------------------------------------
...
__kernel void initialize_2(double dnym1, __global int *grid_points,
			   double dnzm1, double xi, __global double *g_u,
			   __global int *g_rd_log_u, __global int *g_wr_log_u,
			   int i, __global double *g_ce,
			   __global int *g_rd_log_ce, __global int *g_wr_log_ce,
			   int __ocl_j_bound)
{
	//-------------------------------------------
...
__kernel void initialize_3(double dnym1, double dnzm1, double xi,
			   __global double *g_u, __global int *g_rd_log_u,
			   __global int *g_wr_log_u, int i,
			   __global double *g_ce, __global int *g_rd_log_ce,
			   __global int *g_wr_log_ce, int __ocl_k_bound,
			   int __ocl_j_bound)
{
	//-------------------------------------------
	//OpenCL global indexes (BEGIN)
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void lhsinit_0(__global double *g_lhs, __global int *g_rd_log_lhs,
			__global int *g_wr_log_lhs, __global int *grid_points,
			__global int *rd_log_grid_points,
			__global int *wr_log_grid_points, int __ocl_k_bound)
{
	//-------------------------------------------
	...
	//TLS Checking Buffers (BEGIN)
	__global int (*rd_log_lhs)[25][25][3][5][5] =
	    (__global int (*)[25][25][3][5][5])g_rd_log_lhs;
	__global int (*wr_log_lhs)[25][25][3][5][5] =
	    (__global int (*)[25][25][3][5][5])g_wr_log_lhs;
	//TLS Checking Buffers (END)
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void lhsinit_1(__global double *g_lhs, __global int *g_rd_log_lhs,
			__global int *g_wr_log_lhs, __global int *grid_points,
			__global int *rd_log_grid_points,
			__global int *wr_log_grid_points, int __ocl_k_bound)
{
...
__kernel void lhsx_0(__global int *grid_points, __global double *g_u,
		     __global double *g_fjac, __global int *g_rd_log_fjac,
		     __global int *g_wr_log_fjac, double c2, double c1,
		     __global double *g_njac, __global int *g_rd_log_njac,
		     __global int *g_wr_log_njac, double con43, double c3c4,
		     double c1345, double dt, double tx1, double tx2,
		     __global double *g_lhs, __global int *g_rd_log_lhs,
		     __global int *g_wr_log_lhs, double dx1, double dx2,
		     ...
		     int __ocl_j_bound)
{
	...
	//TLS Checking Buffers (BEGIN)
	__global int (*rd_log_fjac)[24][23][5][5] =
	    (__global int (*)[24][23][5][5])g_rd_log_fjac;
	__global int (*wr_log_fjac)[24][23][5][5] =
	    (__global int (*)[24][23][5][5])g_wr_log_fjac;
	__global int (*rd_log_njac)[24][23][5][5] =
	    (__global int (*)[24][23][5][5])g_rd_log_njac;
	__global int (*wr_log_njac)[24][23][5][5] =
	    (__global int (*)[24][23][5][5])g_wr_log_njac;
	__global int (*rd_log_lhs)[25][25][3][5][5] =
	    (__global int (*)[25][25][3][5][5])g_rd_log_lhs;
	__global int (*wr_log_lhs)[25][25][3][5][5] =
	    (__global int (*)[25][25][3][5][5])g_wr_log_lhs;
	...
	//-------------------------------------------
	{
		for (i = 0; i < grid_points[0]; i++) {
			...
			//      u[i][j][k][4]
			//-------------------------------------------
			__global double *p_u_0_0 = &u[i][j][k][0];
...
//				u_0 = vload4(0, p_u_0_0);
			...
			fjac[i][j][k][1][0] =
			    ...
			    c2 * 0.50 * (u_0.y * u_0.y + u_0.z * u_0.z +
					 u_0.w * u_0.w) * tmp2;
			fjac[i][j][k][1][1] = (2.0 - c2) * (u_0.y / u_0.x);
			fjac[i][j][k][1][2] = -c2 * (u_0.z * tmp1);
			fjac[i][j][k][1][3] = -c2 * (u_0.w * tmp1);
			fjac[i][j][k][1][4] = c2;
			fjac[i][j][k][2][0] = -(u_0.y * u_0.z) * tmp2;
			fjac[i][j][k][2][1] = u_0.z * tmp1;
			fjac[i][j][k][2][2] = u_0.y * tmp1;
			fjac[i][j][k][2][3] = 0.0;
			fjac[i][j][k][2][4] = 0.0;
			fjac[i][j][k][3][0] = -(u_0.y * u_0.w) * tmp2;
			fjac[i][j][k][3][1] = u_0.w * tmp1;
			fjac[i][j][k][3][2] = 0.0;
			fjac[i][j][k][3][3] = u_0.y * tmp1;
			fjac[i][j][k][3][4] = 0.0;
			fjac[i][j][k][4][0] =
			    (c2 *
			...
			fjac[i][j][k][4][1] =
			    c1 * u_1 * tmp1 - 0.50 * c2 * (3.0 * u_0.y * u_0.y +
							   ...
							   u_0.w * u_0.w) *
			    tmp2;
			fjac[i][j][k][4][2] = -c2 * (u_0.z * u_0.y) * tmp2;
			fjac[i][j][k][4][3] = -c2 * (u_0.w * u_0.y) * tmp2;
			fjac[i][j][k][4][4] = c1 * (u_0.y * tmp1);
			njac[i][j][k][0][0] = 0.0;
			njac[i][j][k][0][1] = 0.0;
			njac[i][j][k][0][2] = 0.0;
			njac[i][j][k][0][3] = 0.0;
			njac[i][j][k][0][4] = 0.0;
			njac[i][j][k][1][0] = -con43 * c3c4 * tmp2 * u_0.y;
			njac[i][j][k][1][1] = con43 * c3c4 * tmp1;
			njac[i][j][k][1][2] = 0.0;
			njac[i][j][k][1][3] = 0.0;
			njac[i][j][k][1][4] = 0.0;
			njac[i][j][k][2][0] = -c3c4 * tmp2 * u_0.z;
			njac[i][j][k][2][1] = 0.0;
			njac[i][j][k][2][2] = c3c4 * tmp1;
			njac[i][j][k][2][3] = 0.0;
			njac[i][j][k][2][4] = 0.0;
			njac[i][j][k][3][0] = -c3c4 * tmp2 * u_0.w;
			njac[i][j][k][3][1] = 0.0;
			njac[i][j][k][3][2] = 0.0;
			...
			njac[i][j][k][4][2] = (c3c4 - c1345) * tmp2 * u_0.z;
			njac[i][j][k][4][3] = (c3c4 - c1345) * tmp2 * u_0.w;
			njac[i][j][k][4][4] = (c1345) * tmp1;
			//-------------------------------------------
			// GPU TLS logs (BEGIN) 
			//-------------------------------------------
			atom_inc(wr_log_fjac[i][j][k][0][0]);
			atom_inc(wr_log_fjac[i][j][k][0][1]);
			atom_inc(wr_log_fjac[i][j][k][0][2]);
			atom_inc(wr_log_fjac[i][j][k][0][3]);
			atom_inc(wr_log_fjac[i][j][k][0][4]);
			atom_inc(wr_log_fjac[i][j][k][1][0]);
			atom_inc(wr_log_fjac[i][j][k][1][1]);
			atom_inc(wr_log_fjac[i][j][k][1][2]);
			atom_inc(wr_log_fjac[i][j][k][1][3]);
			atom_inc(wr_log_fjac[i][j][k][1][4]);
			atom_inc(wr_log_fjac[i][j][k][2][0]);
			atom_inc(wr_log_fjac[i][j][k][2][1]);
			atom_inc(wr_log_fjac[i][j][k][2][2]);
			atom_inc(wr_log_fjac[i][j][k][2][3]);
			atom_inc(wr_log_fjac[i][j][k][2][4]);
			atom_inc(wr_log_fjac[i][j][k][3][0]);
			atom_inc(wr_log_fjac[i][j][k][3][1]);
			atom_inc(wr_log_fjac[i][j][k][3][2]);
			atom_inc(wr_log_fjac[i][j][k][3][3]);
			atom_inc(wr_log_fjac[i][j][k][3][4]);
			atom_inc(wr_log_fjac[i][j][k][4][0]);
			atom_inc(wr_log_fjac[i][j][k][4][1]);
			atom_inc(wr_log_fjac[i][j][k][4][2]);
			atom_inc(wr_log_fjac[i][j][k][4][3]);
			atom_inc(wr_log_fjac[i][j][k][4][4]);
			atom_inc(wr_log_njac[i][j][k][0][0]);
			atom_inc(wr_log_njac[i][j][k][0][1]);
			atom_inc(wr_log_njac[i][j][k][0][2]);
			atom_inc(wr_log_njac[i][j][k][0][3]);
			atom_inc(wr_log_njac[i][j][k][0][4]);
			atom_inc(wr_log_njac[i][j][k][1][0]);
			atom_inc(wr_log_njac[i][j][k][1][1]);
			atom_inc(wr_log_njac[i][j][k][1][2]);
			atom_inc(wr_log_njac[i][j][k][1][3]);
			atom_inc(wr_log_njac[i][j][k][1][4]);
			atom_inc(wr_log_njac[i][j][k][2][0]);
			atom_inc(wr_log_njac[i][j][k][2][1]);
			atom_inc(wr_log_njac[i][j][k][2][2]);
			atom_inc(wr_log_njac[i][j][k][2][3]);
			atom_inc(wr_log_njac[i][j][k][2][4]);
			atom_inc(wr_log_njac[i][j][k][3][0]);
			atom_inc(wr_log_njac[i][j][k][3][1]);
			atom_inc(wr_log_njac[i][j][k][3][2]);
			atom_inc(wr_log_njac[i][j][k][3][3]);
			atom_inc(wr_log_njac[i][j][k][3][4]);
			atom_inc(wr_log_njac[i][j][k][4][0]);
			atom_inc(wr_log_njac[i][j][k][4][1]);
			atom_inc(wr_log_njac[i][j][k][4][2]);
			atom_inc(wr_log_njac[i][j][k][4][3]);
			atom_inc(wr_log_njac[i][j][k][4][4]);
			//-------------------------------------------
			// GPU TLS logs (END)
			//-------------------------------------------
		}
		for (i = 1; i < grid_points[0] - 1; i++) {
			...
			// GPU TLS logs (BEGIN) 
			//-------------------------------------------
			atom_inc(wr_log_lhs[i][j][k][0][0][0]);
			rd_log_fjac[i - 1][j][k][0][0] = 1;
			rd_log_njac[i - 1][j][k][0][0] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][0][1]);
			rd_log_fjac[i - 1][j][k][0][1] = 1;
			rd_log_njac[i - 1][j][k][0][1] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][0][2]);
			rd_log_fjac[i - 1][j][k][0][2] = 1;
			rd_log_njac[i - 1][j][k][0][2] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][0][3]);
			rd_log_fjac[i - 1][j][k][0][3] = 1;
			rd_log_njac[i - 1][j][k][0][3] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][0][4]);
			rd_log_fjac[i - 1][j][k][0][4] = 1;
			rd_log_njac[i - 1][j][k][0][4] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][1][0]);
			rd_log_fjac[i - 1][j][k][1][0] = 1;
			rd_log_njac[i - 1][j][k][1][0] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][1][1]);
			rd_log_fjac[i - 1][j][k][1][1] = 1;
			rd_log_njac[i - 1][j][k][1][1] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][1][2]);
			rd_log_fjac[i - 1][j][k][1][2] = 1;
			rd_log_njac[i - 1][j][k][1][2] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][1][3]);
			rd_log_fjac[i - 1][j][k][1][3] = 1;
			rd_log_njac[i - 1][j][k][1][3] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][1][4]);
			rd_log_fjac[i - 1][j][k][1][4] = 1;
			rd_log_njac[i - 1][j][k][1][4] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][2][0]);
			rd_log_fjac[i - 1][j][k][2][0] = 1;
			rd_log_njac[i - 1][j][k][2][0] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][2][1]);
			rd_log_fjac[i - 1][j][k][2][1] = 1;
			rd_log_njac[i - 1][j][k][2][1] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][2][2]);
			rd_log_fjac[i - 1][j][k][2][2] = 1;
			rd_log_njac[i - 1][j][k][2][2] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][2][3]);
			rd_log_fjac[i - 1][j][k][2][3] = 1;
			rd_log_njac[i - 1][j][k][2][3] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][2][4]);
			rd_log_fjac[i - 1][j][k][2][4] = 1;
			rd_log_njac[i - 1][j][k][2][4] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][3][0]);
			rd_log_fjac[i - 1][j][k][3][0] = 1;
			rd_log_njac[i - 1][j][k][3][0] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][3][1]);
			rd_log_fjac[i - 1][j][k][3][1] = 1;
			rd_log_njac[i - 1][j][k][3][1] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][3][2]);
			rd_log_fjac[i - 1][j][k][3][2] = 1;
			rd_log_njac[i - 1][j][k][3][2] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][3][3]);
			rd_log_fjac[i - 1][j][k][3][3] = 1;
			rd_log_njac[i - 1][j][k][3][3] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][3][4]);
			rd_log_fjac[i - 1][j][k][3][4] = 1;
			rd_log_njac[i - 1][j][k][3][4] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][4][0]);
			rd_log_fjac[i - 1][j][k][4][0] = 1;
			rd_log_njac[i - 1][j][k][4][0] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][4][1]);
			rd_log_fjac[i - 1][j][k][4][1] = 1;
			rd_log_njac[i - 1][j][k][4][1] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][4][2]);
			rd_log_fjac[i - 1][j][k][4][2] = 1;
			rd_log_njac[i - 1][j][k][4][2] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][4][3]);
			rd_log_fjac[i - 1][j][k][4][3] = 1;
			rd_log_njac[i - 1][j][k][4][3] = 1;
			atom_inc(wr_log_lhs[i][j][k][0][4][4]);
			rd_log_fjac[i - 1][j][k][4][4] = 1;
			rd_log_njac[i - 1][j][k][4][4] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][0][0]);
			rd_log_njac[i][j][k][0][0] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][0][1]);
			rd_log_njac[i][j][k][0][1] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][0][2]);
			rd_log_njac[i][j][k][0][2] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][0][3]);
			rd_log_njac[i][j][k][0][3] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][0][4]);
			rd_log_njac[i][j][k][0][4] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][1][0]);
			rd_log_njac[i][j][k][1][0] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][1][1]);
			rd_log_njac[i][j][k][1][1] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][1][2]);
			rd_log_njac[i][j][k][1][2] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][1][3]);
			rd_log_njac[i][j][k][1][3] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][1][4]);
			rd_log_njac[i][j][k][1][4] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][2][0]);
			rd_log_njac[i][j][k][2][0] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][2][1]);
			rd_log_njac[i][j][k][2][1] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][2][2]);
			rd_log_njac[i][j][k][2][2] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][2][3]);
			rd_log_njac[i][j][k][2][3] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][2][4]);
			rd_log_njac[i][j][k][2][4] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][3][0]);
			rd_log_njac[i][j][k][3][0] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][3][1]);
			rd_log_njac[i][j][k][3][1] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][3][2]);
			rd_log_njac[i][j][k][3][2] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][3][3]);
			rd_log_njac[i][j][k][3][3] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][3][4]);
			rd_log_njac[i][j][k][3][4] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][4][0]);
			rd_log_njac[i][j][k][4][0] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][4][1]);
			rd_log_njac[i][j][k][4][1] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][4][2]);
			rd_log_njac[i][j][k][4][2] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][4][3]);
			rd_log_njac[i][j][k][4][3] = 1;
			atom_inc(wr_log_lhs[i][j][k][1][4][4]);
			rd_log_njac[i][j][k][4][4] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][0][0]);
			rd_log_fjac[i + 1][j][k][0][0] = 1;
			rd_log_njac[i + 1][j][k][0][0] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][0][1]);
			rd_log_fjac[i + 1][j][k][0][1] = 1;
			rd_log_njac[i + 1][j][k][0][1] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][0][2]);
			rd_log_fjac[i + 1][j][k][0][2] = 1;
			rd_log_njac[i + 1][j][k][0][2] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][0][3]);
			rd_log_fjac[i + 1][j][k][0][3] = 1;
			rd_log_njac[i + 1][j][k][0][3] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][0][4]);
			rd_log_fjac[i + 1][j][k][0][4] = 1;
			rd_log_njac[i + 1][j][k][0][4] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][1][0]);
			rd_log_fjac[i + 1][j][k][1][0] = 1;
			rd_log_njac[i + 1][j][k][1][0] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][1][1]);
			rd_log_fjac[i + 1][j][k][1][1] = 1;
			rd_log_njac[i + 1][j][k][1][1] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][1][2]);
			rd_log_fjac[i + 1][j][k][1][2] = 1;
			rd_log_njac[i + 1][j][k][1][2] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][1][3]);
			rd_log_fjac[i + 1][j][k][1][3] = 1;
			rd_log_njac[i + 1][j][k][1][3] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][1][4]);
			rd_log_fjac[i + 1][j][k][1][4] = 1;
			rd_log_njac[i + 1][j][k][1][4] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][2][0]);
			rd_log_fjac[i + 1][j][k][2][0] = 1;
			rd_log_njac[i + 1][j][k][2][0] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][2][1]);
			rd_log_fjac[i + 1][j][k][2][1] = 1;
			rd_log_njac[i + 1][j][k][2][1] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][2][2]);
			rd_log_fjac[i + 1][j][k][2][2] = 1;
			rd_log_njac[i + 1][j][k][2][2] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][2][3]);
			rd_log_fjac[i + 1][j][k][2][3] = 1;
			rd_log_njac[i + 1][j][k][2][3] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][2][4]);
			rd_log_fjac[i + 1][j][k][2][4] = 1;
			rd_log_njac[i + 1][j][k][2][4] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][3][0]);
			rd_log_fjac[i + 1][j][k][3][0] = 1;
			rd_log_njac[i + 1][j][k][3][0] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][3][1]);
			rd_log_fjac[i + 1][j][k][3][1] = 1;
			rd_log_njac[i + 1][j][k][3][1] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][3][2]);
			rd_log_fjac[i + 1][j][k][3][2] = 1;
			rd_log_njac[i + 1][j][k][3][2] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][3][3]);
			rd_log_fjac[i + 1][j][k][3][3] = 1;
			rd_log_njac[i + 1][j][k][3][3] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][3][4]);
			rd_log_fjac[i + 1][j][k][3][4] = 1;
			rd_log_njac[i + 1][j][k][3][4] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][4][0]);
			rd_log_fjac[i + 1][j][k][4][0] = 1;
			rd_log_njac[i + 1][j][k][4][0] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][4][1]);
			rd_log_fjac[i + 1][j][k][4][1] = 1;
			rd_log_njac[i + 1][j][k][4][1] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][4][2]);
			rd_log_fjac[i + 1][j][k][4][2] = 1;
			rd_log_njac[i + 1][j][k][4][2] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][4][3]);
			rd_log_fjac[i + 1][j][k][4][3] = 1;
			rd_log_njac[i + 1][j][k][4][3] = 1;
			atom_inc(wr_log_lhs[i][j][k][2][4][4]);
			rd_log_fjac[i + 1][j][k][4][4] = 1;
			rd_log_njac[i + 1][j][k][4][4] = 1;
			//-------------------------------------------
			// GPU TLS logs (END)
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void lhsy_0(__global double *g_u, __global double *g_fjac,
		     __global int *g_rd_log_fjac, __global int *g_wr_log_fjac,
		     double c2, double c1, __global double *g_njac,
		     __global int *g_rd_log_njac, __global int *g_wr_log_njac,
		     double c3c4, double con43, double c1345, int __ocl_k_bound,
		     int __ocl_j_bound, int __ocl_i_bound)
{
	...
	//-------------------------------------------
	{
		...
		fjac[i][j][k][0][3] = 0.0;
		fjac[i][j][k][0][4] = 0.0;
		fjac[i][j][k][1][0] = -(u_2.y * u_2.z) * tmp2;
		fjac[i][j][k][1][1] = u_2.z * tmp1;
		fjac[i][j][k][1][2] = u_2.y * tmp1;
		fjac[i][j][k][1][3] = 0.0;
		fjac[i][j][k][1][4] = 0.0;
		fjac[i][j][k][2][0] =
		    -(u_2.z * u_2.z * tmp2) +
		...
		fjac[i][j][k][2][1] = -c2 * u_2.y * tmp1;
		fjac[i][j][k][2][2] = (2.0 - c2) * u_2.z * tmp1;
		fjac[i][j][k][2][3] = -c2 * u_2.w * tmp1;
		fjac[i][j][k][2][4] = c2;
		fjac[i][j][k][3][0] = -(u_2.z * u_2.w) * tmp2;
		fjac[i][j][k][3][1] = 0.0;
		fjac[i][j][k][3][2] = u_2.w * tmp1;
		fjac[i][j][k][3][3] = u_2.z * tmp1;
		fjac[i][j][k][3][4] = 0.0;
		fjac[i][j][k][4][0] =
		    (c2 * (u_2.y * u_2.y + u_2.z * u_2.z + u_2.w * u_2.w) *
		     tmp2 - c1 * u_3 * tmp1) * u_2.z * tmp1;
		fjac[i][j][k][4][1] = -c2 * u_2.y * u_2.z * tmp2;
		fjac[i][j][k][4][2] =
		    c1 * u_3 * tmp1 -
		    0.50 * c2 *
		    ((u_2.y * u_2.y + 3.0 * u_2.z * u_2.z +
		      u_2.w * u_2.w) * tmp2);
		fjac[i][j][k][4][3] = -c2 * (u_2.z * u_2.w) * tmp2;
		fjac[i][j][k][4][4] = c1 * u_2.z * tmp1;
		njac[i][j][k][0][0] = 0.0;
		njac[i][j][k][0][1] = 0.0;
		njac[i][j][k][0][2] = 0.0;
		njac[i][j][k][0][3] = 0.0;
		njac[i][j][k][0][4] = 0.0;
		njac[i][j][k][1][0] = -c3c4 * tmp2 * u_2.y;
		njac[i][j][k][1][1] = c3c4 * tmp1;
		njac[i][j][k][1][2] = 0.0;
		njac[i][j][k][1][3] = 0.0;
		njac[i][j][k][1][4] = 0.0;
		njac[i][j][k][2][0] = -con43 * c3c4 * tmp2 * u_2.z;
		njac[i][j][k][2][1] = 0.0;
		...
		njac[i][j][k][4][0] =
		    ...
		    c1345 * tmp2 * u_3;
		njac[i][j][k][4][1] = (c3c4 - c1345) * tmp2 * u_2.y;
		njac[i][j][k][4][2] = (con43 * c3c4 - c1345) * tmp2 * u_2.z;
		     ...
		     int __ocl_k_bound, int __ocl_j_bound, int __ocl_i_bound)
{
	...
	//-------------------------------------------
	{
		...
		//      njac[i][j + 1][k][4][4]
		//-------------------------------------------
		__global double *p_fjac_0_0 = &fjac[i][j - 1][k][0][1];
		...
		__global double *p_fjac_0_1 = &fjac[i][j + 1][k][3][1];
...
//			fjac_0[1] = vload2(0, p_fjac_0_1);
		...
		__global double *p_fjac_0_2 = &fjac[i][j + 1][k][4][3];
...
//			fjac_0[2] = vload2(0, p_fjac_0_2);
		...
		__global double *p_njac_0_0 = &njac[i][j + 1][k][0][1];
...
//			njac_0[0] = vload2(0, p_njac_0_0);
		...
		__global double *p_njac_0_1 = &njac[i][j][k][0][1];
...
//			njac_0[1] = vload2(0, p_njac_0_1);
		...
		__global double *p_njac_0_2 = &njac[i][j][k][1][1];
...
//			njac_0[2] = vload2(0, p_njac_0_2);
		...
		__global double *p_njac_0_3 = &njac[i][j - 1][k][1][1];
...
//			njac_0[3] = vload2(0, p_njac_0_3);
		...
		__global double *p_njac_0_4 = &njac[i][j + 1][k][3][0];
...
//			njac_0[4] = vload2(0, p_njac_0_4);
		...
		__global double *p_njac_0_5 = &njac[i][j + 1][k][1][3];
...
//			njac_0[5] = vload2(0, p_njac_0_5);
		...
		__global double *p_njac_0_6 = &njac[i][j - 1][k][3][0];
...
//			njac_0[6] = vload2(0, p_njac_0_6);
		...
		__global double *p_njac_0_7 = &njac[i][j][k][3][2];
...
//			njac_0[7] = vload2(0, p_njac_0_7);
		...
		__global double *p_njac_0_8 = &njac[i][j + 1][k][2][3];
...
//			njac_0[8] = vload2(0, p_njac_0_8);
		...
		__global double *p_njac_0_9 = &njac[i][j - 1][k][4][1];
...
//			njac_0[9] = vload2(0, p_njac_0_9);
		...
		__global double *p_njac_0_10 = &njac[i][j + 1][k][4][3];
...
//			njac_0[10] = vload2(0, p_njac_0_10);
		...
		lhs[i][j][k][0][0][0] =
		    ...
		    - tmp1 * dy1;
		lhs[i][j][k][0][0][1] =
		    -tmp2 * fjac_0[0].x - tmp1 * njac[i][j - 1][k][0][1];
		lhs[i][j][k][0][0][2] =
		    -tmp2 * fjac_0[0].y - tmp1 * njac[i][j - 1][k][0][2];
		lhs[i][j][k][0][0][3] =
		...
		lhs[i][j][k][0][1][0] =
		    ...
		    [0];
		lhs[i][j][k][0][1][1] =
		    -tmp2 * fjac[i][j - 1][k][1][1] - tmp1 * njac_0[3].x -
		    tmp1 * dy2;
		lhs[i][j][k][0][1][2] =
		    -tmp2 * fjac[i][j - 1][k][1][2] - tmp1 * njac_0[3].y;
		lhs[i][j][k][0][1][3] =
		...
		lhs[i][j][k][0][2][4] =
		    ...
		    [4];
		lhs[i][j][k][0][3][0] =
		    -tmp2 * fjac[i][j - 1][k][3][0] - tmp1 * njac_0[6].x;
		lhs[i][j][k][0][3][1] =
		    -tmp2 * fjac[i][j - 1][k][3][1] - tmp1 * njac_0[6].y;
		lhs[i][j][k][0][3][2] =
		...
		lhs[i][j][k][0][4][0] =
		    ...
		    [0];
		lhs[i][j][k][0][4][1] =
		    -tmp2 * fjac[i][j - 1][k][4][1] - tmp1 * njac_0[9].x;
		lhs[i][j][k][0][4][2] =
		    -tmp2 * fjac[i][j - 1][k][4][2] - tmp1 * njac_0[9].y;
		lhs[i][j][k][0][4][3] =
		...
		lhs[i][j][k][1][0][1] = tmp1 * 2.0 * njac_0[1].x;
		lhs[i][j][k][1][0][2] = tmp1 * 2.0 * njac_0[1].y;
		lhs[i][j][k][1][0][3] = tmp1 * 2.0 * njac[i][j][k][0][3];
		lhs[i][j][k][1][0][4] = tmp1 * 2.0 * njac[i][j][k][0][4];
		lhs[i][j][k][1][1][0] = tmp1 * 2.0 * njac[i][j][k][1][0];
		lhs[i][j][k][1][1][1] =
		    1.0 + tmp1 * 2.0 * njac_0[2].x + tmp1 * 2.0 * dy2;
		lhs[i][j][k][1][1][2] = tmp1 * 2.0 * njac_0[2].y;
		...
		lhs[i][j][k][1][3][2] = tmp1 * 2.0 * njac_0[7].x;
		lhs[i][j][k][1][3][3] =
		    1.0 + tmp1 * 2.0 * njac_0[7].y + tmp1 * 2.0 * dy4;
		...
		lhs[i][j][k][2][0][0] =
		    ...
		    - tmp1 * dy1;
		lhs[i][j][k][2][0][1] =
		    tmp2 * fjac[i][j + 1][k][0][1] - tmp1 * njac_0[0].x;
		lhs[i][j][k][2][0][2] =
		    tmp2 * fjac[i][j + 1][k][0][2] - tmp1 * njac_0[0].y;
		lhs[i][j][k][2][0][3] =
		    tmp2 * fjac[i][j + 1][k][0][3] - tmp1 * njac[i][j +
								    1][k][0][3];
		lhs[i][j][k][2][0][4] =
		...
		lhs[i][j][k][2][1][2] =
		    tmp2 * fjac[i][j + 1][k][1][2] - tmp1 * njac[i][j +
								    1][k][1][2];
		lhs[i][j][k][2][1][3] =
		    tmp2 * fjac[i][j + 1][k][1][3] - tmp1 * njac_0[5].x;
		lhs[i][j][k][2][1][4] =
		    tmp2 * fjac[i][j + 1][k][1][4] - tmp1 * njac_0[5].y;
		lhs[i][j][k][2][2][0] =
		    tmp2 * fjac[i][j + 1][k][2][0] - tmp1 * njac[i][j +
								    1][k][2][0];
		lhs[i][j][k][2][2][1] =
		...
		lhs[i][j][k][2][2][2] =
		    ...
		    - tmp1 * dy3;
		lhs[i][j][k][2][2][3] =
		    tmp2 * fjac[i][j + 1][k][2][3] - tmp1 * njac_0[8].x;
		lhs[i][j][k][2][2][4] =
		    tmp2 * fjac[i][j + 1][k][2][4] - tmp1 * njac_0[8].y;
		lhs[i][j][k][2][3][0] =
		    tmp2 * fjac[i][j + 1][k][3][0] - tmp1 * njac_0[4].x;
		lhs[i][j][k][2][3][1] = tmp2 * fjac_0[1].x - tmp1 * njac_0[4].y;
		lhs[i][j][k][2][3][2] =
		    tmp2 * fjac_0[1].y - tmp1 * njac[i][j + 1][k][3][2];
		lhs[i][j][k][2][3][3] =
		    tmp2 * fjac[i][j + 1][k][3][3] - tmp1 * njac[i][j +
								    1][k][3][3]
		    - tmp1 * dy4;
		...
		lhs[i][j][k][2][4][2] =
		    tmp2 * fjac[i][j + 1][k][4][2] - tmp1 * njac[i][j +
								    1][k][4][2];
		lhs[i][j][k][2][4][3] =
		    tmp2 * fjac_0[2].x - tmp1 * njac_0[10].x;
		lhs[i][j][k][2][4][4] =
		    tmp2 * fjac_0[2].y - tmp1 * njac_0[10].y - tmp1 * dy5;
		//-------------------------------------------
		// GPU TLS logs (BEGIN) 
		//-------------------------------------------
		atom_inc(wr_log_lhs[i][j][k][0][0][2]);
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void lhsz_0(__global double *g_u, __global double *g_fjac,
		     __global int *g_rd_log_fjac, __global int *g_wr_log_fjac,
		     double c2, double c1, __global double *g_njac,
		     __global int *g_rd_log_njac, __global int *g_wr_log_njac,
		     double c3c4, double con43, double c3, double c4,
		     ...
		     int __ocl_i_bound)
{
	...
	//-------------------------------------------
	{
		...
		//      u[i][j][k][4]
		//-------------------------------------------
		__global double *p_u_4_0 = &u[i][j][k][0];
...
//			u_4 = vload4(0, p_u_4_0);
		...
		fjac[i][j][k][0][3] = 1.0;
		fjac[i][j][k][0][4] = 0.0;
		fjac[i][j][k][1][0] = -(u_4.y * u_4.w) * tmp2;
		fjac[i][j][k][1][1] = u_4.w * tmp1;
		fjac[i][j][k][1][2] = 0.0;
		fjac[i][j][k][1][3] = u_4.y * tmp1;
		fjac[i][j][k][1][4] = 0.0;
		fjac[i][j][k][2][0] = -(u_4.z * u_4.w) * tmp2;
		fjac[i][j][k][2][1] = 0.0;
		fjac[i][j][k][2][2] = u_4.w * tmp1;
		fjac[i][j][k][2][3] = u_4.z * tmp1;
		fjac[i][j][k][2][4] = 0.0;
		fjac[i][j][k][3][0] =
		    -(u_4.w * u_4.w * tmp2) +
		    0.50 * c2 *
		    ((u_4.y * u_4.y + u_4.z * u_4.z + u_4.w * u_4.w) * tmp2);
		fjac[i][j][k][3][1] = -c2 * u_4.y * tmp1;
		fjac[i][j][k][3][2] = -c2 * u_4.z * tmp1;
		fjac[i][j][k][3][3] = (2.0 - c2) * u_4.w * tmp1;
		fjac[i][j][k][3][4] = c2;
		fjac[i][j][k][4][0] =
		    (c2 * (u_4.y * u_4.y + u_4.z * u_4.z + u_4.w * u_4.w) *
		     tmp2 - c1 * (u_5 * tmp1)) * (u_4.w * tmp1);
		fjac[i][j][k][4][1] = -c2 * (u_4.y * u_4.w) * tmp2;
		fjac[i][j][k][4][2] = -c2 * (u_4.z * u_4.w) * tmp2;
		fjac[i][j][k][4][3] =
		    c1 * (u_5 * tmp1) -
		    0.50 * c2 *
		    ((u_4.y * u_4.y + u_4.z * u_4.z +
		      3.0 * u_4.w * u_4.w) * tmp2);
		fjac[i][j][k][4][4] = c1 * u_4.w * tmp1;
		njac[i][j][k][0][0] = 0.0;
		njac[i][j][k][0][1] = 0.0;
		njac[i][j][k][0][2] = 0.0;
		njac[i][j][k][0][3] = 0.0;
		njac[i][j][k][0][4] = 0.0;
		njac[i][j][k][1][0] = -c3c4 * tmp2 * u_4.y;
		njac[i][j][k][1][1] = c3c4 * tmp1;
		njac[i][j][k][1][2] = 0.0;
		njac[i][j][k][1][3] = 0.0;
		njac[i][j][k][1][4] = 0.0;
		njac[i][j][k][2][0] = -c3c4 * tmp2 * u_4.z;
		njac[i][j][k][2][1] = 0.0;
		njac[i][j][k][2][2] = c3c4 * tmp1;
		njac[i][j][k][2][3] = 0.0;
		njac[i][j][k][2][4] = 0.0;
		njac[i][j][k][3][0] = -con43 * c3c4 * tmp2 * u_4.w;
		njac[i][j][k][3][1] = 0.0;
		njac[i][j][k][3][2] = 0.0;
		    ...
		    tmp3 * (((u_4.z) * (u_4.z))) - (con43 * c3c4 -
						    c1345) * tmp3 * (((u_4.w) *
								      (u_4.
								       w))) -
		    c1345 * tmp2 * u_5;
		njac[i][j][k][4][1] = (c3c4 - c1345) * tmp2 * u_4.y;
		njac[i][j][k][4][2] = (c3c4 - c1345) * tmp2 * u_4.z;
		njac[i][j][k][4][3] = (con43 * c3c4 - c1345) * tmp2 * u_4.w;
		njac[i][j][k][4][4] = (c1345) * tmp1;
		//-------------------------------------------
		// GPU TLS logs (BEGIN) 
		//-------------------------------------------
		     ...
		     int __ocl_k_bound, int __ocl_j_bound, int __ocl_i_bound)
{
	...
	//-------------------------------------------
	{
		...
		//      njac[i][j][k + 1][4][4]
		//-------------------------------------------
		__global double *p_fjac_1_0 = &fjac[i][j][k - 1][0][1];
...
//			fjac_1[0] = vload2(0, p_fjac_1_0);
		...
		__global double *p_fjac_1_1 = &fjac[i][j][k + 1][3][1];
...
//			fjac_1[1] = vload2(0, p_fjac_1_1);
		...
		__global double *p_fjac_1_2 = &fjac[i][j][k + 1][4][3];
...
//			fjac_1[2] = vload2(0, p_fjac_1_2);
		...
		__global double *p_njac_1_0 = &njac[i][j][k + 1][0][1];
...
//			njac_1[2] = vload2(0, p_njac_1_2);
		...
		__global double *p_njac_1_3 = &njac[i][j][k - 1][1][1];
...
//			njac_1[3] = vload2(0, p_njac_1_3);
		...
		__global double *p_njac_1_4 = &njac[i][j][k + 1][3][0];
...
//			njac_1[4] = vload2(0, p_njac_1_4);
		...
		__global double *p_njac_1_5 = &njac[i][j][k + 1][1][3];
...
//			njac_1[5] = vload2(0, p_njac_1_5);
		...
		__global double *p_njac_1_6 = &njac[i][j][k - 1][3][0];
...
//			njac_1[7] = vload2(0, p_njac_1_7);
		...
		__global double *p_njac_1_8 = &njac[i][j][k + 1][2][3];
...
//			njac_1[8] = vload2(0, p_njac_1_8);
		...
		__global double *p_njac_1_9 = &njac[i][j][k - 1][4][1];
...
//			njac_1[9] = vload2(0, p_njac_1_9);
		...
		__global double *p_njac_1_10 = &njac[i][j][k + 1][4][3];
...
//			njac_1[10] = vload2(0, p_njac_1_10);
		...
		lhs[i][j][k][0][0][0] =
		    ...
		    - tmp1 * dz1;
		lhs[i][j][k][0][0][1] =
		    -tmp2 * fjac_1[0].x - tmp1 * njac[i][j][k - 1][0][1];
		lhs[i][j][k][0][0][2] =
		    -tmp2 * fjac_1[0].y - tmp1 * njac[i][j][k - 1][0][2];
		lhs[i][j][k][0][0][3] =
		...
		lhs[i][j][k][0][1][0] =
		    ...
		    [0];
		lhs[i][j][k][0][1][1] =
		    -tmp2 * fjac[i][j][k - 1][1][1] - tmp1 * njac_1[3].x -
		    tmp1 * dz2;
		lhs[i][j][k][0][1][2] =
		    -tmp2 * fjac[i][j][k - 1][1][2] - tmp1 * njac_1[3].y;
		lhs[i][j][k][0][1][3] =
		...
		lhs[i][j][k][0][2][4] =
		    ...
		    [4];
		lhs[i][j][k][0][3][0] =
		    -tmp2 * fjac[i][j][k - 1][3][0] - tmp1 * njac_1[6].x;
		lhs[i][j][k][0][3][1] =
		    -tmp2 * fjac[i][j][k - 1][3][1] - tmp1 * njac_1[6].y;
		lhs[i][j][k][0][3][2] =
		...
		lhs[i][j][k][0][4][0] =
		    ...
		    [0];
		lhs[i][j][k][0][4][1] =
		    -tmp2 * fjac[i][j][k - 1][4][1] - tmp1 * njac_1[9].x;
		lhs[i][j][k][0][4][2] =
		    -tmp2 * fjac[i][j][k - 1][4][2] - tmp1 * njac_1[9].y;
		lhs[i][j][k][0][4][3] =
		...
		lhs[i][j][k][1][1][0] = tmp1 * 2.0 * njac[i][j][k][1][0];
		lhs[i][j][k][1][1][1] =
		    1.0 + tmp1 * 2.0 * njac_1[2].x + tmp1 * 2.0 * dz2;
		...
		lhs[i][j][k][1][3][3] =
		    1.0 + tmp1 * 2.0 * njac_1[7].y + tmp1 * 2.0 * dz4;
		lhs[i][j][k][1][3][4] = tmp1 * 2.0 * njac[i][j][k][3][4];
		lhs[i][j][k][1][4][0] = tmp1 * 2.0 * njac[i][j][k][4][0];
		lhs[i][j][k][1][4][1] = tmp1 * 2.0 * njac[i][j][k][4][1];
		lhs[i][j][k][1][4][2] = tmp1 * 2.0 * njac[i][j][k][4][2];
		...
		lhs[i][j][k][2][0][0] =
		    ...
		    - tmp1 * dz1;
		lhs[i][j][k][2][0][1] =
		    tmp2 * fjac[i][j][k + 1][0][1] - tmp1 * njac_1[0].x;
		lhs[i][j][k][2][0][2] =
		    tmp2 * fjac[i][j][k + 1][0][2] - tmp1 * njac_1[0].y;
		lhs[i][j][k][2][0][3] =
		    tmp2 * fjac[i][j][k + 1][0][3] - tmp1 * njac[i][j][k +
								       1][0][3];
		lhs[i][j][k][2][0][4] =
		...
		lhs[i][j][k][2][1][2] =
		    tmp2 * fjac[i][j][k + 1][1][2] - tmp1 * njac[i][j][k +
								       1][1][2];
		lhs[i][j][k][2][1][3] =
		    tmp2 * fjac[i][j][k + 1][1][3] - tmp1 * njac_1[5].x;
		lhs[i][j][k][2][1][4] =
		    tmp2 * fjac[i][j][k + 1][1][4] - tmp1 * njac_1[5].y;
		lhs[i][j][k][2][2][0] =
		    tmp2 * fjac[i][j][k + 1][2][0] - tmp1 * njac[i][j][k +
								       1][2][0];
		lhs[i][j][k][2][2][1] =
		...
		lhs[i][j][k][2][2][2] =
		    ...
		    - tmp1 * dz3;
		lhs[i][j][k][2][2][3] =
		    tmp2 * fjac[i][j][k + 1][2][3] - tmp1 * njac_1[8].x;
		lhs[i][j][k][2][2][4] =
		    tmp2 * fjac[i][j][k + 1][2][4] - tmp1 * njac_1[8].y;
		lhs[i][j][k][2][3][0] =
		    tmp2 * fjac[i][j][k + 1][3][0] - tmp1 * njac_1[4].x;
		lhs[i][j][k][2][3][1] = tmp2 * fjac_1[1].x - tmp1 * njac_1[4].y;
		lhs[i][j][k][2][3][2] =
		    tmp2 * fjac_1[1].y - tmp1 * njac[i][j][k + 1][3][2];
		lhs[i][j][k][2][3][3] =
		    tmp2 * fjac[i][j][k + 1][3][3] - tmp1 * njac[i][j][k +
								       1][3][3]
		    - tmp1 * dz4;
		...
		lhs[i][j][k][2][4][4] =
		    tmp2 * fjac_1[2].y - tmp1 * njac_1[10].y - tmp1 * dz5;
		//-------------------------------------------
		// GPU TLS logs (BEGIN) 
		//-------------------------------------------
		atom_inc(wr_log_lhs[i][j][k][0][0][2]);
...
__kernel void compute_rhs_0(__global double *g_u, __global double *g_rho_i,
			    __global int *g_rd_log_rho_i,
			    __global int *g_wr_log_rho_i, __global double *g_us,
			    __global int *g_rd_log_us,
			    __global int *g_wr_log_us, __global double *g_vs,
			    __global int *g_rd_log_vs,
			    __global int *g_wr_log_vs, __global double *g_ws,
			    __global int *g_rd_log_ws,
			    __global int *g_wr_log_ws,
			    __global double *g_square,
			    __global int *g_rd_log_square,
			    __global int *g_wr_log_square,
			    __global double *g_qs, __global int *g_rd_log_qs,
			    __global int *g_wr_log_qs, int __ocl_k_bound,
			    int __ocl_j_bound, int __ocl_i_bound)
{
	//-------------------------------------------
	//OpenCL global indexes (BEGIN)
	...
	//TLS Checking Buffers (BEGIN)
	__global int (*rd_log_rho_i)[25][25] =
	    (__global int (*)[25][25])g_rd_log_rho_i;
	__global int (*wr_log_rho_i)[25][25] =
	    (__global int (*)[25][25])g_wr_log_rho_i;
	__global int (*rd_log_us)[25][25] =
	    (__global int (*)[25][25])g_rd_log_us;
	__global int (*wr_log_us)[25][25] =
	    (__global int (*)[25][25])g_wr_log_us;
	__global int (*rd_log_vs)[25][25] =
	    (__global int (*)[25][25])g_rd_log_vs;
	__global int (*wr_log_vs)[25][25] =
	    (__global int (*)[25][25])g_wr_log_vs;
	__global int (*rd_log_ws)[25][25] =
	    (__global int (*)[25][25])g_rd_log_ws;
	__global int (*wr_log_ws)[25][25] =
	    (__global int (*)[25][25])g_wr_log_ws;
	__global int (*rd_log_square)[25][25] =
	    (__global int (*)[25][25])g_rd_log_square;
	__global int (*wr_log_square)[25][25] =
	    (__global int (*)[25][25])g_wr_log_square;
	__global int (*rd_log_qs)[25][25] =
	    (__global int (*)[25][25])g_rd_log_qs;
	__global int (*wr_log_qs)[25][25] =
	    (__global int (*)[25][25])g_wr_log_qs;
	//TLS Checking Buffers (END)
...
//			u_6 = vload4(0, p_u_6_0);
		...
		rho_inv = 1.0 / u_6.x;
		rho_i[i][j][k] = rho_inv;
		us[i][j][k] = u_6.y * rho_inv;
		vs[i][j][k] = u_6.z * rho_inv;
		ws[i][j][k] = u_6.w * rho_inv;
		square[i][j][k] =
		...
		// GPU TLS logs (BEGIN) 
		//-------------------------------------------
		atom_inc(wr_log_rho_i[i][j][k]);
		atom_inc(wr_log_us[i][j][k]);
		atom_inc(wr_log_vs[i][j][k]);
		atom_inc(wr_log_ws[i][j][k]);
		atom_inc(wr_log_square[i][j][k]);
		atom_inc(wr_log_qs[i][j][k]);
		rd_log_square[i][j][k] = 1;
		//-------------------------------------------
		// GPU TLS logs (END)
		//-------------------------------------------
	}
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void compute_rhs_1(__global double *g_rhs, __global int *g_rd_log_rhs,
			    ...
			    int __ocl_j_bound)
{
	...
	//TLS Checking Buffers (BEGIN)
	__global int (*rd_log_rhs)[25][25][5] =
	    (__global int (*)[25][25][5])g_rd_log_rhs;
	__global int (*wr_log_rhs)[25][25][5] =
	    (__global int (*)[25][25][5])g_wr_log_rhs;
	//TLS Checking Buffers (END)
...
__kernel void compute_rhs_2(__global double *g_us, __global double *g_rhs,
			    ...
			    __global int *g_wr_log_rhs, double dx1tx1,
			    __global double *g_u, double tx2, double dx2tx1,
			    double xxcon2, double con43,
	...
	//-------------------------------------------
	{
		...
		rhs[i][j][k][0] =
		    rhs[i][j][k][0] + dx1tx1 * (u[i + 1][j][k][0] -
						...
						[0]) - tx2 * (u_7[1] - u_7[0]);
		rhs[i][j][k][1] =
		    rhs[i][j][k][1] + dx2tx1 * (u_7[1] - 2.0 * u[i][j][k][1] +
						u_7[0]) +
		    xxcon2 * con43 * (up1 - 2.0 * uijk + um1) -
		    tx2 * (u_7[1] * up1 - u_7[0] * um1 +
			   (u[i + 1][j][k][4] - square[i + 1][j][k] - u_7[3] +
			    square_0) * c2);
		rhs[i][j][k][2] =
		    rhs[i][j][k][2] + dx3tx1 * (u_7[4] - 2.0 * u[i][j][k][2] +
						u_7[2]) + xxcon2 * (vs[i +
								       1][j][k]
								    -
								    2.0 *
		...
		rhs[i][j][k][3] =
		    rhs[i][j][k][3] + dx4tx1 * (u_7[6] - 2.0 * u[i][j][k][3] +
						u_7[5]) + xxcon2 * (ws[i +
								       1][j][k]
								    -
								    2.0 *
		...
		rhs[i][j][k][4] =
		    ...
		    xxcon5 * (u[i + 1][j][k][4] * rho_i[i + 1][j][k] -
			      2.0 * u_7[7] * rho_i[i][j][k] + u_7[3] * rho_i[i -
									     1]
			      [j][k]) - tx2 * ((c1 * u[i + 1][j][k][4] -
						c2 * square[i +
							    1][j][k]) * up1 -
					       (c1 * u_7[3] -
						c2 * square_0) * um1);
		...
		// GPU TLS logs (BEGIN) 
		//-------------------------------------------
		atom_inc(wr_log_rhs[i][j][k][1]);
		rd_log_rhs[i][j][k][1] = 1;
		atom_inc(wr_log_rhs[i][j][k][2]);
		rd_log_rhs[i][j][k][2] = 1;
		atom_inc(wr_log_rhs[i][j][k][3]);
		rd_log_rhs[i][j][k][3] = 1;
		atom_inc(wr_log_rhs[i][j][k][4]);
		rd_log_rhs[i][j][k][4] = 1;
		//-------------------------------------------
		// GPU TLS logs (END)
		//-------------------------------------------
	}
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void compute_rhs_3(__global double *g_rhs, __global int *g_rd_log_rhs,
			    __global int *g_wr_log_rhs, int i, double dssp,
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void compute_rhs_4(__global double *g_rhs, __global int *g_rd_log_rhs,
			    __global int *g_wr_log_rhs, int i, double dssp,
			    __global double *g_u, int __ocl_k_bound,
			    int __ocl_j_bound)
{
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void compute_rhs_5(__global double *g_rhs, __global int *g_rd_log_rhs,
			    __global int *g_wr_log_rhs, double dssp,
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void compute_rhs_6(__global double *g_rhs, __global int *g_rd_log_rhs,
			    __global int *g_wr_log_rhs, int i, double dssp,
			    __global double *g_u, int __ocl_k_bound,
			    int __ocl_j_bound)
{
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void compute_rhs_7(__global double *g_rhs, __global int *g_rd_log_rhs,
			    __global int *g_wr_log_rhs, int i, double dssp,
			    __global double *g_u, int __ocl_k_bound,
			    int __ocl_j_bound)
{
...
__kernel void compute_rhs_8(__global double *g_vs, __global double *g_rhs,
			    __global int *g_rd_log_rhs,
			    __global int *g_wr_log_rhs, double dy1ty1,
			    __global double *g_u, double ty2, double dy2ty1,
			    double yycon2, __global double *g_us, double dy3ty1,
			    double con43, __global double *g_square, double c2,
			    double dy4ty1, __global double *g_ws, double dy5ty1,
			    double yycon3, __global double *g_qs, double yycon4,
			    double yycon5, __global double *g_rho_i, double c1,
			    int __ocl_k_bound, int __ocl_j_bound,
			    int __ocl_i_bound)
{
	//-------------------------------------------
	...
	//-------------------------------------------
	{
		...
		//      square[i][j - 1][k]
		//-------------------------------------------
		__global double *p_u_8_0 = &u[i][j - 1][k][1];
...
//			u_8[0] = vload2(0, p_u_8_0);
		...
		__global double *p_u_8_1 = &u[i][j - 1][k][3];
...
//			u_8[1] = vload2(0, p_u_8_1);
		...
		rhs[i][j][k][0] =
		    rhs[i][j][k][0] + dy1ty1 * (u[i][j + 1][k][0] -
						...
						[0]) - ty2 * (u_9[1] -
							      u_8[0].y);
		rhs[i][j][k][1] =
		    rhs[i][j][k][1] + dy2ty1 * (u_9[0] - 2.0 * u[i][j][k][1] +
						u_8[0].x) + yycon2 * (us[i][j +
									    1]
								      [k] -
								      2.0 *
								      us[i][j]
		...
		rhs[i][j][k][2] =
		    rhs[i][j][k][2] + dy3ty1 * (u_9[1] - 2.0 * u[i][j][k][2] +
						u_8[0].y) +
		    yycon2 * con43 * (vp1 - 2.0 * vijk + vm1) -
		    ty2 * (u_9[1] * vp1 - u_8[0].y * vm1 +
			   (u[i][j + 1][k][4] - square[i][j + 1][k] - u_8[1].y +
			    square_1) * c2);
		rhs[i][j][k][3] =
		    rhs[i][j][k][3] + dy4ty1 * (u_9[2] - 2.0 * u[i][j][k][3] +
						u_8[1].x) + yycon2 * (ws[i][j +
									    1]
								      [k] -
								      2.0 *
								      ws[i][j]
		...
		rhs[i][j][k][4] =
		    ...
		    yycon5 * (u[i][j + 1][k][4] * rho_i[i][j + 1][k] -
			      2.0 * u_9[3] * rho_i[i][j][k] +
			      u_8[1].y * rho_i[i][j - 1][k]) -
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void compute_rhs_9(__global double *g_rhs, __global int *g_rd_log_rhs,
			    __global int *g_wr_log_rhs, int j, double dssp,
			    __global double *g_u, int __ocl_k_bound,
			    int __ocl_i_bound)
{
	//-------------------------------------------
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void compute_rhs_10(__global double *g_rhs, __global int *g_rd_log_rhs,
			     __global int *g_wr_log_rhs, int j, double dssp,
			     __global double *g_u, int __ocl_k_bound,
			     int __ocl_i_bound)
{
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void compute_rhs_11(__global double *g_rhs, __global int *g_rd_log_rhs,
			     __global int *g_wr_log_rhs, double dssp,
			     __global double *g_u, __global int *grid_points,
			     __global int *rd_log_grid_points,
			     __global int *wr_log_grid_points,
			     int __ocl_k_bound, int __ocl_j_bound)
{
	//-------------------------------------------
	//OpenCL global indexes (BEGIN)
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void compute_rhs_12(__global double *g_rhs, __global int *g_rd_log_rhs,
			     __global int *g_wr_log_rhs, int j, double dssp,
			     __global double *g_u, int __ocl_k_bound,
			     int __ocl_i_bound)
{
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void compute_rhs_13(__global double *g_rhs, __global int *g_rd_log_rhs,
			     __global int *g_wr_log_rhs, int j, double dssp,
			     __global double *g_u, int __ocl_k_bound,
			     int __ocl_i_bound)
{
...
__kernel void compute_rhs_14(__global double *g_ws, __global double *g_rhs,
			     __global int *g_rd_log_rhs,
			     __global int *g_wr_log_rhs, double dz1tz1,
			     __global double *g_u, double tz2, double dz2tz1,
			     double zzcon2, __global double *g_us,
			     double dz3tz1, __global double *g_vs,
			     double dz4tz1, double con43,
			     ...
			     int __ocl_i_bound)
{
	...
	//-------------------------------------------
	{
		...
		//      rho_i[i][j][k]
		//-------------------------------------------
		__global double *p_ws_0_0 = &ws[i][j][k - 1];
...
//			ws_0 = vload2(0, p_ws_0_0);
		...
		u_10[6] = u[i][j][k - 1][4];
		u_10[7] = u[i][j][k][4];
		__global double *p_us_0_0 = &us[i][j][k - 1];
...
//			us_0 = vload2(0, p_us_0_0);
		...
		__global double *p_vs_0_0 = &vs[i][j][k - 1];
		...
		__global double *p_qs_0_0 = &qs[i][j][k - 1];
...
//			qs_0 = vload2(0, p_qs_0_0);
		...
		__global double *p_rho_i_0_0 = &rho_i[i][j][k - 1];
...
//			rho_i_0 = vload2(0, p_rho_i_0_0);
		...
		rhs[i][j][k][0] =
		    ...
		    tz2 * (u_10[5] - u_10[1]);
		rhs[i][j][k][1] =
		    rhs[i][j][k][1] + dz2tz1 * (u_10[2] - 2.0 * u[i][j][k][1] +
						u_10[0]) +
		    zzcon2 * (us[i][j][k + 1] - 2.0 * us_0.y + us_0.x) -
		    tz2 * (u_10[2] * wp1 - u_10[0] * wm1);
		rhs[i][j][k][2] =
		    rhs[i][j][k][2] + dz3tz1 * (u_10[4] - 2.0 * u[i][j][k][2] +
						u_10[3]) +
		    zzcon2 * (vs[i][j][k + 1] - 2.0 * vs_0.y + vs_0.x) -
		    tz2 * (u_10[4] * wp1 - u_10[3] * wm1);
		rhs[i][j][k][3] =
		    rhs[i][j][k][3] + dz4tz1 * (u_10[5] - 2.0 * u[i][j][k][3] +
						u_10[1]) +
		    zzcon2 * con43 * (wp1 - 2.0 * wijk + wm1) -
		    tz2 * (u_10[5] * wp1 - u_10[1] * wm1 +
			   (u[i][j][k + 1][4] - square[i][j][k + 1] - u_10[6] +
			    square_2) * c2);
		rhs[i][j][k][4] =
		    rhs[i][j][k][4] + dz5tz1 * (u[i][j][k + 1][4] -
						2.0 * u_10[7] + u_10[6]) +
		    zzcon3 * (qs[i][j][k + 1] - 2.0 * qs_0.y + qs_0.x) +
		...
		// GPU TLS logs (BEGIN) 
		//-------------------------------------------
		atom_inc(wr_log_rhs[i][j][k][0]);
		rd_log_rhs[i][j][k][0] = 1;
		atom_inc(wr_log_rhs[i][j][k][1]);
		rd_log_rhs[i][j][k][1] = 1;
		atom_inc(wr_log_rhs[i][j][k][2]);
		rd_log_rhs[i][j][k][2] = 1;
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void compute_rhs_15(__global double *g_rhs, __global int *g_rd_log_rhs,
			     __global int *g_wr_log_rhs, int k, double dssp,
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void compute_rhs_16(__global double *g_rhs, __global int *g_rd_log_rhs,
			     __global int *g_wr_log_rhs, int k, double dssp,
			     __global double *g_u, int __ocl_j_bound,
			     int __ocl_i_bound)
{
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void compute_rhs_17(__global double *g_rhs, __global int *g_rd_log_rhs,
			     __global int *g_wr_log_rhs, double dssp,
			     __global double *g_u, __global int *grid_points,
			     __global int *rd_log_grid_points,
			     __global int *wr_log_grid_points,
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void compute_rhs_18(__global double *g_rhs, __global int *g_rd_log_rhs,
			     __global int *g_wr_log_rhs, int k, double dssp,
			     __global double *g_u, int __ocl_j_bound,
			     int __ocl_i_bound)
{
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void compute_rhs_19(__global double *g_rhs, __global int *g_rd_log_rhs,
			     __global int *g_wr_log_rhs, int k, double dssp,
			     __global double *g_u, int __ocl_j_bound,
			     int __ocl_i_bound)
{
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void compute_rhs_20(__global double *g_rhs, __global int *g_rd_log_rhs,
			     __global int *g_wr_log_rhs, double dt,
			     __global int *grid_points,
			     __global int *rd_log_grid_points,
			     __global int *wr_log_grid_points,
			     int __ocl_k_bound, int __ocl_j_bound)
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void x_backsubstitute_0(__global double *g_rhs,
				 __global int *g_rd_log_rhs,
				 __global int *g_wr_log_rhs, int i,
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void x_solve_cell_0(__global double *g_lhs, __global int *g_rd_log_lhs,
			     __global int *g_wr_log_lhs, __global double *g_rhs,
			     __global int *g_rd_log_rhs,
			     __global int *g_wr_log_rhs, int __ocl_k_bound,
			     int __ocl_j_bound)
{
	//-------------------------------------------
	//OpenCL global indexes (BEGIN)
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void x_solve_cell_1(__global double *g_lhs, __global int *g_rd_log_lhs,
			     __global int *g_wr_log_lhs, int i,
			     __global double *g_rhs, __global int *g_rd_log_rhs,
			     __global int *g_wr_log_rhs, int __ocl_k_bound,
			     int __ocl_j_bound)
{
	//-------------------------------------------
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void x_solve_cell_2(__global double *g_lhs, __global int *g_rd_log_lhs,
			     __global int *g_wr_log_lhs, int isize,
			     __global double *g_rhs, __global int *g_rd_log_rhs,
			     __global int *g_wr_log_rhs, int i,
			     int __ocl_k_bound, int __ocl_j_bound)
{
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void y_backsubstitute_0(__global double *g_rhs,
				 __global int *g_rd_log_rhs,
				 __global int *g_wr_log_rhs, int j,
				 __global double *g_lhs,
				 __global int *grid_points,
				 __global int *rd_log_grid_points,
				 __global int *wr_log_grid_points,
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void y_solve_cell_0(__global double *g_lhs, __global int *g_rd_log_lhs,
			     __global int *g_wr_log_lhs, __global double *g_rhs,
			     __global int *g_rd_log_rhs,
			     __global int *g_wr_log_rhs, int __ocl_k_bound,
			     int __ocl_i_bound)
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void y_solve_cell_1(__global double *g_lhs, __global int *g_rd_log_lhs,
			     __global int *g_wr_log_lhs, int j,
			     __global double *g_rhs, __global int *g_rd_log_rhs,
			     __global int *g_wr_log_rhs, int __ocl_k_bound,
			     int __ocl_i_bound)
{
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void y_solve_cell_2(__global double *g_lhs, __global int *g_rd_log_lhs,
			     __global int *g_wr_log_lhs, int jsize,
			     __global double *g_rhs, __global int *g_rd_log_rhs,
			     __global int *g_wr_log_rhs, int __ocl_k_bound,
			     int __ocl_i_bound)
{
...
__kernel void z_backsubstitute_0(__global int *grid_points,
				 ...
				 __global int *g_rd_log_rhs,
				 __global int *g_wr_log_rhs,
				 __global double *g_lhs, int __ocl_i_bound)
{
	//-------------------------------------------
	//OpenCL global indexes (BEGIN)
	//-------------------------------------------
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void z_solve_cell_0(__global double *g_lhs, __global int *g_rd_log_lhs,
			     __global int *g_wr_log_lhs, __global double *g_rhs,
			     __global int *g_rd_log_rhs,
			     __global int *g_wr_log_rhs, int __ocl_j_bound,
			     int __ocl_i_bound)
{
	//-------------------------------------------
	//OpenCL global indexes (BEGIN)
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void z_solve_cell_1(__global double *g_lhs, __global int *g_rd_log_lhs,
			     __global int *g_wr_log_lhs, int k,
			     __global double *g_rhs, __global int *g_rd_log_rhs,
			     __global int *g_wr_log_rhs, int __ocl_j_bound,
			     int __ocl_i_bound)
{
...
//The nested loops were swaped. 
//-------------------------------------------------------------------------------
__kernel void z_solve_cell_2(__global double *g_lhs, __global int *g_rd_log_lhs,
			     __global int *g_wr_log_lhs, int ksize,
			     __global double *g_rhs, __global int *g_rd_log_rhs,
			     __global int *g_wr_log_rhs, int __ocl_j_bound,
			     int __ocl_i_bound)
{
