...
featFuncsTest = [
             ...
             lambda f: f["CoalescedMemAccess"] / f["GlobalMemUse"],
	         lambda f: f["LocalMemUse"] / (f["GlobalMemUse"] ) * f["AvgWorksize"],
			 #lambda f: (f["FloatOps"] / 1.5  + 4*f["Float4Ops"] + f["IntOps"] + 4*f["Int4Ops"] )/f["GlobalMemUse"],
			 lambda f: (f["FloatOps"] + 4*f["Float4Ops"])/f["GlobalMemUse"],
	         #lambda f: log(f["AvgWorksize"],2),
            ]
...
def read_features (filename):
  ...
  if "KernelCalls" not in featVals:
    featVals["KernelCalls"] = 1
  if "Kernels" not in featVals:
    featVals["Kernels"] = 1
...
def collect_train (cpu_name, gpu_name):
  ...
  ret = collect_train_data (cursCPU, cursGPU, kernelList, kernelConstr, kernelOrder, featFuncsTrain)
...
def collect_test_data (cursCPU, cursGPU, benchmarks, featFuncs):
...
def collect_test (cpu_name, gpu_name):
  ...
  ret = collect_test_data (cursCPU, cursGPU, benchmarks, featFuncsTest)
